...
static int parse_type_name(parser_t *p, cc_type_t *out_type, int *out_struct_id, long *out_array_len,
                           int *out_array_ndim, long out_array_dims[CC_MAX_ARRAY_DIMS], int allow_void,
                           const char *what);
...
static uint64_t parser_const_mask_bits(const parser_t *p, uint64_t bits, cc_type_t t, int struct_id);
...
static int parser_allow_static_assert_extension(void) {
...
static uint64_t parser_const_mask_bits(const parser_t *p, uint64_t bits, cc_type_t t, int struct_id) {
    int width = parser_const_type_bits(p, t, struct_id);
...
static int64_t parser_const_signed_value(const parser_t *p, uint64_t bits, cc_type_t t, int struct_id) {
    int width = parser_const_type_bits(p, t, struct_id);
    uint64_t u = parser_const_mask_bits(p, bits, t, struct_id);

    if (width <= 0) {
        return 0;
    }
    if (width < 64 && !parser_is_unsigned_integral_type(t) && !is_pointer_type(t) &&
        (u & (1ULL << (width - 1))) != 0) {
        u |= ~((1ULL << width) - 1ULL);
    }
    return (int64_t)u;
...
static uint64_t parser_const_cast_value_bits(const parser_t *p, uint64_t src_bits, cc_type_t src_type, int src_struct_id,
    ...
    if (dst_type == CC_TYPE_BOOL) {
        ...
        return ((parser_is_unsigned_integral_type(src_type) || is_pointer_type(src_type))
                    ? parser_const_unsigned_value(p, src_bits, src_type, src_struct_id)
                    : (uint64_t)parser_const_signed_value(p, src_bits, src_type, src_struct_id)) != 0
                   ? 1
                   : 0;
    }
    if (parser_is_unsigned_integral_type(src_type) || is_pointer_type(src_type)) {
        raw = parser_const_unsigned_value(p, src_bits, src_type, src_struct_id);
    } else {
        raw = (uint64_t)parser_const_signed_value(p, src_bits, src_type, src_struct_id);
    }
    if (ok != NULL) {
        *ok = 1;
    }
...
#define RETURN_OK()                                                                                                  \
    ...
    while (1) {
        ...
        if (p->tok.kind == TOK_IDENT) {
            ...
            if (tok_is_ident(p, "_Alignas") || (parser_is_c23_or_newer() && tok_is_ident(p, "alignas"))) {
                ...
                if (is_declspec_start(p)) {
                    if (parse_type_name(p, &aty, &asid, NULL, NULL, NULL, 1,
                                        "expected type name in _Alignas/alignas") != 0) {
                        return -1;
                    }
            ...
            if (tok_is_ident(p, "_Atomic")) {
                ...
                if (peek_kind(p) == TOK_LPAREN) {
                    ...
                    if (parse_type_name(p, &aty, &asid, NULL, NULL, NULL, 1, "expected type name in _Atomic(...)") !=
                        0) {
                        return -1;
                    }
        ...
        case TOK_KW_UNION: {
            ...
            while (p->tok.kind != TOK_RBRACE) {
                ...
                for (;;) {
                    ...
                    if (mbase_arr_ndim > 0 && is_pointer_type(mtype)) {
                        cc_type_t elem_type = ptr_deref_type(mtype);
                        long elem_size = parser_type_size_bytes(p, elem_type, mstruct);
                        long elem_align = parser_type_align_bytes(p, elem_type, mstruct);
                        if (elem_size > 0) {
                            msize = elem_size;
                        }
                        if (elem_align > 0) {
        ...
        case TOK_KW_ENUM: {
            ...
            if (parser_is_c23_or_newer() && p->tok.kind == TOK_COLON) {
                ...
                if (parse_type_name(p, &uty, &usid, NULL, NULL, NULL, 1, "expected enum underlying type") != 0) {
                    return -1;
                }
                if (usid >= 0) {
    ...
    if (!seen) {
        ...
        if (what != NULL && near_tok[0] != '\0') {
            char msg[160];
            snprintf(msg, sizeof(msg), "%s near '%s'", what, near_tok);
    ...
    if (!seen_type) {
        if (g_parser_allow_oldstyle_funcdecl && seen) {
            /*
             * GNU toolchains accept implicit int in declaration-specifier
             * lists such as `extern foo();`. Keep this enabled in legacy/GNU
             * modes so autoconf probes compile.
             */
            seen_int = 1;
            seen_type = 1;
        } else {
...
static int eval_const_expr_value(parser_t *p, const cc_expr_t *e, parser_const_value_t *out) {
    ...
    switch (e->kind) {
    case CC_EXPR_INT:
        out->type = e->value_type != CC_TYPE_VOID ? e->value_type : CC_TYPE_INT;
        out->struct_id = e->struct_id;
        out->bits = parser_const_mask_bits(p, (uint64_t)e->int_val, out->type, out->struct_id);
        return 0;
    case CC_EXPR_CAST: {
        int ok = 0;
        ...
        out->type = e->aux_type;
        out->struct_id = e->aux_struct_id;
        out->bits = parser_const_cast_value_bits(p, lhs.bits, lhs.type, lhs.struct_id, out->type, out->struct_id, &ok);
        return ok ? 0 : -1;
    }
    case CC_EXPR_SIZEOF:
        out->type = g_parser_pointer_size_bytes >= 8 ? CC_TYPE_ULONG : CC_TYPE_UINT;
        out->struct_id = -1;
        if (e->lhs != NULL && e->lhs->kind == CC_EXPR_STR) {
            ...
            if (bound > 0 && elem_size > 0 && bound <= LONG_MAX / elem_size) {
                out->bits = parser_const_mask_bits(p, (uint64_t)(bound * elem_size), out->type, out->struct_id);
                return 0;
            }
        }
        if (e->lhs != NULL && e->lhs->kind == CC_EXPR_IDENT && e->lhs->ident != NULL) {
            ...
            if (vidx >= 0 && p->vars[vidx].array_ndim > 0) {
                ...
                out->bits = parser_const_mask_bits(p, (uint64_t)total, out->type, out->struct_id);
                return 0;
            }
            if (vidx < 0 && p->tu != NULL) {
                size_t gi;
        ...
        }
        {
            ...
            if (is_pointer_type(st)) {
                out->bits = parser_const_mask_bits(p, (uint64_t)g_parser_pointer_size_bytes, out->type, out->struct_id);
                return 0;
            }
    ...
    case CC_EXPR_BIN:
        ...
        case CC_BIN_GE: {
            cc_type_t op_ty = (e->op == CC_BIN_SHL || e->op == CC_BIN_SHR)
                                  ? parser_const_result_type(p, e->lhs)
                                  : parser_common_arith_type(parser_const_result_type(p, e->lhs),
                                                             parser_const_result_type(p, e->rhs));
            uint64_t lhs_bits;
            uint64_t rhs_bits;
            uint64_t ua;
            uint64_t ub;
            ...
            lhs_bits = parser_const_cast_value_bits(p, lhs.bits, lhs.type, lhs.struct_id, op_ty, -1, &ok_a);
            rhs_bits = parser_const_cast_value_bits(p, rhs.bits, rhs.type, rhs.struct_id, op_ty, -1, &ok_b);
            if (!ok_a || !ok_b) {
                return -1;
            }
            ua = parser_const_unsigned_value(p, lhs_bits, op_ty, -1);
            ub = parser_const_unsigned_value(p, rhs_bits, op_ty, -1);
            sa = parser_const_signed_value(p, lhs_bits, op_ty, -1);
            sb = parser_const_signed_value(p, rhs_bits, op_ty, -1);

            switch (e->op) {
            case CC_BIN_ADD:
                out->type = op_ty;
                out->bits = parser_const_mask_bits(
                    p, parser_is_unsigned_integral_type(op_ty) ? (ua + ub) : (uint64_t)(sa + sb), op_ty, -1);
                out->struct_id = -1;
                return 0;
            case CC_BIN_SUB:
                out->type = op_ty;
            ...
            case CC_BIN_SHL:
                out->type = op_ty;
                out->bits = parser_const_mask_bits(
                    p, parser_is_unsigned_integral_type(op_ty) ? (ua << (ub & 63)) : (uint64_t)(sa << (ub & 63)),
                    op_ty, -1);
                out->struct_id = -1;
                return 0;
            case CC_BIN_SHR:
            ...
            case CC_BIN_BAND:
                out->type = op_ty;
                out->bits = parser_const_mask_bits(p, ua & ub, op_ty, -1);
                out->struct_id = -1;
                return 0;
            case CC_BIN_BOR:
                out->type = op_ty;
            ...
            case CC_BIN_LT:
                out->type = CC_TYPE_INT;
                out->bits = parser_is_unsigned_integral_type(op_ty) ? ((ua < ub) ? 1 : 0) : ((sa < sb) ? 1 : 0);
                out->struct_id = -1;
                return 0;
            case CC_BIN_LE:
                out->type = CC_TYPE_INT;
    ...
    case CC_EXPR_TERNARY:
        ...
        if (((parser_is_unsigned_integral_type(cond.type) || is_pointer_type(cond.type))
...
static int eval_const_array_bound_expr(parser_t *p, const cc_expr_t *e, long *out) {
    ...
    if (eval_const_expr_value(p, e, &v) != 0) {
        return -1;
    }
    if (parser_is_unsigned_integral_type(v.type) || is_pointer_type(v.type)) {
        u = parser_const_unsigned_value(p, v.bits, v.type, v.struct_id);
        if (u > (uint64_t)LONG_MAX) {
            return -1;
        }
        *out = (long)u;
        return 0;
    }
    s = parser_const_signed_value(p, v.bits, v.type, v.struct_id);
    if (s < (int64_t)LONG_MIN || s > (int64_t)LONG_MAX) {
        return -1;
    }
    *out = (long)s;
    return 0;
...
static int parse_array_extent(parser_t *p, long *out_n, int *out_const_n, cc_expr_t **out_bound_expr) {
    ...
    if (eval_const_array_bound_expr(p, bound, &n) == 0) {
        if (n < 0 || (!parser_is_gnu_mode() && n == 0)) {
            set_diag(p->diag, p->tok.line, p->tok.col, "array bound must be positive");
            free_expr(bound);
            return -1;
        }
        if (n > 0 || (parser_is_gnu_mode() && n == 0)) {
...
static unsigned parser_decode_escape_unit(const unsigned char *s, size_t n, size_t *adv_out) {
    unsigned v = 0;
    size_t i = 0;
    ...
    if (s[0] == 'x') {
        ...
        int seen = 0;
        i = 1;
        while (i < n && (hv = parser_hex_value(s[i])) >= 0) {
            seen = 1;
            v = (v << 4) | (unsigned)hv;
            i++;
        }
    ...
    if (s[0] >= '0' && s[0] <= '7') {
        int digits = 0;
        while (i < n && digits < 3 && s[i] >= '0' && s[i] <= '7') {
...
static char *canonicalize_narrow_string_token(const cc_token_t *tok) {
    ...
    if (tok == NULL || tok->kind != TOK_STR || tok->start == NULL) {
        return NULL;
    }
    end = tok->len;
    if (tok->len >= 2 && tok->start[0] == '"' && tok->start[tok->len - 1] == '"') {
    ...
    s = (const unsigned char *)tok->start;
    out[w++] = '"';
    i = begin;
    while (i < end) {
        unsigned ch;
        ...
        if (s[i] == '\\') {
            ...
            ch = parser_decode_escape_unit(s + i, end - i, &adv);
            if (adv == 0) {
                adv = 1;
            }
            i += adv;
        ...
        snprintf(out + w, cap - w, "\\%03o", ch & 0xffu);
        w += 4;
    }
    out[w++] = '"';
    out[w] = '\0';
...
static int parse_type_name(parser_t *p, cc_type_t *out_type, int *out_struct_id, long *out_array_len,
    ...
    if (parse_declspec(p, &ty, &sid, &decl_array_len, &decl_array_ndim, decl_array_dims, allow_void, what, NULL, NULL,
                       NULL) != 0) {
        return -1;
    }
    while (is_ptr_declarator_tok(p->tok.kind)) {
    ...
    while (p->tok.kind == TOK_LPAREN) {
        ...
        if (p->tok.kind == TOK_LBRACK) {
            ...
            if (prepend_array_info(&decl_array_len, &decl_array_ndim, decl_array_dims, suffix_array_len,
                                   suffix_array_ndim, suffix_array_dims) != 0) {
                set_diag(p->diag, p->tok.line, p->tok.col, "array rank > 4 is not yet supported");
                return -1;
            }
        }
...
static int parse_static_assert_decl(parser_t *p, int require_semi) {
    ...
    if (!parser_is_c11_or_newer() && !parser_allow_static_assert_extension()) {
    ...
    } else if (!parser_is_c23_or_newer() && !parser_allow_static_assert_extension()) {
        free_expr(cond_expr);
...
static cc_expr_t *parse_generic_expr(parser_t *p) {
    ...
    while (p->tok.kind != TOK_RPAREN) {
        ...
        } else if (parse_type_name(p, &assoc.type, &assoc.struct_id, NULL, NULL, NULL, 1,
                                   "expected type name in _Generic") != 0) {
            free_expr(control);
            free(items);
            return NULL;
        }
...
static cc_expr_t *parse_primary(parser_t *p) {
    ...
    if (p->tok.kind == TOK_STR) {
        ...
        char *lit = str_ty == CC_TYPE_CHAR ? canonicalize_narrow_string_token(&p->tok)
                                           : xstrdup_n(p->tok.start, p->tok.len);
        ...
        while (p->tok.kind == TOK_STR) {
            ...
            char *part = part_ty == CC_TYPE_CHAR ? canonicalize_narrow_string_token(&p->tok)
                                                 : xstrdup_n(p->tok.start, p->tok.len);
            size_t alen;
            size_t plen;
            char *next_lit;
    ...
    if (p->tok.kind == TOK_IDENT) {
        ...
        if (p->tok.kind == TOK_LPAREN) {
            ...
            if (strcmp(e->ident, "__builtin_offsetof") == 0) {
                ...
                if (parse_type_name(p, &sty, &ssid, NULL, NULL, NULL, 1,
                                    "expected type name in __builtin_offsetof") != 0) {
                    free_expr(e);
                    return NULL;
                }
                if (ssid < 0) {
            ...
            if (strcmp(e->ident, "__builtin_va_arg") == 0) {
                ...
                if (parse_type_name(p, &aty, &asid, NULL, NULL, NULL, 1,
                                    "expected type name in __builtin_va_arg") != 0) {
                    free_expr(e);
                    return NULL;
                }
                e->aux_type = aty;
...
static cc_expr_t *clone_expr(const cc_expr_t *src) {
    ...
    dst->float_val_ld = src->float_val_ld;
    dst->op = src->op;
...
static cc_expr_t *parse_unary(parser_t *p) {
    ...
    if (p->tok.kind == TOK_KW_SIZEOF) {
        ...
        if (is_type_name_start_after_lparen(p)) {
            ...
            e->array_ndim = sizeof_array_ndim;
            memcpy(e->array_dims, sizeof_array_dims, sizeof(e->array_dims));
    ...
    if (p->tok.kind == TOK_PLUS) {
        ...
        /*
         * Unary plus performs integer promotions. Model it as multiplication
         * by one so existing usual-arithmetic-conversion semantics apply, and
         * non-arithmetic operands are still rejected.
         */
        one = new_int_expr(1);
        if (one == NULL) {
            free_expr(rhs);
        ...
        return new_bin_expr(CC_BIN_MUL, one, rhs);
    }
...
static int parse_function(parser_t *p, cc_function_t *f) {
    ...
    if (is_declspec_start(p)) {
    ...
    } else if (g_parser_allow_oldstyle_funcdecl) {
        /*
         * Legacy/GNU compatibility: allow omitted return type in function
         * definitions/declarations (`main() { ... }` => `int main() { ... }`).
         */
        ftype = CC_TYPE_INT;
        ftype_sid = -1;
        fn_storage = 0;
        ...
        set_diag(p->diag, p->tok.line, p->tok.col, "expected function return type");
        return -1;
    }
    if (p->tok.kind == TOK_LPAREN && is_ptr_declarator_tok(peek_kind(p))) {
    ...
    if ((f->storage & CC_STORAGE_INLINE) != 0 && (f->storage & (CC_STORAGE_STATIC | CC_STORAGE_EXTERN)) == 0 &&
        g_parser_gnu89_inline) {
        /*
         * GNU89 inline mode keeps plain inline definitions externally
         * visible unless explicitly static.
         */
    ...
    if (oldstyle_decl_count > 0) {
        f->has_oldstyle_param_decls = 1;
    }
