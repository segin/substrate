...
- pointer expressions: unary address-of (`&`) for identifier lvalues, unary dereference (`*`) for pointers, pointer/null equality comparisons, pointer argument/return lowering, and indirect assignment through pointers (`*p = expr`)
...
- no structs/arrays yet (pointer arithmetic and compound assignments on indirect lvalues are still limited)
