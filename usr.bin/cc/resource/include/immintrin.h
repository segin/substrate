...
static __inline__ __m128i _mm_clmulepi64_si128(__m128i a, __m128i b, const int imm8) {
	__m128i_u ua, ub, r;
	uint64_t lo, hi;
	unsigned sa = ((unsigned)imm8 & 0x10u) ? 1u : 0u;
	unsigned sb = ((unsigned)imm8 & 0x01u) ? 1u : 0u;
	ua.v = a;
	ub.v = b;
...
static __inline__ __m256i _mm256_clmulepi64_epi128(__m256i a, __m256i b, const int imm8) {
	__m256i_u ua, ub, r;
	unsigned lane;
	ua.v = a;
	ub.v = b;
...
static __inline__ __m128i _mm_shuffle_epi8(__m128i a, __m128i mask) {
	...
	um.v = mask;
	for(i=0;i<16;i++) {
		unsigned char sel = um.u8[i];
		if((sel & 0x80u) != 0) {
			r.u8[i] = 0u;
		} else {
			r.u8[i] = ua.u8[sel & 0x0fu];
		}
	}
	return(r.v);
}
