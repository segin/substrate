            ...
            if (o.bootstrap_gcc) {
                ...
                if (cc_compile_c_to_s(out_pp, in, out_s, o.debug, o.target, opt_level_num(&o), &diag) != 0) {
                    ...
                    fprintf(stderr,
                            ...
                            "(int/bool/char/unsigned-char/short/unsigned-short/unsigned-int/long-long/unsigned-long-long/float/double scalar + one/two/three-level pointer functions/prototypes, declarations (including C99 for-init declarations), assignments/calls, unary address-of/dereference pointer ops, pointer arithmetic/indexing (`ptr +/- int`, `int + ptr`, compatible `ptr - ptr` for non-void pointers, and `ptr[idx]`/`idx[ptr]`), compound assignments, ++/--, if/else, while/do/for, switch/case/default, goto/labels, break/continue, C95 digraph/trigraph lexical forms, arithmetic/logical/bitwise/shift/comma operators (including &&/|| short-circuit), ternary `?:`, scalar casts, pointer/integer casts, `sizeof` on supported scalar types, numeric comparisons, and returns)\n");
                    goto out;
                }
            }
