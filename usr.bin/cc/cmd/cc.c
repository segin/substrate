...
static int gcc_print_file_name(const char *name, cc_target_t target,
  ...
  if (snprintf(opt, sizeof(opt), "-print-file-name=%s", name) >=
      (int)sizeof(opt)) {
    return -1;
  }
  argv[0] = "gcc";
...
static int toolchain_guard_enter(const char *tool) {
  ...
  max_depth =
      parse_env_long(getenv(SUBSTRATE_TC_MAX_ENV), SUBSTRATE_TC_DEFAULT_MAX);
  if (max_depth < 4) {
    max_depth = 4;
  }
  if (depth >= max_depth) {
    ...
    fprintf(
        ...
        depth, max_depth, trace != NULL && trace[0] != '\0' ? ", trace: " : "",
...
static int add_gcc_system_include(cc_opts_t *o, const char *name) {
  ...
  if (strvec_push(&o->cpp_flags, "-isystem") != 0 ||
      strvec_push(&o->cpp_flags, path) != 0) {
    return -1;
  }
  return 0;
}
...
static int make_temp_path(cc_opts_t *o, const char *prefix, const char *suffix,
  ...
  if (suffix != NULL) {
    ...
    if (asprintf(&with_suffix, "%s%s", templ, suffix) < 0) {
      unlink(templ);
      free(templ);
      return -1;
    }
    ...
    if (snprintf(out, PATH_MAX, "%s", with_suffix) >= PATH_MAX) {
      ...
      ret = strvec_push(&o->temp_files, out);
    }
    free(with_suffix);
  } else {
...
static int i386_features_from_arch_name(const char *name, int *out_level,
                                        int *out_has_sse, int *out_has_sse2,
                                        int *out_has_mmx, int *out_supports_sse,
                                        int *out_supports_sse2,
  ...
  if (strcmp(name, "i686") == 0 || strcmp(name, "pentiumpro") == 0 ||
      strcmp(name, "pentium2") == 0) {
    *out_level = 6;
    *out_has_sse = 0;
    *out_has_sse2 = 0;
    *out_has_mmx = 1;
  ...
  if (strcmp(name, "pentium4") == 0 || strcmp(name, "prescott") == 0 ||
      strcmp(name, "nocona") == 0 || strcmp(name, "core2") == 0 ||
      strcmp(name, "x86-64") == 0 || strcmp(name, "x86-64-v1") == 0 ||
      strcmp(name, "x86-64-v2") == 0 || strcmp(name, "x86-64-v3") == 0 ||
      strcmp(name, "x86-64-v4") == 0) {
    *out_level = 7;
    *out_has_sse = 1;
    *out_has_sse2 = 0;
    *out_has_mmx = 1;
...
static int parse_args(int argc, char **argv, cc_opts_t *o) {
  ...
  for (i = 1; i < argc; ++i) {
    ...
    if (strcmp(a, "-MF") == 0 || strcmp(a, "-MT") == 0 ||
        strcmp(a, "-MQ") == 0 || strcmp(a, "-include") == 0 ||
        strcmp(a, "-imacros") == 0) {
      if (i + 1 >= argc) {
        return -1;
      }
      if (strvec_push(&o->cpp_flags, a) != 0 ||
          strvec_push(&o->cpp_flags, argv[i + 1]) != 0) {
        return -1;
      }
      ++i;
      continue;
    }
    if (strncmp(a, "-MF", 3) == 0 || strncmp(a, "-MT", 3) == 0 ||
        strncmp(a, "-MQ", 3) == 0 || strncmp(a, "-include", 8) == 0 ||
        strncmp(a, "-imacros", 8) == 0) {
      if (strvec_push(&o->cpp_flags, a) != 0) {
        return -1;
      }
      continue;
    ...
    if (strcmp(a, "-march") == 0 || strcmp(a, "-mcpu") == 0 ||
        strcmp(a, "-mtune") == 0) {
      int level = 0;
      int has_sse = 0;
      int has_sse2 = 0;
      ...
      if (strvec_push(&o->c_flags, a) != 0 ||
          strvec_push(&o->c_flags, argv[i + 1]) != 0 ||
          strvec_push(&o->cpp_flags, a) != 0 ||
          strvec_push(&o->cpp_flags, argv[i + 1]) != 0) {
        return -1;
      }
      if (!is_tune && i386_features_from_arch_name(
                          argv[i + 1], &level, &has_sse, &has_sse2, &has_mmx,
    ...
    if (strncmp(a, "-march=", 7) == 0 || strncmp(a, "-mcpu=", 6) == 0 ||
        strncmp(a, "-mtune=", 7) == 0) {
      int level = 0;
      int has_sse = 0;
      int has_sse2 = 0;
      ...
      if (!is_tune &&
          i386_features_from_arch_name(
              eq != NULL ? eq + 1 : NULL, &level, &has_sse, &has_sse2, &has_mmx,
              &supports_sse, &supports_sse2, &supports_mmx)) {
        o->i386_isa_level = level;
        o->i386_has_sse = has_sse;
        o->i386_has_sse2 = has_sse2;
    ...
    if (strcmp(a, "-msse") == 0 || strcmp(a, "-mno-sse") == 0) {
      ...
      if (strcmp(a, "-msse") == 0) {
        if (o->target == CC_TARGET_I386 && !o->i386_supports_sse) {
          fprintf(
              stderr,
              "cc: error: -msse is not supported for selected -march level\n");
          return -1;
        }
        o->i386_has_sse = 1;
      } else {
    ...
    if (strcmp(a, "-msse2") == 0 || strcmp(a, "-mno-sse2") == 0) {
      ...
      if (strcmp(a, "-msse2") == 0) {
        if (o->target == CC_TARGET_I386 && !o->i386_supports_sse2) {
          fprintf(
              stderr,
              "cc: error: -msse2 is not supported for selected -march level\n");
          return -1;
        }
        o->i386_has_sse2 = 1;
      } else {
    ...
    if (strcmp(a, "-mmmx") == 0 || strcmp(a, "-mno-mmx") == 0) {
      ...
      if (strcmp(a, "-mmmx") == 0) {
        if (o->target == CC_TARGET_I386 && !o->i386_supports_mmx) {
          fprintf(
              stderr,
              "cc: error: -mmmx is not supported for selected -march level\n");
          return -1;
        }
        o->i386_has_mmx = 1;
      } else {
    ...
    if (strcmp(a, "-mavx") == 0 || strcmp(a, "-mno-avx") == 0 ||
        strcmp(a, "-mavx2") == 0 || strcmp(a, "-mno-avx2") == 0) {
      if (strvec_push(&o->c_flags, a) != 0 ||
          strvec_push(&o->cpp_flags, a) != 0) {
        return -1;
      }
    ...
    if (strcmp(a, "-mfpmath=sse") == 0 || strcmp(a, "-mfpmath=387") == 0) {
      ...
      o->i386_fp_math_mode =
          strcmp(a, "-mfpmath=387") == 0 ? I386_FPMATH_387 : I386_FPMATH_SSE;
      continue;
    }
    ...
    if (strcmp(a, "-L") == 0 || strcmp(a, "-l") == 0) {
      ...
      if (strvec_push(&o->ld_flags, a) != 0 ||
          strvec_push(&o->ld_flags, argv[i + 1]) != 0) {
        return -1;
      }
      ++i;
      continue;
    ...
    if (strcmp(a, "-fpp-max-include-depth") == 0 ||
        strcmp(a, "-fpp-max-macro-depth") == 0 ||
        strcmp(a, "-fpp-max-expand-passes") == 0 ||
        strcmp(a, "-fpp-max-expanded-bytes") == 0 ||
        strcmp(a, "-fpp-max-output-bytes") == 0 ||
        strcmp(a, "-fpp-max-tokens") == 0) {
      if (i + 1 >= argc) {
        return -1;
      }
      if (strvec_push(&o->cpp_flags, a) != 0 ||
    ...
    if (strncmp(a, "-fpp-max-include-depth=", 24) == 0 ||
        strncmp(a, "-fpp-max-macro-depth=", 22) == 0 ||
        strncmp(a, "-fpp-max-expand-passes=", 24) == 0 ||
        strncmp(a, "-fpp-max-expanded-bytes=", 24) == 0 ||
        strncmp(a, "-fpp-max-output-bytes=", 22) == 0 ||
        strncmp(a, "-fpp-max-tokens=", 17) == 0) {
      if (strvec_push(&o->cpp_flags, a) != 0) {
        return -1;
      }
      continue;
  ...
  if (o->inputs.count == 0 && !o->query_print_file_name &&
      !o->query_print_libgcc_file_name) {
...
static int run_preprocess(const cc_opts_t *o, const char *in, const char *out,
  ...
  if (cc_preprocess_file(pp_in, out, std_mode, pp_flags, pp_count, &diag) !=
      0) {
    free(pp_flags_aug);
    if (use_stdin_tmp) {
      unlink(stdin_tmp);
  ...
  if (as_cpp_mode) {
    ...
    if (fp == NULL) {
      ...
      fprintf(stderr,
              "cpp: error: failed to reopen preprocessed assembly output\n");
      return -1;
    }
    if (fseek(fp, 0, SEEK_END) != 0) {
      ...
      fprintf(stderr,
              "cpp: error: failed to size preprocessed assembly output\n");
      return -1;
    }
    sz = ftell(fp);
    if (sz < 0 || fseek(fp, 0, SEEK_SET) != 0) {
      ...
      fprintf(stderr,
              "cpp: error: failed to read preprocessed assembly output\n");
      return -1;
    }
    ...
    if (src == NULL || dst == NULL) {
      ...
      fprintf(stderr,
              "cpp: error: out of memory normalizing assembly output\n");
      return -1;
    }
    ...
    if (fp == NULL || fwrite(dst, 1, w, fp) != w) {
      ...
      fprintf(stderr,
              "cpp: error: failed writing normalized assembly output\n");
      return -1;
    }
    fclose(fp);
    free(src);
...
static int run_bootstrap_frontend(const cc_opts_t *o, const char *in_c,
                                  const char *out_s) {
  char stdflag[64];
  ...
  argv[at++] = "-x";
  argv[at++] = "c";
  snprintf(stdflag, sizeof(stdflag), "-std=%s",
           o->std != NULL ? o->std : "gnu17");
  argv[at++] = stdflag;
  if (want_trigraphs) {
    argv[at++] = "-trigraphs";
  }
...
static const char *resolve_tool_default(const cc_opts_t *o,
                                        const char *env_name,
                                        const char *tool_name,
                                        const char *sibling_dir, char *buf,
                                        size_t bufsz) {
  const char *override;
  const char *self;
  const char *slash;
  ...
  if (o == NULL || o->self_path == NULL || o->self_path[0] == '\0' ||
      buf == NULL || bufsz == 0) {
    return tool_name;
  }
  ...
  if (snprintf(buf, bufsz, "%.*s/../%s/%s", (int)dirlen, self, sibling_dir,
               tool_name) > 0 &&
      access(buf, X_OK) == 0) {
    return buf;
  }
  if (snprintf(buf, bufsz, "%.*s/%s", (int)dirlen, self, tool_name) > 0 &&
      access(buf, X_OK) == 0) {
    return buf;
  }
  return tool_name;
...
static int run_emit_ssa_tool(const cc_opts_t *o, const char *self,
                             const char *ir_file) {
  char *argv[3];
  ...
  if (slash != NULL) {
    ...
    if (dlen + strlen("/ir-verifier") + 1 < sizeof(tool_path)) {
      snprintf(tool_path, sizeof(tool_path), "%.*s/ir-verifier", (int)dlen,
               self);
      argv[0] = tool_path;
    } else {
      argv[0] = "ir-verifier";
...
static int maybe_print_info_and_exit(int argc, char **argv) {
  ...
  for (i = 1; i < argc; ++i) {
    if (strcmp(argv[i], "-m32") == 0) {
      ...
      info = argv[i];
               ...
               strcmp(argv[i], "-dumpfullversion") == 0) {
      info = argv[i];
  ...
  if (strcmp(info, "-dumpversion") == 0 ||
      strcmp(info, "-dumpfullversion") == 0) {
    printf("13.2.0\n");
    return 1;
  }
  if (strcmp(info, "-dumpmachine") == 0) {
    printf("%s\n", target == CC_TARGET_I386 ? "i386-pc-linux-gnu"
                                            : "x86_64-pc-linux-gnu");
    return 1;
  }
  return 0;
...
int cc_main(int argc, char **argv) {
  ...
  if (o.emit_ssa) {
    if (o.inputs.count != 1 || !has_ext(o.inputs.items[0], ".ir")) {
      fprintf(stderr,
              "cc: -emit-ssa currently expects exactly one .ir input\n");
      goto out;
    }
  ...
  for (i = 0; i < o.inputs.count; ++i) {
    ...
    if (input_is_S) {
      ...
      if (make_temp_path(&o, "ccspp_", ".s", tmp_s) != 0) {
        fprintf(stderr,
                "cc: failed to create temporary preprocessed assembly file\n");
        goto out;
      }
    ...
    if (input_is_c) {
      ...
      if (o.bootstrap_gcc) {
        ...
        if (cc_compile_c_to_s(
                out_pp, in, out_s, o.std != NULL ? o.std : "gnu17", o.debug,
                o.target, opt_level_num(&o), o.wall, o.werror, o.pedantic,
                o.pedantic_errors, o.gnu89_inline_mode, o.gnu89_inline_override,
                o.i386_isa_level, o.i386_has_sse2, o.i386_has_mmx,
                o.i386_fp_math_mode, o.implicit_funcdecl_override,
                (o.pic || o.shared), &diag) != 0) {
