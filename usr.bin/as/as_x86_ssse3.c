...
static void set_err(ss_ctx_t *ctx, const char *fmt, ...) {
    va_list ap;
...
static int emit8(ss_ctx_t *ctx, uint8_t v) {
...
static int emit32(ss_ctx_t *ctx, uint32_t v) {
...
static int modrm_sib_disp64(ss_ctx_t *ctx, as_x86_reg_t reg_field, const as_x86_operand_t *rm_op,
...
static int emit_ssse3_core(ss_ctx_t *ctx, uint8_t map, uint8_t opcode, const as_x86_ssse3_insn_t *insn) {
    uint8_t rex_r = 0;
    uint8_t rex_x = 0;
    uint8_t rex_b = 0;
    size_t rex_pos;
    ...
    if (insn->has_imm8 && emit8(ctx, insn->imm8) != 0) {
        return -1;
    }
...
int as_x86_encode_ssse3(const as_x86_ssse3_insn_t *insn, uint8_t *out, size_t out_cap,
    ...
    if (insn == NULL || out == NULL || insn->mnemonic == NULL || insn->op_count != 2 ||
        insn->dst.kind != AS_X86_OP_REG) {
        return -1;
    }
    ...
    if (streq_ci(insn->mnemonic, "pabsb")) {
        opcode = 0x1c;
    } else if (streq_ci(insn->mnemonic, "pabsw")) {
        opcode = 0x1d;
    } else if (streq_ci(insn->mnemonic, "pabsd")) {
        opcode = 0x1e;
    } else if (streq_ci(insn->mnemonic, "palignr")) {
        map = 0x3a;
        opcode = 0x0f;
    } else if (streq_ci(insn->mnemonic, "phaddw")) {
        opcode = 0x01;
    } else if (streq_ci(insn->mnemonic, "phaddd")) {
        opcode = 0x02;
    } else if (streq_ci(insn->mnemonic, "phaddsw")) {
        opcode = 0x03;
    } else if (streq_ci(insn->mnemonic, "phsubw")) {
        opcode = 0x05;
    } else if (streq_ci(insn->mnemonic, "phsubd")) {
        opcode = 0x06;
    } else if (streq_ci(insn->mnemonic, "phsubsw")) {
        opcode = 0x07;
    } else if (streq_ci(insn->mnemonic, "pmaddubsw")) {
        opcode = 0x04;
    } else if (streq_ci(insn->mnemonic, "pmulhrsw")) {
        opcode = 0x0b;
    } else if (streq_ci(insn->mnemonic, "pshufb")) {
        opcode = 0x00;
    } else if (streq_ci(insn->mnemonic, "psignb")) {
        opcode = 0x08;
    } else if (streq_ci(insn->mnemonic, "psignw")) {
        opcode = 0x09;
    } else if (streq_ci(insn->mnemonic, "psignd")) {
        opcode = 0x0a;
    } else {
        set_err(&ctx, "unsupported SSSE3 mnemonic: %s", insn->mnemonic);
        return -1;
    }

    if (streq_ci(insn->mnemonic, "palignr") && !insn->has_imm8) {
        return -1;
    }

    if (emit_ssse3_core(&ctx, map, opcode, insn) != 0) {
        return -1;
    }
