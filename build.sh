...
#   cjson -> disasterparty -> motifgpt (motifgpt's configure ends with
#     "disasterparty not found", and disasterparty itself wants libcjson)
#   NetSurf: libparserutils, libwapcaplet, then libhubbub, libcss, libdom
#     -- libdom/build.sh asserts libhubbub is already in the sysroot
...
#   python after sqlite3, expat, libffi, ncurses, openssl, zlib and bzip2 --
#     every one of those is an extension module it links
...
#
DEFAULT_CONTRIB="bzip2 libiconv zlib openssl ncurses gzip tzdata make sed expr libarchive mpg123 curl nginx inetutils zsh e2fsprogs e2tools gmp mpfr gdb cmake xorgproto xcb-proto libXau xtrans libxcb libX11 libXext libICE libSM libXt libXmu libXpm libXaw libXinerama libjpeg lmdb mksh tcl libtirpc xterm xauth luit xrdb libXdmcp pixman libxshmfence libfontenc libXfont libxkbfile xkbcomp xkeyboard-config encodings font-util font-misc-misc font-adobe-75dpi font-adobe-100dpi font-bh-lucida xorg-server libXScrnSaver libXrender xbitmaps motif cde expat libpng freetype fontconfig libXft libffi glib2 libxml2 libxslt file libXfixes libXi libXtst dbus tde pkg-config ca-certificates sqlite3 cjson quickjs mandoc less nano mc perl texinfo python posixtestsuite iceauth libXfont2 libXcursor fribidi libparserutils libwapcaplet libhubbub libcss libdom glib1 gtk1 libogg libvorbis flac speex libopus opusfile faad2 fdk-aac spandsp taglib sox openssh rpcbind disasterparty motifgpt twm ctwm matwm2 xprop xset xsetroot xkill xeyes xcalc xedit xman cairo atk gdk-pixbuf harfbuzz freetype-harfbuzz pango gtk2 hexchat links elinks sdl3 sdl2-compat sdl12-compat psymp3 libXrandr libXv libXcomposite libXdamage libXxf86vm libFS xcb-util appres bdftopcf beforelight bitmap constype editres fonttosfnt fslsfonts fstobdf ico listres mkfontscale oclock rendercheck rgb sessreg setxkbmap showfont smproxy transset viewres x11perf xbacklight xbiff xclipboard xclock xcmsdb xcompmgr xconsole xcursorgen xditview xdpyinfo xev xfd xfontsel xgamma xgc xhost xkbevd xkbprint xkbutils xload xlogo xlsatoms xlsclients xlsfonts xmag xmessage xmodmap xmore xpr xrandr xrefresh xsetmode xsetpointer xsm xstdcmap xvinfo xwd xwininfo xwud"
