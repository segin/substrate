...
- the `vga=WxH@BPP` kernel command line parameter (`vga=1024x768@32`, `vga=640x480`, `vga=320x200@8`, etc.) selects a specific framebuffer resolution across all registered video drivers; the init path enumerates all driver modes and picks the best matching driver/mode combination; when BPP is omitted, the highest available colour depth is preferred
- legacy VGA graphics modes (CGA 320x200x2, Mode 13h 320x200x8, EGA 640x350x4, VGA 640x480x4, Hercules 720x348x1) are fully wirable via `vga=` for ancient hardware support
