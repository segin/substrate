...
- `sys/net/`: Network stack — TCP/IPv4, AF_INET / AF_UNIX sockets, loopback.
  - IPv4 input reassembles fragments (`ip4_reasm_input` in `inet.c`: a bounded 8-datagram table with a 30 s timeout) before protocol dispatch, so a UDP datagram up to 65507 octets can be received; output never fragments.  TCP demultiplexes through a connection hash on (lport, raddr, rport) and a listener hash on lport (`tcp_find`), with the global PCB list kept for the timer and the bind/port scans.
