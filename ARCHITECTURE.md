...
- **Native C Subset Pipeline:** In-tree compile path now exists for a strict subset:
  ...
  - `middle/ast2ir.c` lowers AST to SSA-like instructions (`param`, `const`, `addr`, `load`, `mov`, arithmetic/bitwise/shift, comparisons, typed casts, labels/branches, `call`, `ret`) including switch compare-chain dispatch, short-circuit logical lowering via explicit branch/label control flow, and direct label/goto branch emission.
...
- **Testing Strategy:** `tests/usr.bin/cc/` covers IR verifier regressions plus driver smoke tests for preprocess, native subset codegen (including stack-argument ABI cases, variadic calls, declaration/prototype compatibility, extern call emission, pointer addr/deref + pointer-parameter flow, `if/else`/loop/switch branching, `goto`/label flow, `break`/`continue`/`case`/`default` validation, C95 digraph/trigraph lexical forms, C99 `for`-declaration scope checks, short-circuit logical behavior, bitwise/shift/comma/compound/update operators, ternary/cast/sizeof behavior, floating comparison + floating-condition truthiness behavior, unsigned integer semantics including literal suffix cases, optimization behavior, `-m32` ELF32 object generation including `double`, and stack-slot pressure/frame-size regressions), staged assembly/linking, and bootstrap fallback behavior.
