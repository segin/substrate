...
- **Native C Subset Pipeline:** In-tree compile path now exists for a strict subset:
  ...
  - `middle/ast2ir.c` lowers AST to SSA-like instructions (`param`, `const`, `mov`, arithmetic, comparisons, typed casts, labels/branches, `call`, `ret`) including switch compare-chain dispatch
...
- **Testing Strategy:** `tests/usr.bin/cc/` covers IR verifier regressions plus driver smoke tests for preprocess, native subset codegen (including stack-argument ABI cases, variadic calls, `if/else`/loop/switch branching, `break`/`continue`/`case`/`default` validation, C95 digraph/trigraph lexical forms, optimization behavior, `-m32` ELF32 object generation including `double`, and stack-slot pressure/frame-size regressions), staged assembly/linking, and bootstrap fallback behavior.
