...
- **`as`**: x86 assembler tool for i386/x86_64 with an in-tree standalone frontend under active rollout.
  ...
  - ARMv7/Thumb encoding infrastructure is staged in `usr.bin/as/as_arm_encode.c` (`cond` field/class packing, barrel-shifter operand2 helpers, ARM mode-2 addressing + LDM/STM alias mapping, mnemonic `cond`/`S` splitting, mode directives/interwork helpers, IT block state tracking, and Thumb unified `.w/.n` width handling with narrow/wide add-immediate encoders) with host-side coverage in `tests/usr.bin/as/test_arm_encode_core.sh` and `tests/usr.bin/as/test_arm_thumb_core.sh`.
  - ARMv7 data-processing staging is in `usr.bin/as/as_arm_dataproc.c` (core ARM data-processing immediate/register/shifted forms for arithmetic/logical/test/move opcodes plus a descriptor-driven coverage table for ARM DSP/saturating/bitfield/extend/divide/reverse/select families and Thumb-2 `orn`) with host-side coverage in `tests/usr.bin/as/test_arm_dataproc_core.sh`.
  - ARMv7/Thumb branch staging is in `usr.bin/as/as_arm_branch.c` (A32 `b`/`bl` immediate with range/alignment checks, register interwork `bx`/`blx`, and Thumb `cbz`/`cbnz` + `tbb`/`tbh` encoders) with host-side coverage in `tests/usr.bin/as/test_arm_branch_core.sh`.
  - ARMv7 load/store staging is in `usr.bin/as/as_arm_loadstore.c` (mode-2 generic `ldr/str/ldrb/strb` addressing forms, descriptor coverage for halfword/signed/exclusive/user-mode/preload families, and `ldr` pseudo literal-pool emission) with host-side coverage in `tests/usr.bin/as/test_arm_loadstore_core.sh`.
  - ARMv7 system/coproc staging is in `usr.bin/as/as_arm_system.c` (immediate-capable `svc/swi`, `bkpt`, `hlt`, `dbg` encoders plus descriptor coverage for `mrs/msr`, CPS/SETEND, barriers/hints, coprocessor transfer families, and `clrex`) with host-side coverage in `tests/usr.bin/as/test_arm_system_core.sh`.
  - ARMv7 VFPv3/v4 staging is in `usr.bin/as/as_arm_vfp.c` (descriptor-driven coverage for VFP arithmetic/FMA/convert/compare/load-store families, explicit ARM<->VFP move forms, and ARMv8 FP rounding conversion opcodes) with host-side coverage in `tests/usr.bin/as/test_arm_vfp_core.sh`.
  - ARMv7 NEON staging is in `usr.bin/as/as_arm_neon.c` (descriptor-driven coverage for typed NEON arithmetic/logical/compare/permute/load-store/duplicate/convert/accumulate families, including reciprocal estimate/step and polynomial examples) with host-side coverage in `tests/usr.bin/as/test_arm_neon_core.sh`.
  - ARMv7 relocation staging is in `usr.bin/as/as_arm_reloc.c` (REL relocation kind mapping for branch/MOVW-MOVT/GOT-TLS/PREL31 families and relocation emission helpers) with host-side coverage in `tests/usr.bin/as/test_arm_reloc_core.sh`.
  - AArch64 core encoding infrastructure is in `usr.bin/as/as_a64_encode.c` (condition and register parsing, logical-immediate bitmask packing, MOV{N,Z,K} shift packing, ADR/ADRP immediate splitting, and fixed-width 32-bit little-endian word emission) with host-side coverage in `tests/usr.bin/as/test_a64_encode_core.sh`.
  - AArch64 data-processing immediate staging is in `usr.bin/as/as_a64_dp_imm.c` (descriptor-driven coverage for ADD/SUB immediate, logical-immediate, MOV{N,Z,K}, ADR/ADRP, bitfield families and aliases, and EXTR) with host-side coverage in `tests/usr.bin/as/test_a64_dp_imm_core.sh`.
  - AArch64 data-processing register staging is in `usr.bin/as/as_a64_dp_reg.c` (descriptor-driven coverage for shifted/extended ALU ops, carry arithmetic, multiply families, divide/bit-manipulation, conditional select aliases, and conditional compare) with host-side coverage in `tests/usr.bin/as/test_a64_dp_reg_core.sh`.
  - AArch64 branch/system staging is in `usr.bin/as/as_a64_branch.c` (range-checked encoders for B/BL, B.cond, BR/BLR/RET, CBZ/CBNZ, TBZ/TBNZ, and immediate exception-class instructions `svc/hvc/smc/brk/hlt`) with host-side coverage in `tests/usr.bin/as/test_a64_branch_core.sh`.
  - AArch64 load/store staging is in `usr.bin/as/as_a64_loadstore.c` (descriptor-driven coverage for scalar/pair/non-temporal/literal addressing forms, exclusive + acquire/release families, and `prfm`) with host-side coverage in `tests/usr.bin/as/test_a64_loadstore_core.sh`.
  - AArch64 system-instruction staging is in `usr.bin/as/as_a64_system.c` (descriptor-driven coverage for `mrs/msr`, hints/barriers/`clrex`, `sys/sysl`, and maintenance aliases `dc/ic/at/tlbi`) with host-side coverage in `tests/usr.bin/as/test_a64_system_core.sh`.
  - AArch64 SIMD/FP staging is in `usr.bin/as/as_a64_simd.c` (descriptor-driven coverage for Advanced SIMD arithmetic/shift/logical/compare/permute/load-store/FP conversion families plus optional crypto and CRC32 instruction sets) with host-side coverage in `tests/usr.bin/as/test_a64_simd_core.sh`.
  - AArch64 ARMv8.1 staging is in `usr.bin/as/as_a64_v81.c` (descriptor-driven coverage for LSE atomics, store-only atomic aliases, RDMA `sqrdmlah/sqrdmlsh`, LOR `ldlar/stllr`, VHE-related EL2 system-register access examples, PAN-related unprivileged `ldtr/sttr` forms, and HPD-relevant `tcr_el1` register accesses) with host-side coverage in `tests/usr.bin/as/test_a64_v81_core.sh`.
  - AArch64 relocation staging is in `usr.bin/as/as_a64_reloc.c` (RELA relocation kind mapping for absolute/PC-relative addressing, branch/test relocation families, MOVW split-immediate relocations, GOT page/load relocations, and AArch64 TLS GD/LD/IE/LE relocation families) with host-side coverage in `tests/usr.bin/as/test_a64_reloc_core.sh`.
  - Legacy i386 + x86-64 extension encoding is staged in `usr.bin/as/as_x86_encode.c` (ModR/M+SIB/displacement/prefixes, REX/RIP-relative forms, high-register support, and special opcodes including `syscall/sysret/swapgs/cmpxchg16b`) with host byte-exact regression coverage in `tests/usr.bin/as/test_x86_legacy_encode.sh` and `tests/usr.bin/as/test_x86_64_encode_ext.sh`.
  - VEX (AVX-class) prefix encoding is staged in `usr.bin/as/as_x86_vex.c` with automatic `C5`/`C4` selection and explicit `L/W/pp/vvvv` field handling for 3-operand forms; regression coverage lives in `tests/usr.bin/as/test_x86_vex_encode.sh`.
  ...
  - SSSE3 opcode support (`pabs*`, `palignr`, `phadd*`, `phsub*`, `pmaddubsw`, `pmulhrsw`, `pshufb`, `psign*`) is staged in `usr.bin/as/as_x86_ssse3.c` with byte-exact coverage in `tests/usr.bin/as/test_x86_ssse3_core.sh`.
  - SSE4.1 opcode support (`blend*`, `dpp*`, `extract/insert`, `movntdqa`, `mpsadbw`, `packusdw`, `pblend*`, `pcmpeqq`, `pextr*`, `pinsr*`, `pmax*`, `pmin*`, `pmovsx*`, `pmovzx*`, `pmuldq`, `pmulld`, `ptest`, `round*`, `phminposuw`) is staged in `usr.bin/as/as_x86_sse41.c` with byte-exact coverage in `tests/usr.bin/as/test_x86_sse41_core.sh`.
  - SSE4.2 opcode support (`crc32`, `pcmpestri/m`, `pcmpistri/m`, `pcmpgtq`) is staged in `usr.bin/as/as_x86_sse42.c` with byte-exact coverage in `tests/usr.bin/as/test_x86_sse42_core.sh`.
  - AVX core support (VEX-promoted vector ALU forms, `vbroadcast*`, `vextractf128`/`vinsertf128`, `vmaskmov*`, `vperm2f128`, `vpermil*`, `vtest*`, `vzeroall`/`vzeroupper`) is staged in `usr.bin/as/as_x86_avx.c` with byte-exact coverage in `tests/usr.bin/as/test_x86_avx_core.sh`.
  ...
  - AVX-512BW staged support (byte/word arithmetic+permute/compare classes, variable shifts, `vdbpsadbw`, mask conversion moves, and blend/test families) is staged in `usr.bin/as/as_x86_avx512bw.c` with byte-exact coverage in `tests/usr.bin/as/test_x86_avx512bw_core.sh`.
  - AVX-512CD staged support (`vpconflict*`, `vplzcnt*`, and mask-broadcast helpers) is staged in `usr.bin/as/as_x86_avx512cd.c` with byte-exact coverage in `tests/usr.bin/as/test_x86_avx512cd_core.sh`.
  - AVX-512DQ staged support (QQ conversion families including truncating forms, `vpmullq`, mask/vector move conversions, insert/extract `i64x2`/`i32x8`, `vrange*`/`vreduce*`/`vfpclass*`, EVEX bitwise FP ops, and `vbroadcast*f/i32x{2,8}`) is staged in `usr.bin/as/as_x86_avx512dq.c` with byte-exact coverage in `tests/usr.bin/as/test_x86_avx512dq_core.sh`.
  - AVX-512VL staged support is wired across AVX-512F/BW/CD/DQ encoders via vector-length-aware EVEX L'L selection (`xmm`/`ymm`/`zmm`) while preserving opmask+zeroing+broadcast control bits, with coverage in `tests/usr.bin/as/test_x86_avx512vl_core.sh`.
  ...
  - Regression coverage lives in `tests/usr.bin/as/` including compatibility, relocation/ELF, diagnostics, determinism, fuzz-smoke, rollout integration harnesses, the `tests/usr.bin/as/test_unit_matrix.sh` per-arch unit matrix runner, `tests/usr.bin/as/test_integration_matrix.sh` for cross-arch/round-trip/toolchain-interoperability checks, `tests/usr.bin/as/test_fuzz_matrix.sh` for grammar-aware/parser-fuzz + byte-level ELF mutation validation, and `tests/usr.bin/as/test_build_matrix.sh` for recursive `NATIVE_BUILD=1` + install/alias + `libelfobj` dependency checks.
...
- **Read/Memory Model:** Supports copy-based parse (`elf_open_memory`), zero-copy parse views (`elf_open_memory_nocopy`), and option-driven open paths (`elf_open_with_options` / `elf_open_memory_with_options`) for `mmap` and lazy symbol/relocation materialization.
...
- **Testing/Fuzzing Strategy:** Unit/round-trip/link tests in `usr.lib/elfobj/tests/` (including debug/unwind, validation hardening, perf I/O, and class/endian/type matrix coverage), parser+relocation fuzz entrypoints in `usr.lib/elfobj/fuzz/` with executable smoke drivers, toolchain integration scripts (`test_tool_integration.sh`, `test_build_install.sh`), and micro-benchmark/perf-gate coverage in `usr.lib/elfobj/bench/` and `usr.lib/elfobj/tests/test_perf_gate.sh`.
