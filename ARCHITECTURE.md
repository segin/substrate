...
- **Native C Subset Pipeline:** In-tree compile path now exists for a strict subset:
  - `frontend/lexer.c` + `frontend/parser.c` parse scalar `_Bool`/`char`/`unsigned char`/`short`/`unsigned short`/`int`/`unsigned int`/`long long`/`unsigned long long`/`float`/`double`/`void` functions with declarations, assignments, calls, and returns; one-level typed pointers (`T*`) are supported in declarations/params/returns with unary `&`/`*` expressions, including `void*` object/parameter declarators (while preserving `void`-only empty parameter list behavior). Lexer accepts decimal/octal/hex integer literals with unsigned/long-long suffix typing and character literals with common escapes
  ...
  - phase-9 adds operator lowering for `%` plus bitwise/shift/comma, compound/update operators (including correct prefix/postfix `++/--` expression-value behavior), short-circuit logical expressions, ternary/cast/sizeof support, numeric comparison expansion to floating operands, floating-condition truthiness (`!= 0.0`) handling, label/goto flow without introducing full phi-form SSA yet, unsigned scalar compare/div/mod/right-shift semantics in SSA/backend emission, literal suffix typing (`u`/`ul`/`ull`) feeding those semantics, explicit `short`/`unsigned short` scalar-size metadata (`sizeof == 2`), and one-level pointer addr/load/store lowering for `&`, `*`, and `*p = expr`; scaled pointer arithmetic (`ptr +/- int`, `int + ptr`), identifier pointer `++/--` stepping, ordered comparison of compatible pointers, dereference compound assignments (`*lvalue op= rhs`), and prefix dereference updates (`++*p`/`--*p`) are supported for non-`void*` pointers.
