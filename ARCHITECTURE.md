...
- **`as`**: x86 assembler tool for i386/x86_64 with an in-tree standalone frontend under active rollout.
  ...
  - Legacy i386 encoding engine is staged in `usr.bin/as/as_x86_encode.c` (ModR/M+SIB/displacement/prefixes plus baseline opcode groups) with host byte-exact regression coverage in `tests/usr.bin/as/test_x86_legacy_encode.sh`.
