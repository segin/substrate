...
int main(int argc, char **argv)
{
    ...
    if (opts.show_help) {
        ...
        puts("      --no-preserve=LIST       do not preserve listed metadata");
        ...
        puts("  -b[SIZE], --buffer-size=SIZE set copy buffer size or enable backup");
        puts("      --backup[=CONTROL]       backup existing destination");
        puts("  -S, --suffix=SUFFIX          backup suffix (default ~)");
        puts("  -t, --target-directory=DIR   copy all sources into DIR");
        puts("  -T, --no-target-directory    treat destination as a normal file");
        puts("      --sparse=MODE            sparse strategy: auto|always|never");
        puts("      --reflink=WHEN           clone policy: auto|always|never");
        puts("      --remove-destination     remove destination before opening");
        puts("      --atomic-replace         write temp file + rename (default)");
        puts("      --no-atomic-replace      disable atomic replacement");
        puts("  -u, --update                 copy only when source is newer");
