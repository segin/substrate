...
static void init_empty_directory_block_at(uint32_t block, uint32_t self_ino, uint32_t parent_ino)
{
    uint8_t *blk = mock_disk + block * 1024;
    uint16_t dot_len = (uint16_t)(((8 + 1 + 3) / 4) * 4);
    ext2_dirent_t *dot;
    ext2_dirent_t *dotdot;

    memset(blk, 0, 1024);
    dot = (ext2_dirent_t *)blk;
    dotdot = (ext2_dirent_t *)(blk + dot_len);
...
static void setup_fs(ext2_fs_t *fs, fs_node_t *dev_node, ext2_node_t *root_ctx, fs_node_t *root)
{
    ...
    init_empty_directory_block_at(ROOT_BLOCK, ROOT_INO, ROOT_INO);
...
static void teardown_fs(ext2_fs_t *fs)
{
...
/* REQ-04-0191: vop_rename same-dir */
static bool test_rename_same_dir(void)
{
    ext2_fs_t fs;
    fs_node_t dev_node;
    ...
    /* create "old" */
    if (ext2_mknod(&root, "old", S_IFREG | 0644, 0) != 0)
        return false;

    if (!ext2_finddir(&root, "old"))
        return false;

    /* rename old -> new in same directory */
    if (ext2_rename(&root, "old", &root, "new") != 0)
        return false;

    /* "old" must be gone */
    if (ext2_finddir(&root, "old") != NULL)
        return false;

    /* "new" must exist */
    if (!ext2_finddir(&root, "new"))
        return false;
...
/* REQ-04-0191: vop_rename cross-dir (file moved into subdir) */
static bool test_rename_cross_dir(void)
{
    ext2_fs_t fs;
    fs_node_t dev_node;
    ext2_node_t root_ctx;
    ...
    /* create subdir "sub" in root */
    if (ext2_mkdir(&root, "sub", 0755) != 0)
        return false;

    fs_node_t *subdir = ext2_finddir(&root, "sub");
    if (!subdir || (subdir->flags & 0x7) != FS_DIRECTORY)
        return false;

    /* subdir needs vop pointers */
    subdir->finddir = ext2_finddir;
    subdir->mkdir = ext2_mkdir;
    subdir->unlink = ext2_unlink;
    subdir->rmdir = ext2_rmdir;

    /* create file "file" in root */
    if (ext2_mknod(&root, "file", S_IFREG | 0644, 0) != 0)
        return false;

    if (!ext2_finddir(&root, "file"))
        return false;

    /* rename root/file -> sub/file */
    if (ext2_rename(&root, "file", subdir, "file") != 0)
        return false;
    ...
    /* "file" must exist in subdir */
    if (!ext2_finddir(subdir, "file"))
        return false;
...
int main(void)
{
    ...
    if (!test_rename_same_dir()) {
        printf("FAIL: ext2 rename same-dir\n");
        ok = false;
    } else {
        printf("PASS: ext2 rename same-dir\n");
    }

    if (!test_rename_cross_dir()) {
        printf("FAIL: ext2 rename cross-dir\n");
        ok = false;
    } else {
        printf("PASS: ext2 rename cross-dir\n");
    }
