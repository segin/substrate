...
#define TEST_ASSERT(cond) \
    do { \
        if (!(cond)) { \
            ...
            panic("test_udf assertion failed"); \
        } \
    } while (0)
...
static void test_structure_sizes(void) {
    ...
    TEST_ASSERT(sizeof(struct udf_tag) == 16);
    
    /* Extent AD must be 8 bytes */
    TEST_ASSERT(sizeof(struct udf_extent_ad) == 8);
    
    /* Short AD must be 8 bytes */
    TEST_ASSERT(sizeof(struct udf_short_ad) == 8);
    
    /* Long AD must be 16 bytes */
    TEST_ASSERT(sizeof(struct udf_long_ad) == 16);
    
    /* Timestamp must be 12 bytes */
    TEST_ASSERT(sizeof(struct udf_timestamp) == 12);
    
    /* RegID must be 32 bytes */
    TEST_ASSERT(sizeof(struct udf_regid) == 32);
    
    /* Charspec must be 64 bytes */
    TEST_ASSERT(sizeof(struct udf_charspec) == 64);

    kprintf("test_structure_sizes: PASSED\n");
}
...
static void test_tag_checksum(void) {
    ...
    TEST_ASSERT(tag.tag_checksum != 0 || (tag.tag_id == 0 && tag.desc_version == 0));
    
    kprintf("test_tag_checksum: PASSED\n");
}

/* Test FID characteristics flags */
static void test_fid_flags(void) {
    TEST_ASSERT(UDF_FID_HIDDEN == 0x01);
    TEST_ASSERT(UDF_FID_DIRECTORY == 0x02);
    TEST_ASSERT(UDF_FID_DELETED == 0x04);
    TEST_ASSERT(UDF_FID_PARENT == 0x08);
    
    kprintf("test_fid_flags: PASSED\n");
}

/* Test ICB allocation descriptor types */
static void test_icb_ad_types(void) {
    TEST_ASSERT(UDF_ICB_FLAG_AD_SHORT == 0);
    TEST_ASSERT(UDF_ICB_FLAG_AD_LONG == 1);
    TEST_ASSERT(UDF_ICB_FLAG_AD_EXT == 2);
    TEST_ASSERT(UDF_ICB_FLAG_AD_INLINE == 3);
    
    kprintf("test_icb_ad_types: PASSED\n");
}

/* Test file types */
static void test_file_types(void) {
    TEST_ASSERT(UDF_FILETYPE_DIR == 4);
    TEST_ASSERT(UDF_FILETYPE_FILE == 5);
    TEST_ASSERT(UDF_FILETYPE_SYMLINK == 12);
    
    kprintf("test_file_types: PASSED\n");
}

/* Test tag IDs */
static void test_tag_ids(void) {
    TEST_ASSERT(UDF_TAG_PRIMARY_VD == 1);
    TEST_ASSERT(UDF_TAG_ANCHOR_VDP == 2);
    TEST_ASSERT(UDF_TAG_PARTITION_D == 5);
    TEST_ASSERT(UDF_TAG_LOGICAL_VD == 6);
    TEST_ASSERT(UDF_TAG_FSD == 256);
    TEST_ASSERT(UDF_TAG_FID == 257);
    TEST_ASSERT(UDF_TAG_FE == 261);
    TEST_ASSERT(UDF_TAG_EFE == 266);
    
    kprintf("test_tag_ids: PASSED\n");
}
...
static void test_property_alloc_desc(void) {
    ...
    for (unsigned int n = 1; n <= 100; n++) {
        TEST_ASSERT(n * sizeof(struct udf_short_ad) == n * 8);
    }
    
    kprintf("test_property_alloc_desc: PASSED\n");
}
...
static void test_property_fid_size(void) {
    ...
    for (int impl_len = 0; impl_len < 16; impl_len++) {
        for (int name_len = 1; name_len < 64; name_len++) {
            ...
            TEST_ASSERT(fid_size % 4 == 0);
            TEST_ASSERT(fid_size >= 38);
        }
    }
    
    kprintf("test_property_fid_size: PASSED\n");
}
...
static void write_sector(uint32_t sector, void *data, uint32_t size) {
    TEST_ASSERT(size <= UDF_SECTOR_SIZE);
...
static void test_vds_success(void) {
    ...
    TEST_ASSERT(ret == 0);
    TEST_ASSERT(pvd_out.tag.tag_id == UDF_TAG_PRIMARY_VD);
    TEST_ASSERT(pd_out.tag.tag_id == UDF_TAG_PARTITION_D);
    TEST_ASSERT(lvd_out.tag.tag_id == UDF_TAG_LOGICAL_VD);

    teardown_mock_disk();
    kprintf("test_vds_success: PASSED\n");
}
...
static void test_vds_incomplete(void) {
    ...
    teardown_mock_disk();
    kprintf("test_vds_incomplete: PASSED\n");
}
...
static void test_vds_terminating(void) {
    ...
    teardown_mock_disk();
    kprintf("test_vds_terminating: PASSED\n");
}
...
static void test_vds_invalid_tag(void) {
    ...
    int ret = udf_read_vds(&dev, &vds_extent, &pvd_out, &pd_out, &lvd_out);
    TEST_ASSERT(ret == 0); // Should skip 51 and find PD at 52
    TEST_ASSERT(pvd_out.tag.tag_id == UDF_TAG_PRIMARY_VD);
    TEST_ASSERT(pd_out.tag.tag_id == UDF_TAG_PARTITION_D);
    TEST_ASSERT(pd_out.tag.tag_location == 52);
    TEST_ASSERT(lvd_out.tag.tag_id == UDF_TAG_LOGICAL_VD);

    teardown_mock_disk();
    kprintf("test_vds_invalid_tag: PASSED\n");
}
...
static void test_vds_crc_mismatch(void) {
    ...
    teardown_mock_disk();
    kprintf("test_vds_crc_mismatch: PASSED\n");
}
...
static void test_vds_zero_length(void) {
    ...
    teardown_mock_disk();
    kprintf("test_vds_zero_length: PASSED\n");
}
...
static void test_vds_duplicate_tags(void) {
    ...
    teardown_mock_disk();
    kprintf("test_vds_duplicate_tags: PASSED\n");
}
...
static void test_vds_early_termination(void) {
    ...
    teardown_mock_disk();
    kprintf("test_vds_early_termination: PASSED\n");
}
...
static void test_vds_extent_out_of_bounds(void) {
    ...
    teardown_mock_disk();
    kprintf("test_vds_extent_out_of_bounds: PASSED\n");
}
...
static void test_vds_read_error(void) {
    ...
    mock_fail_sector = -1;
    teardown_mock_disk();
    kprintf("test_vds_read_error: PASSED\n");
}


void run_udf_tests(void) {
    kprintf("=== UDF Unit Tests ===\n");
