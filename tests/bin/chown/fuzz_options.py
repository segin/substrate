...
SEED = int(os.environ.get("CHOWN_FUZZ_SEED", "1717"))
ITERATIONS = int(os.environ.get("CHOWN_FUZZ_ITERS", "120"))
...
def main() -> int:
    if len(sys.argv) != 2:
        print("usage: fuzz_options.py /path/to/chown_host", file=sys.stderr)
        return 1

    rng = random.Random(SEED)
    binary = os.path.abspath(sys.argv[1])
    tmp = tempfile.mkdtemp(prefix="chown-fuzz-opt-")
    try:
        for _ in range(ITERATIONS):
            ...
            cp = subprocess.run([binary, *args], cwd=tmp,
                                ...
                                timeout=3)
            if cp.returncode < 0:
                raise AssertionError(f"chown_host crashed with args={args!r}")
    finally:
        shutil.rmtree(tmp)
