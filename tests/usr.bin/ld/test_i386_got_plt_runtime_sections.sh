...
SRC
gcc -m32 -fPIC -c -o "$TMP/main.o" "$TMP/main.c"

"$LDX" -m32 -shared -o "$TMP/out.so" "$TMP/main.o"

"$READELF" -S "$TMP/out.so" > "$TMP/sections.txt"
"$READELF" -d "$TMP/out.so" > "$TMP/dynamic.txt"
"$READELF" -r "$TMP/out.so" > "$TMP/relocs.txt"

for sec in ".plt" ".got.plt" ".rel.plt" ".got" ".rel.dyn"; do
...
for tag in "(PLTGOT)" "(PLTREL)" "(PLTRELSZ)" "(JMPREL)" "(REL)" "(RELSZ)" "(RELENT)"; do
...
if ! grep -q "R_386_JUMP_SLO" "$TMP/relocs.txt"; then
	echo "FAIL: expected JUMP_SLOT relocations in .rel.plt" >&2
	cat "$TMP/relocs.txt" >&2
	exit 1
fi
if ! grep -q "R_386_GLOB_DAT" "$TMP/relocs.txt"; then
	echo "FAIL: expected GLOB_DAT relocations in .rel.dyn" >&2
	cat "$TMP/relocs.txt" >&2
	exit 1
fi

echo "ok: i386 dynamic links synthesize GOT/PLT and runtime relocation tables"
