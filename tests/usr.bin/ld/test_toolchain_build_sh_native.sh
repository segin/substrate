...
make -C "$ROOT/usr.bin/cc" clean >/dev/null
...
if ! make -C "$ROOT/bin/sh" NATIVE_BUILD=1 \
	CC="$CCX" AS="$ASX" LD="$LDX" CFLAGS='-O2 -Wall -Wextra -Werror -std=gnu99' \
	-j"$(getconf _NPROCESSORS_ONLN 2>/dev/null || echo 1)" >"$TMP/build.log" 2>&1; then
	echo "FAIL: bin/sh build failed" >&2
	sed -n '1,220p' "$TMP/build.log" >&2
	exit 1
fi
