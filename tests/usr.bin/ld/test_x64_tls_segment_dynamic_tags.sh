...
TMP=${TMPDIR:-/tmp}/ldx86-tls-seg64-$$
mkdir -p "$TMP"
...
gcc -m64 -O2 -fPIC -c -o "$TMP/tls_mix.o" "$TMP/tls_mix.c"
"$LDX" -m64 -shared -o "$TMP/tls_mix.so" "$TMP/tls_mix.o"
...
for tag in "(RELA)" "(RELASZ)" "(RELAENT)"; do
...
for t in "R_X86_64_DTPMOD64" "R_X86_64_DTPOFF64"; do
...
echo "ok: x86-64 TLS output includes PT_TLS and TLS runtime dynamic tags"
