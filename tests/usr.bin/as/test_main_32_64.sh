...
# Relocatable link smoke tests through ld.
if "$LD" -r -o "$TMP/main64.r.o" "$TMP/main64.o" && \
   "$LD" -r -o "$TMP/main32.r.o" "$TMP/main32.o"; then
    echo "ok: linked relocatable outputs via ld"
else
    echo "warn: ld validation rejected compiler-style relocations (known issue)"
fi
