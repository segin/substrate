...
static void run_case(const simd_case_t *tc) {
    as_a64_simd_insn_t in;
    uint8_t out[8];
    size_t out_len = 0;
    char err[160];
    ...
    if (as_a64_encode_simd(&in, out, sizeof(out), &out_len, err, sizeof(err)) != 0) {
        fprintf(stderr, "%s encode error: %s\\n", tc->mnemonic, err);
        fail("encode failed");
    }

    if (out_len != 4 || read32le(out) != tc->word) {
        fprintf(stderr, "%s mismatch: len=%zu word=0x%08x exp=0x%08x\\n",
                tc->mnemonic, out_len, read32le(out), tc->word);
        fail("word mismatch");
    }
}

static const simd_case_t k_cases[] = {
