#!/bin/sh
#
# contrib/bdftopcf/fetch.sh — fetch the bdftopcf tarball, verify, extract,
# apply the substrate patch series (if any).

set -eu

VERSION="1.1"
TARBALL="bdftopcf-${VERSION}.tar.gz"
...
TREE_DIR="${BUILD_DIR}/bdftopcf-${VERSION}"
...
echo "==> bdftopcf ${VERSION} ready at ${TREE_DIR}"
