...
xev 1.2.7, cross-built for `i386-unknown-substrate`.

Upstream tarball: `https://www.x.org/archive/individual/app/xev-1.2.7.tar.gz`
...
./build.sh      # cross-configure, make, stage into dist-overlay/dist-xev
```

`build.sh` stages into `${SUBSTRATE_TOP}/dist-overlay/dist-xev/usr`, which
