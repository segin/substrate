#!/bin/sh
#
# contrib/libXaw/build.sh — cross-build libXaw for substrate.
...
#   DESTDIR         staging dir (default ${SUBSTRATE_TOP}/dist-overlay/dist-libXaw)
...
# -include sys/select.h: src/TextAction.c uses select()/fd_set without
# including <sys/select.h>; substrate keeps the POSIX header layout
# strict, so fd_set does not leak in via <sys/types.h>.
export CPPFLAGS="${CPP} -include sys/select.h"
