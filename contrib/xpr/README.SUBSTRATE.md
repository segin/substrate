...
xpr 1.2.1, cross-built for `i386-unknown-substrate`.

Upstream tarball: `https://www.x.org/archive/individual/app/xpr-1.2.1.tar.gz`
...
./build.sh      # cross-configure, make, stage into dist-overlay/dist-xpr
```

`build.sh` stages into `${SUBSTRATE_TOP}/dist-overlay/dist-xpr/usr`, which
