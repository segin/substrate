#!/bin/sh
# contrib/tde/tdeutils/build.sh — cross-build a curated subset of tdeutils
...
VER="14.1.6"; TREE="${HERE}/build/tdeutils-trinity-${VER}"
DEST="${SUBSTRATE_TOP}/dist-overlay/dist-tdeutils"
...
cmake -G "Unix Makefiles" \
    ...
    -DBUILD_KCALC=OFF -DBUILD_KCHARSELECT=ON -DBUILD_KHEXEDIT=ON \
    -DBUILD_KJOTS=ON -DBUILD_KTIMER=ON -DBUILD_KDF=ON \
    -DWITH_ARTS=OFF -DWITH_SENSORS=OFF -DWITH_SNMP=OFF -DWITH_XMMS=OFF \
    -DWITH_DPMS=OFF -DWITH_XSCREENSAVER=OFF -DWITH_TDENEWSTUFF=OFF \
...
echo "==> tdeutils staged under ${DEST}/opt/trinity and merged into ${MERGED}"
