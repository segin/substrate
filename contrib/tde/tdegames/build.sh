#!/bin/sh
# contrib/tde/tdegames/build.sh — cross-build a curated subset of tdegames
...
VER="14.1.6"; TREE="${HERE}/build/tdegames-trinity-${VER}"
DEST="${SUBSTRATE_TOP}/dist-overlay/dist-tdegames"
...
cmake -G "Unix Makefiles" \
    ...
    -DBUILD_KMINES=ON -DBUILD_KPAT=ON -DBUILD_KREVERSI=ON -DBUILD_KSAME=ON \
    -DBUILD_KATOMIC=ON -DBUILD_KJUMPINGCUBE=ON -DBUILD_KLINES=ON \
    -DBUILD_KBLACKBOX=ON -DBUILD_KNETWALK=ON -DBUILD_KBOUNCE=ON \
    -DWITH_ARTS=OFF \
...
echo "==> tdegames staged under ${DEST}/opt/trinity and merged into ${MERGED}"
