#!/bin/sh
#
# contrib/xkill/build.sh — cross-build xkill for substrate.
# xkill kills an X client by its window — you pick the window with the mouse.  Produces /usr/bin/xkill.
#
# Depends on the staged X client stack: xorgproto xcb-proto libXau xtrans libxcb libX11 libXext libXmu.
#
# Env:
#   STAGE1_PREFIX   substrate toolchain prefix (default /opt/substrate)
#   DESTDIR         staging dir (default ${SUBSTRATE_TOP}/dist-overlay/dist-xkill)
...
VERSION="1.0.7"
TREE_DIR="${HERE}/build/xkill-${VERSION}"
...
: "${DESTDIR:=${SUBSTRATE_TOP}/dist-overlay/dist-xkill}"
...
PKGP=""; CPP=""; LDF=""
for dep in xorgproto xcb-proto libXau xtrans libxcb libX11 libXext libXmu; do
...
echo "==> Done.  xkill staged under ${DESTDIR}"
