...
xsm 1.0.6, cross-built for `i386-unknown-substrate`.

Upstream tarball: `https://www.x.org/archive/individual/app/xsm-1.0.6.tar.gz`
...
./build.sh      # cross-configure, make, stage into dist-overlay/dist-xsm
```

`build.sh` stages into `${SUBSTRATE_TOP}/dist-overlay/dist-xsm/usr`, which
