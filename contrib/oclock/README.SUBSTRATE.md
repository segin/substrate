...
oclock 1.0.6, cross-built for `i386-unknown-substrate`.

Upstream tarball: `https://www.x.org/archive/individual/app/oclock-1.0.6.tar.gz`
...
./build.sh      # cross-configure, make, stage into dist-overlay/dist-oclock
```

`build.sh` stages into `${SUBSTRATE_TOP}/dist-overlay/dist-oclock/usr`, which
