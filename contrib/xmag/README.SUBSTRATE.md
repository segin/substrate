...
xmag 1.0.8, cross-built for `i386-unknown-substrate`.

Upstream tarball: `https://www.x.org/archive/individual/app/xmag-1.0.8.tar.gz`
...
./build.sh      # cross-configure, make, stage into dist-overlay/dist-xmag
```

`build.sh` stages into `${SUBSTRATE_TOP}/dist-overlay/dist-xmag/usr`, which
