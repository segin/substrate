...
xkbevd 1.1.6, cross-built for `i386-unknown-substrate`.

Upstream tarball: `https://www.x.org/archive/individual/app/xkbevd-1.1.6.tar.gz`
...
./build.sh      # cross-configure, make, stage into dist-overlay/dist-xkbevd
```

`build.sh` stages into `${SUBSTRATE_TOP}/dist-overlay/dist-xkbevd/usr`, which
