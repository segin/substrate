# x11perf on Substrate

x11perf 1.6.1, cross-built for `i386-unknown-substrate`.

Upstream tarball: `https://www.x.org/archive/individual/app/x11perf-1.6.1.tar.gz`
...
./build.sh      # cross-configure, make, stage into dist-overlay/dist-x11perf
```

`build.sh` stages into `${SUBSTRATE_TOP}/dist-overlay/dist-x11perf/usr`, which
