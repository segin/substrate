...
: "${STAGE1_PREFIX:=/opt/substrate}"; : "${DESTDIR:=${SUBSTRATE_TOP}/dist-overlay/dist-fontconfig}"
: "${JOBS:=$(nproc 2>/dev/null || echo 4)}"
PATH="${STAGE1_PREFIX}/bin:${PATH}"; export PATH
. "${HERE}/../substrate-autotools.sh"
[ -d "${TREE_DIR}" ] || { echo "build.sh: run ./fetch.sh first" >&2; exit 1; }
FT="${SUBSTRATE_TOP}/dist-overlay/dist-freetype"; XP="${SUBSTRATE_TOP}/dist-overlay/dist-expat"; ZL="${SUBSTRATE_TOP}/dist-overlay/dist-zlib"; PNG="${SUBSTRATE_TOP}/dist-overlay/dist-libpng"
