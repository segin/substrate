#!/bin/sh
#
# contrib/libdom/build.sh — cross-compile the NetSurf HTML5
# parser for substrate.  Static libdom.a + headers + .pc.
#
...
[ -f "${SYSROOT}/lib/pkgconfig/libhubbub.pc" ] || {
    echo "build.sh: libhubbub not staged in the sysroot" >&2; exit 1; }
...
# WITH_EXPAT_BINDING=no: substrate has no expat, and elinks drives
# libdom through the hubbub (HTML) binding, not the XML one.
