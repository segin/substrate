...
xload 1.2.2, cross-built for `i386-unknown-substrate`.

Upstream tarball: `https://www.x.org/archive/individual/app/xload-1.2.2.tar.gz`
...
./build.sh      # cross-configure, make, stage into dist-overlay/dist-xload
```

`build.sh` stages into `${SUBSTRATE_TOP}/dist-overlay/dist-xload/usr`, which
