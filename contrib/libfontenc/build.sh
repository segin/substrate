#!/bin/sh
# contrib/libfontenc/build.sh — cross-build libfontenc for substrate.
# Produces /usr/lib/libfontenc.{a,so.1} + headers + pkgconfig.
#
# libfontenc is a shared-memory fence primitive used by X clients
...
VERSION="1.1.8"
TREE_DIR="${HERE}/build/libfontenc-${VERSION}"
...
: "${DESTDIR:=${SUBSTRATE_TOP}/dist-overlay/dist-libfontenc}"
...
echo "==> Done.  libfontenc staged under ${DESTDIR}"
