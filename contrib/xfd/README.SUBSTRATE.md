...
xfd 1.1.6, cross-built for `i386-unknown-substrate`.

Upstream tarball: `https://www.x.org/archive/individual/app/xfd-1.1.6.tar.gz`
...
./build.sh      # cross-configure, make, stage into dist-overlay/dist-xfd
```

`build.sh` stages into `${SUBSTRATE_TOP}/dist-overlay/dist-xfd/usr`, which
