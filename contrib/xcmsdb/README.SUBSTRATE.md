...
xcmsdb 1.0.7, cross-built for `i386-unknown-substrate`.

Upstream tarball: `https://www.x.org/archive/individual/app/xcmsdb-1.0.7.tar.gz`
...
./build.sh      # cross-configure, make, stage into dist-overlay/dist-xcmsdb
```

`build.sh` stages into `${SUBSTRATE_TOP}/dist-overlay/dist-xcmsdb/usr`, which
