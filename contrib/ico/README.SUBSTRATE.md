...
ico 1.0.7, cross-built for `i386-unknown-substrate`.

Upstream tarball: `https://www.x.org/archive/individual/app/ico-1.0.7.tar.gz`
...
./build.sh      # cross-configure, make, stage into dist-overlay/dist-ico
```

`build.sh` stages into `${SUBSTRATE_TOP}/dist-overlay/dist-ico/usr`, which
