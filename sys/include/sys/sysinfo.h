...
/* Per-thread snapshot returned by sys_proc_thr_list.  The kernel writes
 * this from kernel thread_t; userspace gets a stable wire shape so
 * libpthread / procfs / ps don't pull in kernel internals.  Layout
 * is fixed (changing fields rev's the ABI). */
