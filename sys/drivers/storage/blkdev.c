...
static uint32_t blkdev_vfs_read(fs_node_t *node, uint32_t offset, uint32_t size, uint8_t *buffer) {
...
// VFS write wrapper
static uint32_t blkdev_vfs_write(fs_node_t *node, uint32_t offset, uint32_t size, uint8_t *buffer) {
    blkdev_t *dev = (blkdev_t *)node->impl;
