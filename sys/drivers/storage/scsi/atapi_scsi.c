...
static int atapi_reset_device(scsi_link_t *link, scsi_device_t *dev) {
     ...
     *
     * Use a real wall-clock delay rather than a `for(volatile;;)` spin
     * — the compiler is allowed to elide volatile loop counters when
     * nothing else uses them, and even when it doesn't the wall time
