...
static int minix_write_inode_raw(minix_fs_t *fs, uint32_t inode_num, void *inode);
...
static void minix_free_all_zones(minix_fs_t *fs, struct minix_inode_v1 *inode) {
    for (int i = 0; i < 7; i++) {
        if (inode->i_zone[i]) {
...
static int minix_mknod(fs_node_t *dir, const char *name, uint16_t mode, uint32_t dev) {
    ...
    struct minix_inode_v2 inode_v2;

    // TODO: Get real time
    uint32_t current_time = 0;

    if (v2) {
        memset(&inode_v2, 0, sizeof(inode_v2));
        inode_v2.i_mode = mode;
        inode_v2.i_uid = dir->uid;
        inode_v2.i_gid = dir->gid;
        inode_v2.i_nlinks = 1;
        inode_v2.i_atime = current_time;
        inode_v2.i_mtime = current_time;
        inode_v2.i_ctime = current_time;
        inode_v2.i_zone[0] = dev;
        inode_ptr = &inode_v2;
    } else {
        memset(&inode_v1, 0, sizeof(inode_v1));
        inode_v1.i_mode = mode;
        inode_v1.i_uid = (uint16_t)dir->uid;
        inode_v1.i_gid = (uint8_t)dir->gid;
        inode_v1.i_nlinks = 1;
        inode_v1.i_time = current_time;
        inode_v1.i_zone[0] = (uint16_t)dev;
        inode_ptr = &inode_v1;
    }

    // 3. Write Inode
    if (minix_write_inode_raw(fs, inode_num, inode_ptr) != 0) {
...
static int minix_link(fs_node_t *dir, fs_node_t *node, const char *name) {
    ...
    if (v2) {
        ...
        if (inode->i_nlinks >= 65535) return -1;
        inode->i_nlinks++;
    } else {
    ...
    // TODO: Update ctime for node
    // For now we just write the inode which updates fields if they were modified in cached struct
    // We should explicitly set ctime here if we had a clock.
    // ((struct minix_inode_v?)node->ptr)->i_ctime = current_time;

    if (minix_write_inode(fs, node) != 0) {
        if (v2) ((struct minix_inode_v2 *)node->ptr)->i_nlinks--;
        else ((struct minix_inode_v1 *)node->ptr)->i_nlinks--;
        return -1;
    }

    if (minix_dir_add(dir, name, (uint16_t)node->inode) != 0) {
        if (v2) ((struct minix_inode_v2 *)node->ptr)->i_nlinks--;
        else ((struct minix_inode_v1 *)node->ptr)->i_nlinks--;
        minix_write_inode(fs, node);
        return -1;
    ...
    // We don't have current time easily available without syscalls/helper, skipping exact time update
    // but we should at least mark it dirty if we had a dirty flag.
    // For now just write it back to ensure size update from minix_dir_add persists if it changed.
    minix_write_inode(fs, dir);
