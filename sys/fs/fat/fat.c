...
static struct dirent *fat_readdir(fs_node_t *node, uint64_t index) {
    ...
    while (cluster < 0x0FFFFFFF) {
        ...
        for (uint32_t i = 0; i < cluster_size; i += 32) {
            ...
            if ((unsigned char)entry->name[0] == 0xE5) continue;      // Deleted entry
