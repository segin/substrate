...
static uint8_t uma_bootstrap_mem[UMA_BOOTSTRAP_ZONES * UMA_ZONE_SIZE_MAX];
...
void uma_startup(void) {
    ...
    if (uma_ncpu < 1) uma_ncpu = 1;
    if (uma_ncpu > UMA_MAX_CPUS) uma_ncpu = UMA_MAX_CPUS;
...
uma_zone_t *uma_zcreate(
    ...
    if (!uma_dynamic_alloc) {
        if (uma_bootstrap_idx < UMA_BOOTSTRAP_ZONES) {
            ...
            uma_bootstrap_idx++;
            memset(zone, 0, sizeof(uma_zone_t) + (uma_ncpu - 1) * sizeof(uma_cache_t));
        } else {
