...
/*
 * Linux i386 errno definitions.
 *
 * Substrate natively uses Linux-compatible errno values, so the
 * native-to-Linux translation is an identity mapping. This header
 * exists for completeness and to support future architectures where
 * the mapping may diverge.
 *
 * Reference: NetBSD sys/compat/linux/common/linux_errno.h
 */

/* POSIX errnos - identical on all Linux architectures */
...
/* i386-specific errnos (values diverge from generic on some arches) */
 ...
 * The C library makes the errno positive.
 */
#define LINUX_SCERR_SIGN -

/*
 * Substrate uses Linux-compatible errno values natively, so
 * native_to_linux_errno() is an identity function. If the
 * native errno numbering ever changes, populate a real table.
 */
static inline int native_to_linux_errno(int native_errno) {
	return native_errno;
}
