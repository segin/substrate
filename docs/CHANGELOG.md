...
- **A page-list link that is not a page can no longer become a PTE:** `vm_object_lookup_page()` walked an object's page list and returned whatever it found, so a single bad pointer went straight to `vm_fault()`, which took `m->phys_addr` — read out of whatever the pointer happened to address — and handed it to `pmap_enter()`. The fault still reported success, so the process silently got a mapping to arbitrary memory: Microsoft Word 3.0's free-list head came back as `0xc011cf2e`, a kernel virtual address that is not even page aligned, and its allocator's circular scan never terminated. Page-array membership is exactly decidable, so `vm_phys_page_is_valid()` (`sys/vm/phys_mem.c`) answers it safely for an arbitrary pointer — in bounds, on an element boundary, canaries intact, checked in that order because `vm_page_valid()` dereferences. The list walk stops at the first link that fails it and `vm_fault` treats an unsound page as simply not resident, filling a fresh one; the pages past a bad link were already unreachable, so nothing is lost that was not lost already.
...
- **TTY reads honour `O_NONBLOCK`:** the tty layer never looked at it, so a non-blocking read of a terminal blocked anyway -- forever, if nothing was typed. The read callbacks are handed only an `fs_node_t*`, so per-fd status flags are reachable solely through the `file_t` that read(2) publishes on the thread for the duration of the call (`current_thread->io_file`); af_unix, af_inet, af_packet and the input subsystem all consult it already and the tty simply never did. `kern_read()` did not publish it either -- only `sys_read()` did -- so the personalities, which call `kern_read()`, could not have seen the flag even had the tty asked. Both fixed: `tty_read()` now returns the bytes it has, or `EAGAIN`, instead of parking, in both the raw and canonical branches. This is why Microsoft Word 3.0 painted its screen and then went dead to the keyboard: it polls the classic System V way -- `fcntl(F_SETFL, O_NDELAY)`, read one byte, clear the flag -- and that read never came back.
...
- **Xenix/86 binaries run under the same personality:** a real Xenix/286 ran them, and an 8086 `x.out` (`x_cpu` 0x04) is not a real-mode image — it carries the same protected-mode LDT selectors and segment table as a 286 one, and the 8086 instruction set is a strict subset. The loader had claimed only 0x09 and rejected the rest, which excluded the entire Development System: SCO shipped it as the *x86* Development System, 97 of the media's 304 binaries.
- **The 8087 emulator these binaries actually contain:** they hold no x87 instructions at all — the linker rewrote each one into a two-byte software interrupt (`INT 0xF0+n` for `ESC 0xD8+n`) so the program would run on a machine with no coprocessor. Only `CD 05` was recognised, so the first `fldcw` of a program's FP init took an unhandled #GP. They are patched back to `9B <ESC>` in place — the encoding is two bytes precisely so that fits — and the real x87 runs them.
- **Large-data images get far argv/envp:** an `XE_LDATA` image is handed argv and envp as arrays of 4-byte far pointers, not 2-byte near offsets, and its crt0 walks the vector with a stride of 4 stopping on a NULL *far* pointer. Given near offsets it read each pair as one pointer and took the second offset as a selector — which killed `emacs`, `tput`, `kermit` and `awk` in the C runtime before `main`.
- **Impure (combined I&D) images load:** `cc`'s default output has `XE_SEP` clear, no text segment at all, and an `xe_eseg` naming a selector that appears nowhere in the segment table. Every stock Xenix binary is separate I&D, so nothing exercised that shape until the compiler could run; the loader now aliases a code descriptor onto DGROUP for it.
- **16-bit `lseek` and `chsize`:** `lseek` widened its offset unsigned, so every negative seek became ~4 GiB, and `chsize` took CX/SI as 32-bit halves rather than 16-bit ones. Both surfaced through `brand(1)`, the serialization tool, which produced 8 GB files.
- **Xenix `O_NDELAY` reads report "nothing there" as a zero-length read:** System V, and so Xenix, has no `EAGAIN` for this -- it returns 0, famously indistinguishable from end of file. The personality now maps substrate's `-EAGAIN` to 0 on `read`, since handing a 1987 binary an errno it predates would set carry and look like a hard error.
- **`stkgrow` no longer spends the program's own stack reserve:** it demanded `brk + X286_STACK_GUARD` of clearance, but that guard is already charged on the break's side twice over (`brkctl` caps at `XOUT286_WINDOW_SIZE - XOUT286_STACK_RESERVE`, `x286_set_break` at `sp - X286_STACK_GUARD`). Charging it again here spends it out of the stack. Word reserves exactly 2 KiB, stopping its break at 0xf800 for the purpose, and the guard made half of that unusable: `stkgrow 0xfac0`, a 1344-byte stack well inside the reserve, returned ENOMEM and Word took SIGBUS. The floor is the break.
- **Status:** a 37-command sample runs clean — 37 ok, 0 crash, 0 noload — `cc` compiles and links end to end with correct floating point, and Microsoft Word 3.0 reaches its full editing screen under `TERM=ansi`. Host-side tooling for the media lives in `tools/xenix/` (`xenixfs.py` reads SCO Xenix / System V filesystem images with MBR partition support; `build-image.sh` builds the populated `/perso/xenix286s` image).
