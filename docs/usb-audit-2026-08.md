...
are ordered by severity.  See Status below for what has been fixed.
...
**All 22 fixed**, in commits d67cd42ff, d4d7dd218, a18ae9aac, 36e1e080e,
edfde844b, 930468c9f, 547f3e50a, 0265f44d0.  Each is boot-verified under QEMU
on all three HCDs.

The paths that only exist on real hardware are implemented to the spec and to
what both references do, but cannot be exercised under emulation and are
labelled untested in their commits: split transactions to a full/low-speed
device behind a high-speed hub (QEMU's usb-hub is full-speed and refuses to
attach to an EHCI bus), and SuperSpeed enumeration (QEMU's usb-storage
negotiates high speed).
