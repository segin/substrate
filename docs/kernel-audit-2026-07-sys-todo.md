...
- [ ] **A05** [sys/arch/i386/pmap.c:1897] TLB shootdown ACK counter is global and shared across rounds; wait times out silently
- [ ] **A06** [sys/drivers/usb/uac.c:684] uac_detach: use-after-free of freed usb_device via feeder kthread on hot-unplug
- [ ] **A07** [sys/drivers/video/fb.c:593] Runtime FBIOPUT_VIDEO_MODE desyncs fb geometry from console shadow buffer -> OOB shadow read
- [ ] **A08** [sys/exec/formats/elf.c:1700] exec raises setuid/setgid credentials and resets signal dispositions before the last fallible step
- [ ] **A09** [sys/exec/formats/elf.c:756] Double-free of segment pages on elf_load error paths
- [ ] **A10** [sys/exec/formats/elf.c:1743] Failed execve after setuid does not roll back credentials or signal state
...
- [ ] **A27** [sys/pm/wait.c:160] Concurrent wait4() double-frees a zombie child
- [ ] **A28** [sys/vfs/vfs.c:1371] mountlist traversed lock-free while mount/unmount mutate and kfree() entries
- [ ] **A29** [sys/vm/uma_core.c:998] UMA page hash walked without slab lock — use-after-free of freed slab headers
- [ ] **A30** [sys/vm/vm_fault.c:160] Write to PROT_READ / PROT_NONE private mapping silently granted (protection bypass)
- [ ] **A31** [sys/vm/vm_object.c:172] In-place COW eviction frees a still-mapped frame; later pmap_enter unholds a recycled frame

## MEDIUM

- [ ] **A32** [sys/arch/i386/fpu/fpu_emu.c:20] Lazy-FPU owner is a single global, not per-CPU
...
- [ ] **A40** [sys/kern/geom/geom_mbr.c:63] MBR extended-partition (EBR) chain parse can loop forever on crafted media
...
- [ ] **A54** [sys/kern/lockmgr.c:118] lockmgr LK_WANT_EXCL stuck set when a parked exclusive waiter is killed
...
- [ ] **A72** [sys/pm/pgrp.c:342] pgrp/session syscalls return bare -1 instead of -errno
- [ ] **A73** [sys/pm/pgrp.c:397] sys_setpgid leaks a freshly allocated session when pgrp_alloc fails
- [ ] **A74** [sys/pm/process.c:627] fork error paths and proc_destroy leak the chroot root_node vnode reference
- [ ] **A75** [sys/pm/sched.c:413] sched_yield walks allthread without tid_lock (KERN-06 gap) — SMP UAF
- [ ] **A76** [sys/pm/wait.c:192] Grandchildren's fault/context-switch counters are double-counted in wait4
- [ ] **A77** [sys/vm/phys_mem.c:513] vm_phys_mark_used reserves pages without setting PG_PMM_ALLOC
- [ ] **A78** [sys/vm/uma_core.c:1000] uma_zfree double-free guard is a non-atomic check-then-set (TOCTOU)
- [ ] **A79** [sys/vm/vm_kmem.c:300] kmem_get_stats/kmem_get_snapshot take kmem_stats_lock without disabling IRQs
- [ ] **A80** [sys/vm/vm_object.c:16] vm_object bootstrap pool allocation counter is unsynchronized
- [ ] **A81** [sys/vm/vm_syscalls.c:694] sys_msync returns bare -1 instead of -errno
- [ ] **A82** [sys/arch/i386/pmap.c:838] pmap_enter on a non-current pmap switches CR3 but is not preemption-safe
- [ ] **A83** [sys/drivers/video/bdf_pcf.c:298] bdf_parse: 32-bit multiply overflow in total_glyph_size -> undersized kmalloc -> heap overflow
- [ ] **A84** [sys/drivers/video/bdf_pcf.c:91] pcf_parse: uint32 overflow of offset+constant bypasses table bounds checks -> wild-pointer OOB read
- [ ] **A85** [sys/fs/udf/udf_write.c:692] UDF inline write: ext_attr_length underflow + offset+size overflow → heap OOB write
- [ ] **A86** [sys/kern/runqueue.c:110] runqueue_remove recomputes level from mutable priority -> wrong-queue list corruption
- [ ] **A87** [sys/pm/sched.c:415] sched_yield pick loop walks allthread without tid_lock (SMP UAF)
