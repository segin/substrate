...
## 1. Purpose and Scope
`as` is the Substrate system assembler for i386 and x86_64.
...
- Integration with `ld` and existing toolchain flows.
...
## 6. Assembly Backend
`as` is a driver that forwards source to the host GCC/GAS backend for both `-32`
and `-64` modes. This yields complete practical x86 ISA coverage (including modern
extensions supported by the host toolchain).

Behavior:
- `as` normalizes options (`-I`, `-D`, `-march`, `-mtune`, `-Wa`, `-g`) and
