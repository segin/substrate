# Substrate Native ABI (x86_64)

This document describes the binary interface for the Substrate Native personality on x86-64.
...
| 4      | `st_ino`        | `uint64_t` | 8    | Inode number                        |
...
| 56     | `st_atime`      | `time_t`   | 8    | Time of last access                 |
| 64     | `st_atime_nsec` | `uint32_t` | 4    | Nsecs of last access                |
| 68     | `st_pad2`       | `uint32_t` | 4    | Padding                             |
| 72     | `st_mtime`      | `time_t`   | 8    | Time of last modification           |
| 80     | `st_mtime_nsec` | `uint32_t` | 4    | Nsecs of last modification          |
| 84     | `st_pad3`       | `uint32_t` | 4    | Padding                             |
| 88     | `st_ctime`      | `time_t`   | 8    | Time of last status change          |
| 96     | `st_ctime_nsec` | `uint32_t` | 4    | Nsecs of last status change         |
| 100    | `st_pad4`       | `uint32_t` | 4    | Padding                             |

> [!IMPORTANT]
> **Packing Guarantee:** This structure is explicitly packed. The layout shown is the exact byte layout with no compiler-inserted padding. Use `__attribute__((packed))` or equivalent when declaring.

**(Total Size: 104 bytes with 64-bit ino_t)**
