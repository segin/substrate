...
- `TIOCSCTTY`: assigns the tty to the calling session leader and sets foreground pgrp to the caller's process group
- `TIOCNOTTY`: clears tty session and foreground pgrp when invoked by the owning session
