...
- ANSI status replies that are terminal input rather than display output, such as `CSI 5 n` and `CSI 6 n`, are reinjected into the active VT tty input queue instead of being rendered on screen.
