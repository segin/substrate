    ...
    - [x] **Core Structures & Life Cycle:** (REQ: REQ-04-0002)
        - [x] **`struct vnode`:** (REQ: REQ-04-0003)
            ...
            - [x] **Life Cycle:** (REQ: REQ-04-0014)
                ...
                - [x] `vput(vp)`: unlock and vrele. (REQ: REQ-04-0018)
        ...
        - [x] **`struct mount`:** (REQ: REQ-04-0024)
            - [x] `mnt_vnodecovered` (vnode mounted on). (REQ: REQ-04-0025)
            - [x] `mnt_op` (VFS ops vector). (REQ: REQ-04-0026)
            - [x] `mnt_data` (private fs data). (REQ: REQ-04-0027)
            - [x] `mnt_flag`: MNT_RDONLY, MNT_NOEXEC, MNT_NOSUID, MNT_NODEV, MNT_SYNCHRONOUS, MNT_ASYNC, MNT_UNION, MNT_LOCAL. (REQ: REQ-04-0028)
            - [x] `mnt_nvnodelist` (list of active vnodes). (REQ: REQ-04-0029)
            - [x] `mnt_stat` (cached `struct statfs`). (REQ: REQ-04-0030)
            - [x] `mnt_maxsymlinklen`: max symlink target stored inline. (REQ: REQ-04-0031)
            - [x] `mnt_lock`: mount-level reader/writer lock. (REQ: REQ-04-0032)
        - [x] **`struct file`:** (REQ: REQ-04-0033)
            - [x] `f_type`: DTYPE_VNODE, DTYPE_SOCKET, DTYPE_PIPE, DTYPE_KQUEUE. (REQ: REQ-04-0034)
            - [x] `f_data` (pointer to vnode/socket/pipe). (REQ: REQ-04-0035)
            - [x] `f_flag`: FREAD, FWRITE, FNONBLOCK, FAPPEND, O_DIRECT, O_CLOEXEC. (REQ: REQ-04-0036)
            - [x] `f_ops` (file operations vector: read, write, ioctl, poll, close, stat). (REQ: REQ-04-0037)
            - [x] `f_offset` (current file offset, atomic for concurrent access). (REQ: REQ-04-0038)
            - [x] `f_count` (reference count). (REQ: REQ-04-0039)
            - [x] `f_cred` (credentials at open time). (REQ: REQ-04-0040)

    - [x] **Pathname Lookup (`namei`):** (REQ: REQ-04-0041)
        - [x] **`struct nameidata`:** lookup state, path string, purpose (LOOKUP/CREATE/DELETE/RENAME). (REQ: REQ-04-0042)
        - [x] **`struct componentname`:** current component, `cn_flags`, `cn_nameptr`, `cn_namelen`. (REQ: REQ-04-0043)
        - [x] **Lookup Logic:** (REQ: REQ-04-0044)
            - [x] Parse `/` delimiters, handle multiple consecutive slashes. (REQ: REQ-04-0045)
            - [x] Handle `.` (current directory) and `..` (parent directory). (REQ: REQ-04-0046)
            - [x] Cross mount points via `mnt_vnodecovered`/`v_mountedhere`. (REQ: REQ-04-0047)
            - [x] Symbolic link resolution with recursion limit (MAXSYMLINKS = 32). (REQ: REQ-04-0048)
            - [x] Handle trailing slash on non-directory (ENOTDIR). (REQ: REQ-04-0049)
            - [x] Handle `AT_FDCWD` and `*at()` syscall relative lookups. (REQ: REQ-04-0050)
        - [x] **Name Cache (`nchash`):** (REQ: REQ-04-0051)
            - [x] Global hash table: `(directory vnode, name)` → target vnode. (REQ: REQ-04-0052)
            - [x] Negative entries: cache "does not exist" results. (REQ: REQ-04-0053)
            - [x] `cache_lookup(dvp, vpp, cnp)`: check cache. (REQ: REQ-04-0054)
            - [x] `cache_enter(dvp, vp, cnp)`: add to cache. (REQ: REQ-04-0055)
            - [x] `cache_purge(vp)`: remove all entries for vnode. (REQ: REQ-04-0056)
            - [x] `cache_purgevfs(mp)`: remove all entries for mount point. (REQ: REQ-04-0057)
            - [x] LRU eviction of cache entries under memory pressure. (REQ: REQ-04-0058)
            - [x] Reader/writer lock for SMP scalability. (REQ: REQ-04-0059)

    - [x] **Operations Vectors:** (REQ: REQ-04-0060)
        - [x] **`vfs_ops` (Filesystem-level):** (REQ: REQ-04-0061)
            - [x] `vfs_mount(mp, path, data, ndp, p)`: mount filesystem at path. (REQ: REQ-04-0062)
            - [x] `vfs_start(mp, flags, p)`: post-mount initialization. (REQ: REQ-04-0063)
            - [x] `vfs_unmount(mp, mntflags, p)`: unmount, flush, free. (REQ: REQ-04-0064)
                - [x] `MNT_FORCE`: forced unmount (kill active references). (REQ: REQ-04-0065)
            - [x] `vfs_root(mp, vpp)`: return root vnode. (REQ: REQ-04-0066)
            - [x] `vfs_statfs(mp, sbp, p)`: fill `struct statfs` (f_blocks, f_bfree, f_bavail, f_files, f_ffree). (REQ: REQ-04-0067)
            - [x] `vfs_sync(mp, waitfor, cred, p)`: sync dirty buffers. (REQ: REQ-04-0068)
                - [x] `MNT_WAIT` (synchronous) / `MNT_NOWAIT` (asynchronous). (REQ: REQ-04-0069)
            - [x] `vfs_vget(mp, ino, vpp)`: get vnode by inode number. (REQ: REQ-04-0070)
            - [x] `vfs_fhtovp(mp, fhp, vpp)`: NFS file handle → vnode. (REQ: REQ-04-0071)
            - [x] `vfs_init(vfsp)`: filesystem type initialization (register). (REQ: REQ-04-0072)
            - [x] `vfs_uninit(vfsp)`: filesystem type teardown (unregister). (REQ: REQ-04-0073)
        - [x] **`vnode_ops` (File-level):** (REQ: REQ-04-0074)
            - [x] **Name Resolution:** (REQ: REQ-04-0075)
                - [x] `vop_lookup(dvp, vpp, cnp)`: look up component in directory. (REQ: REQ-04-0076)
                - [x] `vop_cachedlookup(dvp, vpp, cnp)`: cache-first wrapper. (REQ: REQ-04-0077)
            - [x] **Creation/Deletion:** (REQ: REQ-04-0078)
                - [x] `vop_create(dvp, vpp, cnp, vap)`: create regular file. (REQ: REQ-04-0079)
                - [x] `vop_mknod(dvp, vpp, cnp, vap)`: create device node (block/char/FIFO). (REQ: REQ-04-0080)
                - [x] `vop_mkdir(dvp, vpp, cnp, vap)`: create directory (with `.` and `..`). (REQ: REQ-04-0081)
                - [x] `vop_whiteout(dvp, cnp, flags)`: UnionFS whiteout entry. (REQ: REQ-04-0082)
                - [x] `vop_remove(dvp, vp, cnp)`: unlink directory entry. (REQ: REQ-04-0083)
                - [x] `vop_rmdir(dvp, vp, cnp)`: remove empty directory. (REQ: REQ-04-0084)
            - [x] **Access/Attributes:** (REQ: REQ-04-0085)
                - [x] `vop_access(vp, mode, cred, p)`: check r/w/x permissions (POSIX semantics). (REQ: REQ-04-0086)
                - [x] `vop_getattr(vp, vap, cred, p)`: stat (mode, nlink, uid, gid, size, times). (REQ: REQ-04-0087)
                - [x] `vop_setattr(vp, vap, cred, p)`: chmod/chown/truncate/utimes. (REQ: REQ-04-0088)
                - [x] `vop_pathconf(vp, name, retval)`: POSIX pathconf (`_PC_LINK_MAX`, `_PC_NAME_MAX`, etc.). (REQ: REQ-04-0089)
            - [x] **I/O Operations:** (REQ: REQ-04-0090)
                - [x] `vop_open(vp, mode, cred, p)`: open callback (access check, device init). (REQ: REQ-04-0091)
                - [x] `vop_close(vp, fflag, cred, p)`: close callback (sync on last close). (REQ: REQ-04-0092)
                - [x] `vop_read(vp, uio, ioflag, cred)`: read via buffer cache, update atime. (REQ: REQ-04-0093)
                - [x] `vop_write(vp, uio, ioflag, cred)`: write, extend file, update mtime. (REQ: REQ-04-0094)
                    - [x] Flags: `IO_APPEND`, `IO_SYNC`, `IO_UNIT`. (REQ: REQ-04-0095)
                - [x] `vop_ioctl(vp, cmd, data, fflag, cred, p)`: device control. (REQ: REQ-04-0096)
                - [x] `vop_poll(vp, events, cred, p)`: select/poll (POLLIN/POLLOUT/POLLHUP/POLLERR). (REQ: REQ-04-0097)
                - [x] `vop_fsync(vp, cred, waitfor, p)`: flush dirty data. (REQ: REQ-04-0098)
                - [x] `vop_bmap(vp, bn, vpp, bnp, runp)`: logical→physical block mapping. (REQ: REQ-04-0099)
                - [x] `vop_strategy(vp, bp)`: submit buffer to block device. (REQ: REQ-04-0100)
            - [x] **Directories:** (REQ: REQ-04-0101)
                - [x] `vop_readdir(vp, uio, cred, eofflag, ncookies, cookies)`: read `struct dirent` entries. (REQ: REQ-04-0102)
            - [x] **Links:** (REQ: REQ-04-0103)
                - [x] `vop_link(dvp, vp, cnp)`: create hard link (increment nlink). (REQ: REQ-04-0104)
                - [x] `vop_rename(fdvp, fvp, fcnp, tdvp, tvp, tcnp)`: rename/move (atomic within FS). (REQ: REQ-04-0105)
                    - [x] Same-directory rename, cross-directory rename, overwrite existing. (REQ: REQ-04-0106)
                    - [x] Directory rename: update `..` entry, detect cycles. (REQ: REQ-04-0107)
                - [x] `vop_symlink(dvp, vpp, cnp, vap, target)`: create symbolic link. (REQ: REQ-04-0108)
                - [x] `vop_readlink(vp, uio, cred)`: read symlink target. (REQ: REQ-04-0109)
            - [x] **VM/Memory Integration:** (REQ: REQ-04-0110)
                - [x] `vop_inactive(vp, p)`: last FD closed — truncate if nlink=0, sync dirty. (REQ: REQ-04-0111)
                - [x] `vop_reclaim(vp, p)`: vnode recycled — free fs-private data. (REQ: REQ-04-0112)
                - [x] `vop_lock(vp, flags, p)`: lockmgr lock (LK_SHARED/EXCLUSIVE/NOWAIT/UPGRADE). (REQ: REQ-04-0113)
                - [x] `vop_unlock(vp, flags, p)`: lockmgr unlock. (REQ: REQ-04-0114)
                - [x] `vop_islocked(vp, p)`: query lock status. (REQ: REQ-04-0115)
                - [x] `vop_print(vp)`: debug dump (type, flags, refcount, fs info). (REQ: REQ-04-0116)
                - [x] `vop_advlock(vp, id, op, fl, flags)`: POSIX advisory locking (F_GETLK/F_SETLK/F_SETLKW). (REQ: REQ-04-0117)

    - [x] **Native Build Integration (`native_dist`):** (REQ: REQ-04-0118)
      - [x] Update `native_dist` target to build userspace with `HOSTCC`. (REQ: REQ-04-0119)
      - [x] Sanitize `Makefile.inc` for non-cross builds. (REQ: REQ-04-0120)
      - [x] Verify `bin/` and `sbin/` builds on host. (REQ: REQ-04-0121)
      - [x] Host output directory: `host_dist`. (REQ: REQ-04-0122)

    - [x] **Compiler Construction Tools:** (REQ: REQ-04-0123)
      - [x] **`yacc` (LALR(1) Parser Generator):** (REQ: REQ-04-0124)
        - [x] Initial skeleton (`usr.bin/yacc`). (REQ: REQ-04-0125)
        - [x] Grammar file parsing (rules, precedence, %token/%type). (REQ: REQ-04-0126)
        - [x] LALR(1) parse table generation (states, actions, gotos). (REQ: REQ-04-0127)
        - [x] Conflict resolution (shift/reduce, reduce/reduce). (REQ: REQ-04-0128)
        - [x] Output `y.tab.c` and `y.tab.h`. (REQ: REQ-04-0129)
        - [x] `-d` flag for header generation. (REQ: REQ-04-0130)
        - [x] `-v` flag for verbose state output (`y.output`). (REQ: REQ-04-0131)
      - [x] **`lex` (Lexical Analyzer Generator):** (REQ: REQ-04-0132)
        - [x] Regular expression to NFA conversion (Thompson's construction). (REQ: REQ-04-0133)
        - [x] NFA to DFA conversion (subset construction). (REQ: REQ-04-0134)
        - [x] DFA minimization (Hopcroft's algorithm). (REQ: REQ-04-0135)
        - [x] Output `lex.yy.c`. (REQ: REQ-04-0136)
        - [x] Character class support (`[a-zA-Z]`, POSIX classes). (REQ: REQ-04-0137)
        - [x] Start conditions (exclusive/inclusive). (REQ: REQ-04-0138)
        - [x] `yywrap()` and `yylex()` interface. (REQ: REQ-04-0139)

    - [x] **Buffer Cache Integration (`bio`):** (REQ: REQ-04-0140)
        ...
        - [x] **`struct buf` Definition:** (REQ: REQ-04-0141)
          - [x] Fields: `b_flags`, `b_data`, `b_bcount`, `b_blkno`, `b_lblkno`, `b_vp`, `b_rcred`, `b_wcred`, `b_resid`, `b_iodone`, `b_error`. (REQ: REQ-04-0142)
          - [x] Flags: `B_BUSY`, `B_DONE`, `B_ERROR`, `B_DELWRI`, `B_PHYS`, `B_READ`, `B_WRITE`, `B_ASYNC`, `B_INVAL`, `B_NOCACHE`, `B_CACHE`. (REQ: REQ-04-0143)
          - [x] Hash chain linkage for lookup by `(vp, blkno)`. (REQ: REQ-04-0144)
        - [x] **Buffer Queues:** (REQ: REQ-04-0145)
          - [x] `BQ_LOCKED`: buffers currently in use. (REQ: REQ-04-0146)
          - [x] `BQ_CLEAN`: clean LRU (eligible for reuse). (REQ: REQ-04-0147)
          - [x] `BQ_DIRTY`: delayed write queue. (REQ: REQ-04-0148)
          - [x] `BQ_EMPTY`: free buffer headers without data. (REQ: REQ-04-0149)
        - [x] **Buffer Cache Functions:** (REQ: REQ-04-0150)
          - [x] `getblk(vp, blkno, size, slpflag, slptimeo)`: get from cache or allocate. (REQ: REQ-04-0151)
          - [x] `bread(vp, blkno, size, cred, bpp)`: synchronous read (getblk + I/O + biowait). (REQ: REQ-04-0152)
          - [x] `breada(vp, blkno, size, rablkno, rabsize, cred, bpp)`: read-ahead variant. (REQ: REQ-04-0153)
          - [x] `bwrite(bp)`: synchronous write. (REQ: REQ-04-0154)
          - [x] `bawrite(bp)`: asynchronous write (initiate, return immediately). (REQ: REQ-04-0155)
          - [x] `bdwrite(bp)`: delayed write (mark B_DELWRI, release). (REQ: REQ-04-0156)
          - [x] `brelse(bp)`: release buffer to appropriate queue. (REQ: REQ-04-0157)
          - [x] `incore(vp, blkno)`: check if block is in cache. (REQ: REQ-04-0158)
        - [x] **Synchronization:** (REQ: REQ-04-0159)
          - [x] `biowait(bp)`: sleep until B_DONE. (REQ: REQ-04-0160)
          - [x] `biodone(bp)`: mark complete, call `b_iodone`, wakeup. (REQ: REQ-04-0161)
        - [x] **Flushing:** (REQ: REQ-04-0162)
          - [x] `bufsync(freq)`: periodic daemon to write dirty buffers. (REQ: REQ-04-0163)
          - [x] `sync()` syscall: flush all dirty buffers. (REQ: REQ-04-0164)
          - [x] `update` daemon (`syncer` kthread): periodic sync every 30 seconds. (REQ: REQ-04-0165)

    - [x] **VM Integration (Unified Buffer Cache):** (REQ: REQ-04-0166)
      - [x] **VNode Pager:** (REQ: REQ-04-0167)
        - [x] `vnode_pager_getpages()`: read pages from vnode into `vm_page` array. (REQ: REQ-04-0168)
        - [x] `vnode_pager_putpages()`: write dirty pages to vnode. (REQ: REQ-04-0169)
        - [x] Integrate with buffer cache (shared backing pages). (REQ: REQ-04-0170)
      - [x] **Page Cache Integration:** (REQ: REQ-04-0171)
        - [x] Buffer data backed by `vm_page_t` (not `kmalloc`). (REQ: REQ-04-0172)
        - [x] Unified memory accounting for file cache and buffer cache. (REQ: REQ-04-0173)
        - [x] Zero-copy I/O: mmap shares pages with buffer cache. (REQ: REQ-04-0174)

    - [x] **Concurrency & Locking:** (REQ: REQ-04-0175)
        - [x] **VNode Locks (`lockmgr`):** (REQ: REQ-04-0176)
            - [x] `lockmgr(lkp, flags, interlock, p)`: unified lock manager. (REQ: REQ-04-0177)
            - [x] Modes: `LK_SHARED`, `LK_EXCLUSIVE`, `LK_UPGRADE`, `LK_DOWNGRADE`, `LK_DRAIN`. (REQ: REQ-04-0178)
            - [x] `LK_NOWAIT`: non-blocking (return EBUSY). (REQ: REQ-04-0179)
            - [x] Priority inheritance for blocked threads. (REQ: REQ-04-0180)
            - [x] Multiple concurrent readers, single exclusive writer. (REQ: REQ-04-0181)
        - [x] **Interlock:** `v_interlock` mutex for vnode fields (v_usecount, v_flag, v_numoutput). (REQ: REQ-04-0182)
        - [x] **Mount Lock:** `mnt_lock` prevents unmount during vnode operations. (REQ: REQ-04-0183)
        - [x] **Namecache Lock:** reader/writer lock on `nchash` for concurrent lookups. (REQ: REQ-04-0184)
        - [x] **Buffer Lock:** per-buffer `B_BUSY` flag with sleep/wakeup. (REQ: REQ-04-0185)
...
- [ ] **EXT2 (Native Filesystem):** (REQ: REQ-04-0210)
    ...
    - [x] Inode allocation/freeing (bitmap-based). (REQ: REQ-04-0211)
    - [x] Block allocation/freeing (bitmap-based, block groups). (REQ: REQ-04-0212)
    - [x] Directory entry creation/deletion. (REQ: REQ-04-0213)
    - [x] **Extended Features:** (REQ: REQ-04-0214)
        - [x] ext2 revision 1: variable inode size, file type in dir entry. (REQ: REQ-04-0215)
        - [x] Sparse superblock: superblock copies only in select block groups. (REQ: REQ-04-0216)
        - [x] Large file support (>2 GB via triple-indirect blocks). (REQ: REQ-04-0217)
    - [x] **VFS Integration:** (REQ: REQ-04-0218, REQ: REQ-0233, REQ-04-0353)
        - [x] Implement `ext2_mount` / `ext2_unmount` / `ext2_root` / `ext2_statfs`. (REQ: REQ-04-0219)
        - [x] Implement `ext2_lookup` / `ext2_create` / `ext2_remove` / `ext2_rename`. (REQ: REQ-04-0220)
        - [x] Implement `ext2_read` / `ext2_write` / `ext2_fsync`. (REQ: REQ-04-0221)
        - [x] Implement `ext2_readdir` / `ext2_mkdir` / `ext2_rmdir`. (REQ: REQ-04-0222)
        - [x] Implement `ext2_link` / `ext2_symlink` / `ext2_readlink`. (REQ: REQ-04-0223)
    - [ ] **Testing:** (REQ: REQ-04-0186, REQ-04-0224, REQ-04-0244, REQ-04-0277, REQ-04-0305, REQ-04-0328, REQ-04-0339, REQ-04-0360)
