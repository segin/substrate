...
- [ ] **System Call ABI (64-bit Clean):** (REQ: REQ-05-0001)
    ...
    - [x] **Kernel Refactoring:** (REQ: REQ-05-0008, REQ-05-0357)
        - [x] Native `sys_lseek` takes 64-bit offset (split hi/lo on 32-bit stack). (REQ: REQ-05-0009)
        - [x] Native `sys_ftruncate` takes 64-bit length (split hi/lo on 32-bit stack). (REQ: REQ-05-0010)
        - [x] `sys_mmap` offset is 64-bit (using split words on i386). (REQ: REQ-05-0011)
        - [x] `struct stat` uses 64-bit `ino_t`, `off_t`, `blkcnt_t`, `time_t`. (REQ: REQ-05-0012)
    - [x] **LibC Wrappers:** (REQ: REQ-05-0013)
        - [x] Update `lseek` wrapper to pass high/low words on 32-bit. (REQ: REQ-05-0014)
        - [x] Update `ftruncate` wrapper. (REQ: REQ-05-0015)
        - [x] Update `stat`/`fstat` wrappers. (REQ: REQ-05-0016)
    - [x] **Personality Compatibility:** (REQ: REQ-05-0017, REQ-05-0366)
        - [x] Ensure 32-bit Linux personality handles register splitting for 64-bit args correctly. (REQ: REQ-05-0018)
        - [x] Ensure native personality mandates 64-bit types. (REQ: REQ-05-0019)
...
- [ ] **Personalities:** (REQ: REQ-05-0376)
    - [ ] **Xenix & SCO Compatibility (The 6 Flavors):** (REQ: REQ-05-0377)
        ...
        - [ ] **Binary Loaders:** (REQ: REQ-05-0394)
            ...
            - [ ] **COFF (Common Object File Format) - SCO/SVR3:** (REQ: REQ-05-0399)
                ...
                - [x] **Optional Header (`aouthdr`):** (REQ: REQ-05-0405)
                    - [x] Magic: 0x10B (ZMAGIC - Demand Paged). (REQ: REQ-05-0406)
                        - [x] Define `AOUT_ZMAGIC` constant (0x10B). (REQ: REQ-05-0407)
                        - [x] Validate magic number, reject unsupported types (OMAGIC, NMAGIC). (REQ: REQ-05-0408)
                    - [x] `tsize` (Text size), `dsize` (Data size), `bsize` (BSS size). (REQ: REQ-05-0409)
                        - [x] Parse `tsize` from offset 4 (uint32_t). (REQ: REQ-05-0410)
                        - [x] Parse `dsize` from offset 8 (uint32_t). (REQ: REQ-05-0411)
                        - [x] Parse `bsize` from offset 12 (uint32_t). (REQ: REQ-05-0412)
                        - [x] Validate sizes are reasonable (< 2GB, page-aligned for ZMAGIC). (REQ: REQ-05-0413)
                    - [x] `entry` (Entry point virtual address). (REQ: REQ-05-0414)
                        - [x] Parse `entry` from offset 16 (uint32_t). (REQ: REQ-05-0415)
                        - [x] Validate entry is within `.text` segment bounds. (REQ: REQ-05-0416)
                    - [x] `text_start`, `data_start`. (REQ: REQ-05-0417)
                        - [x] Parse `text_start` from offset 20 (uint32_t). (REQ: REQ-05-0418)
                        - [x] Parse `data_start` from offset 24 (uint32_t). (REQ: REQ-05-0419)
                        - [x] Validate `text_start` < `data_start` or handle BSS-only cases. (REQ: REQ-05-0420)
    ...
    - [/] **Exec Implementation & Binary Loaders:**
        - [x] **ELF Loader Improvements:** (REQ: REQ-05-0633)
            - [x] **Stack Setup:** Properly terminate argv/envp vectors. (REQ: REQ-05-0634)
            - [ ] **Default Environment:** Populate PATH and TERM. (REQ: REQ-05-0635)  <!-- N/A: kernel does not impose env defaults; init/login owns this -->
            - [x] **Auxiliary Vector:** Populate AT_PHDR/PHENT/PHNUM/ENTRY for TLS support. (REQ: REQ-05-0636)
            - [x] **Interpreter:** Handle `PT_INTERP` (load ld.so). (REQ: REQ-05-0637)
            - [x] **Auxiliary Vector:** (REQ: REQ-05-0638)
                - [x] Populate `AT_PHDR`, `AT_PHENT`, `AT_PHNUM`. (REQ: REQ-05-0639)
                - [x] Populate `AT_ENTRY`, `AT_BASE` (interpreter base). (REQ: REQ-05-0640)
                - [x] Populate `AT_EXECFN`, `AT_PLATFORM`. (REQ: REQ-05-0641)
                - [x] Populate `AT_RANDOM`, `AT_SECURE`. (REQ: REQ-05-0642)
        - [x] **Shebang Support (`exec_script`):** (REQ: REQ-05-0643)
            - [x] Parse `#!` line. (REQ: REQ-05-0644)
            - [x] Handle recursion depth limits (prevent loops). (REQ: REQ-05-0645)
            - [x] Argument parsing (`#! /bin/sh -x`). (REQ: REQ-05-0646)
        - [/] **COFF Loader (Legacy/Static):** (REQ: REQ-05-0647)
            - [x] Parse File Header and Optional Header. (REQ: REQ-05-0648)
            - [x] Map Sections (Text, Data, BSS) to fixed addresses. (REQ: REQ-05-0649)
            - [x] Basic relocation support (for Xenix 386). (REQ: REQ-05-0650)
        ...
        - [x] **Personality / Migration:** (REQ: REQ-05-0665)
            ...
            - [x] **Signal Translation:** Remap signal numbers. (REQ: REQ-05-0668)
        - [x] **fork() Completeness:** (REQ: REQ-05-0669)
            - [x] **Copy-on-Write:** Duplication of VM space (refcounts). (REQ: REQ-05-0670)
            - [x] **File Descriptors:** Increment refcounts on all FDs. (REQ: REQ-05-0671)
            - [x] `vfork`: Shared VM space, parent blocked until child exec/exit. (REQ: REQ-05-0672)
