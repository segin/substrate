...
### 6. C Library (`lib/c`)
- [x] **`sysctl` Userspace API (`lib/c`):** (REQ: REQ-06-0001)
    - [x] **Headers & ABI Contracts:** (REQ: REQ-06-0002)
        - [x] Define and export canonical declarations in `include/sys/sysctl.h`. (REQ: REQ-06-0003)
        - [x] Define and export stable userspace data types for MIB paths and metadata results. (REQ: REQ-06-0004)
        - [x] Add compile-time guards for structure size/alignment ABI stability. (REQ: REQ-06-0005)
        - [x] Document thread-safety guarantees and reentrancy expectations in headers. (REQ: REQ-06-0006)
    - [x] **Core libc Entry Points:** (REQ: REQ-06-0007)
        - [x] Implement `sysctl()` libc wrapper with strict argument validation. (REQ: REQ-06-0008)
        - [x] Implement `sysctlbyname()` using name-to-MIB translation helper flow. (REQ: REQ-06-0009)
        - [x] Implement `sysctlnametomib()` with deterministic retry behavior. (REQ: REQ-06-0010)
        - [x] Implement helper for dynamic buffer sizing/retry loop (`ENOMEM` growth pattern). (REQ: REQ-06-0011)
        - [x] Implement typed convenience helpers (`int`, `uint`, `quad`, `string`) with explicit bounds checks. (REQ: REQ-06-0012)
    - [x] **Thread Safety & Robustness:** (REQ: REQ-06-0013)
        - [x] Ensure no static mutable buffers in libc `sysctl` helpers. (REQ: REQ-06-0014)
        - [x] Ensure all helpers preserve `errno` semantics consistently on failure. (REQ: REQ-06-0015)
        - [x] Add cancellation-safety review for wrappers used in multi-threaded callers. (REQ: REQ-06-0016)
        - [x] Add overflow-safe arithmetic helpers for buffer growth and MIB conversion. (REQ: REQ-06-0017)
    - [x] **Testing (libc Side):** (REQ: REQ-06-0018)
        - [x] Add unit tests for wrapper argument validation and errno behavior. (REQ: REQ-06-0019)
        - [x] Add unit tests for name-to-MIB and MIB-to-name translation helpers. (REQ: REQ-06-0020)
        - [x] Add integration tests for size-probe + retry loops across changing kernel values. (REQ: REQ-06-0021)
    ...
    - [x] **Manpages & Usage Guidance:** (REQ: REQ-06-0024)
...
- [x] **Stdio:** (REQ: REQ-06-0030)
    - [x] **`FILE` Structure & Buffer Management (`lib/c/src/stdio/`):** (REQ: REQ-06-0031)
        - [x] Define internal `FILE` structure (`_flags`, `_buf`, `_bufsiz`, `_cnt`, `_ptr`, `_fd`, `_lbfsize`, `_close`/`_read`/`_write`/`_seek` function pointers). (REQ: REQ-06-0032)
        - [x] Implement three buffer modes: (REQ: REQ-06-0033)
            - [x] `_IOFBF` (fully buffered): flush on buffer full or explicit `fflush()`. (REQ: REQ-06-0034)
            - [x] `_IOLBF` (line buffered): flush on newline or buffer full. (REQ: REQ-06-0035)
            - [x] `_IONBF` (unbuffered): every `fputc()`/`fwrite()` calls `write()` immediately. (REQ: REQ-06-0036)
        - [x] Implement `setvbuf(FILE *stream, char *buf, int mode, size_t size)`: (REQ: REQ-06-0037)
            - [x] Validate `mode` is `_IOFBF`, `_IOLBF`, or `_IONBF`. (REQ: REQ-06-0038)
            - [x] Allow user-supplied buffer or `NULL` for auto-allocation. (REQ: REQ-06-0039)
            - [x] Must be called after `fopen()` but before any I/O. (REQ: REQ-06-0040)
            - [x] Return 0 on success, non-zero on failure. (REQ: REQ-06-0041, REQ-06-0295)
        - [x] Implement `setbuf(FILE *stream, char *buf)` as wrapper for `setvbuf()`. (REQ: REQ-06-0042)
        - [x] Implement `setlinebuf(FILE *stream)` (BSD extension). (REQ: REQ-06-0043)
        - [x] Implement `fflush(FILE *stream)`: (REQ: REQ-06-0044)
            - [x] Write buffered output data via `write()` syscall. (REQ: REQ-06-0045)
            - [x] Handle partial writes (retry loop). (REQ: REQ-06-0046)
            - [x] If `stream == NULL`, flush all open output streams. (REQ: REQ-06-0044)
            - [x] Return 0 on success, `EOF` on error (set `errno`). (REQ: REQ-06-0048)
        - [x] Pre-allocate `stdin`, `stdout`, `stderr` `FILE` objects: (REQ: REQ-06-0049)
            - [x] `stdin`: fd 0, `_IOLBF` default. (REQ: REQ-06-0050)
            - [x] `stdout`: fd 1, `_IOLBF` if `isatty()`, else `_IOFBF`. (REQ: REQ-06-0051)
            - [x] `stderr`: fd 2, `_IONBF` always. (REQ: REQ-06-0052)
        - [x] Implement `__sinit()` / stream initialization on first use (lazy init pattern). (REQ: REQ-06-0053)
    - [x] **File Open/Close Operations:** (REQ: REQ-06-0054)
        - [x] `fopen(const char *path, const char *mode)`: (REQ: REQ-06-0055)
            - [x] Parse mode string: `"r"`, `"w"`, `"a"`, `"r+"`, `"w+"`, `"a+"`, with `"b"` (binary, no-op on POSIX) and `"x"` (C11 exclusive create). (REQ: REQ-06-0056)
            - [x] Map to `open()` flags: `O_RDONLY`, `O_WRONLY|O_CREAT|O_TRUNC`, `O_WRONLY|O_CREAT|O_APPEND`, etc. (REQ: REQ-06-0057)
            - [x] Allocate `FILE`, assign fd, set default buffer. (REQ: REQ-06-0058)
            - [x] Return `NULL` on failure (set `errno`). (REQ: REQ-06-0059)
        - [x] `fdopen(int fd, const char *mode)`: wrap existing fd in `FILE`. (REQ: REQ-06-0060)
        - [x] `freopen(const char *path, const char *mode, FILE *stream)`: (REQ: REQ-06-0061)
            - [x] Flush and close existing fd. (REQ: REQ-06-0062)
            - [x] Re-open with new path/mode, reuse `FILE` object. (REQ: REQ-06-0063)
            - [x] If `path == NULL`, change mode of existing fd (C99 extension). (REQ: REQ-06-0061)
        - [x] `fclose(FILE *stream)`: (REQ: REQ-06-0065)
            - [x] Flush output buffer via `fflush()`. (REQ: REQ-06-0066)
            - [x] Free allocated buffer (if library-allocated). (REQ: REQ-06-0067)
            - [x] `close()` underlying fd. (REQ: REQ-06-0068)
            - [x] Free `FILE` structure. (REQ: REQ-06-0069)
            - [x] Return 0 on success, `EOF` on error. (REQ: REQ-06-0070)
        - [x] `fcloseall()` (extension): close all open streams. (REQ: REQ-06-0071)
    - [x] **Character & Line I/O:** (REQ: REQ-06-0072)
        - [x] `fgetc(FILE *stream)` / `getc()` macro: read one byte from buffer or refill. (REQ: REQ-06-0073)
        - [x] `fputc(int c, FILE *stream)` / `putc()` macro: write one byte to buffer or flush. (REQ: REQ-06-0074)
        - [x] `getchar()` / `putchar()`: wrappers for `stdin`/`stdout`. (REQ: REQ-06-0075)
        - [x] `ungetc(int c, FILE *stream)`: push back one character (at least 1 byte guaranteed). (REQ: REQ-06-0076)
        - [x] `fgets(char *s, int n, FILE *stream)`: read line up to `n-1` chars or newline. (REQ: REQ-06-0077)
        - [x] `fputs(const char *s, FILE *stream)`: write string (no trailing newline). (REQ: REQ-06-0078)
        - [x] `gets_s()` (C11 bounds-checked, optional) — or `gets()` removed per C11. (REQ: REQ-06-0079)
        - [x] `puts(const char *s)`: write string + newline to `stdout`. (REQ: REQ-06-0080)
    - [x] **Block I/O:** (REQ: REQ-06-0081)
        - [x] `fread(void *ptr, size_t size, size_t nmemb, FILE *stream)`: (REQ: REQ-06-0082)
            - [x] Read `size * nmemb` bytes, buffered. (REQ: REQ-06-0083)
            - [x] Handle partial reads, return number of complete elements. (REQ: REQ-06-0084)
            - [x] Set EOF/error indicators on short read. (REQ: REQ-06-0085)
        - [x] `fwrite(const void *ptr, size_t size, size_t nmemb, FILE *stream)`: (REQ: REQ-06-0086)
            - [x] Write `size * nmemb` bytes through buffer. (REQ: REQ-06-0087)
            - [x] Return number of complete elements written. (REQ: REQ-06-0088)
    - [x] **File Positioning:** (REQ: REQ-06-0089)
        - [x] `fseek(FILE *stream, long offset, int whence)`: (REQ: REQ-06-0090)
            - [x] Flush output buffer before seeking. (REQ: REQ-06-0091)
            - [x] Discard input buffer on seek. (REQ: REQ-06-0092)
            - [x] `whence`: `SEEK_SET`, `SEEK_CUR`, `SEEK_END`. (REQ: REQ-06-0093)
            - [x] Return 0 on success, -1 on error. (REQ: REQ-06-0094)
        - [x] `ftell(FILE *stream)`: return current position (accounting for buffer offset). (REQ: REQ-06-0095)
        - [x] `rewind(FILE *stream)`: `fseek(stream, 0, SEEK_SET)` + clear error indicator. (REQ: REQ-06-0096)
        - [x] `fgetpos(FILE *stream, fpos_t *pos)` / `fsetpos(FILE *stream, const fpos_t *pos)`: opaque position save/restore. (REQ: REQ-06-0097)
        - [x] `fseeko()` / `ftello()` (POSIX): `off_t` variants for large file support. (REQ: REQ-06-0098)
    - [x] **Error & EOF Handling:** (REQ: REQ-06-0099)
        - [x] `feof(FILE *stream)`: return non-zero if EOF indicator set. (REQ: REQ-06-0100)
        - [x] `ferror(FILE *stream)`: return non-zero if error indicator set. (REQ: REQ-06-0101)
        - [x] `clearerr(FILE *stream)`: clear both EOF and error indicators. (REQ: REQ-06-0102)
        - [x] `perror(const char *s)`: print `s: strerror(errno)\n` to `stderr`. (REQ: REQ-06-0103)
    - [x] **Temporary Files:** (REQ: REQ-06-0104)
        - [x] `tmpfile()`: create anonymous temporary `FILE` (deleted on close). (REQ: REQ-06-0105)
        - [x] `tmpnam(char *s)` (deprecated) / `mkstemp()` integration. (REQ: REQ-06-0106)
    - [x] **Complete `printf` Family Implementation (User & Kernel):** (REQ: REQ-06-0107)
        - [x] **Kernel:** Migrate `sys/kern/lib.c` simplistic `sprintf` to full implementation. (REQ: REQ-06-0108)
        - [x] **User:** `lib/c` implementation. (REQ: REQ-06-0109)
        - [x] **Flags:** (REQ: REQ-06-0110)
            - [x] `-` (Left-align within field width). (REQ: REQ-06-0111)
            - [x] `+` (Force sign for positive numbers). (REQ: REQ-06-0112)
            - [x] ` ` (Space prefix for positive numbers). (REQ: REQ-06-0113)
            - [x] `#` (Alternate form: 0x for hex, force decimal point). (REQ: REQ-06-0114)
            - [x] `0` (Zero-padding). (REQ: REQ-06-0115)
        - [x] **Width & Precision:** (REQ: REQ-06-0116)
            - [x] Numeric width (e.g., `%5d`). (REQ: REQ-06-0117)
            - [x] Dynamic width `*` (from argument). (REQ: REQ-06-0118)
            - [x] Numeric precision (e.g., `%.5d`). (REQ: REQ-06-0119)
            - [x] Dynamic precision `.*`(from argument). (REQ: REQ-06-0120)
            - [x] Combined width/precision (e.g., `%5.2f`). (REQ: REQ-06-0121)
            - [x] Negative width logic (treat as `-` flag + positive width). (REQ: REQ-06-0122)
        - [x] **Length Modifiers:** (REQ: REQ-06-0123)
            - [x] `hh` (signed/unsigned char). (REQ: REQ-06-0124)
            - [x] `h` (signed/unsigned short). (REQ: REQ-06-0125)
            - [x] `l` (long, wint_t). (REQ: REQ-06-0126)
            - [x] `ll` (long long). (REQ: REQ-06-0127)
            - [x] `j` (intmax_t, uintmax_t). (REQ: REQ-06-0128)
            - [x] `z` (size_t, ssize_t). (REQ: REQ-06-0129)
            - [x] `t` (ptrdiff_t). (REQ: REQ-06-0130)
            - [x] `L` (long double). (REQ: REQ-06-0131)
        - [x] **Conversion Specifiers:** (REQ: REQ-06-0132, REQ-06-0166)
            - [x] **Integers:** `d`, `i` (signed decimal). (REQ: REQ-06-0133)
            - [x] **Unsigned:** `u` (decimal), `o` (octal). (REQ: REQ-06-0134)
            - [x] **Hex:** `x`, `X` (lower/upper case). (REQ: REQ-06-0135)
            - [x] **Floating Point (Basic):** `f`, `F` (decimal notation). (REQ: REQ-06-0136)
            - [x] **Scientific:** `e`, `E` (exponential notation). (REQ: REQ-06-0137)
            - [x] **Significant:** `g`, `G` (shortest representation). (REQ: REQ-06-0138)
            - [x] **Hex Float:** `a`, `A` (C99 hex float - placeholder). (REQ: REQ-06-0139)
            - [x] **Characters:** `c` (char), `lc` (wint_t). (REQ: REQ-06-0140)
            - [x] **Strings:** `s` (char*), `ls` (wchar_t*). (REQ: REQ-06-0141)
            - [x] **Pointer:** `p` (implementation defined, usually %08x or %016x). (REQ: REQ-06-0142)
            - [x] **Count:** `n` (store number of chars written to int*). (REQ: REQ-06-0143)
            - [x] **Literal:** `%` (print percent sign). (REQ: REQ-06-0144)
        - [x] **Complex/Compound Cases:** (REQ: REQ-06-0145)
            - [x] Prefix combinations (e.g., `%20#llx`). (REQ: REQ-06-0146)
            - [x] Leading zeroes with precision (e.g., `%03d` vs `%.3d`). (REQ: REQ-06-0147)
            - [x] "0" flag ignored if "-" is present. (REQ: REQ-06-0148)
            - [x] "0" flag ignored if precision is specified for integers. (REQ: REQ-06-0149)
            - [x] Space ignored if "+" is present. (REQ: REQ-06-0150)
        - [x] **`printf` Family Wrappers:** (REQ: REQ-06-0151)
            - [x] `fprintf(FILE *stream, const char *fmt, ...)`: output to `FILE` via buffer. (REQ: REQ-06-0152)
            - [x] `printf(const char *fmt, ...)`: wrapper for `fprintf(stdout, ...)`. (REQ: REQ-06-0153)
            - [x] `sprintf(char *str, const char *fmt, ...)`: output to string (no bounds check). (REQ: REQ-06-0154)
            - [x] `snprintf(char *str, size_t size, const char *fmt, ...)`: bounded output to string. (REQ: REQ-06-0155)
            - [x] `dprintf(int fd, const char *fmt, ...)` (POSIX): output to fd directly. (REQ: REQ-06-0156)
            - [x] `asprintf(char **ret, const char *fmt, ...)` (BSD extension): auto-allocating sprintf. (REQ: REQ-06-0157)
            - [x] `vfprintf()`, `vprintf()`, `vsprintf()`, `vsnprintf()`, `vdprintf()`, `vasprintf()`: `va_list` variants. (REQ: REQ-06-0158)
    - [x] **Complete `scanf` Family Implementation:** (REQ: REQ-06-0159)
        - [x] **Core `vfscanf()` Engine:** <!-- lib/c/stdio/scanf.c:scan_core --> (REQ: REQ-06-0160)
            - [x] Literal character matching (non-`%` characters, whitespace collapsing). <!-- lib/c/stdio/scanf.c:scan_core --> (REQ: REQ-06-0161)
            - [x] `%%` literal percent matching. <!-- lib/c/stdio/scanf.c:scan_core --> (REQ: REQ-06-0162)
            - [x] Assignment suppression (`*` flag). <!-- lib/c/stdio/scanf.c:scan_core --> (REQ: REQ-06-0163)
            - [x] Maximum field width. <!-- lib/c/stdio/scanf.c:scan_core --> (REQ: REQ-06-0164)
            - [x] **Length Modifiers:** `hh`, `h`, `l`, `ll`, `j`, `z`, `t`, `L`. <!-- lib/c/stdio/scanf.c:scan_core --> (REQ: REQ-06-0165)
            - [x] **Conversion Specifiers:** <!-- lib/c/stdio/scanf.c:scan_core --> (REQ: REQ-06-0132, REQ-06-0166)
                - [x] `d` (decimal integer), `i` (auto-detect base: 0x/0/decimal). <!-- lib/c/stdio/scanf.c:scan_integer_token --> (REQ: REQ-06-0167)
                - [x] `u` (unsigned decimal), `o` (octal), `x`/`X` (hex). <!-- lib/c/stdio/scanf.c:scan_integer_token --> (REQ: REQ-06-0168)
                - [x] `f`, `e`, `g`, `a` (floating-point input parsing). <!-- lib/c/stdio/scanf.c:scan_decimal_float_token, scan_hex_float_token --> (REQ: REQ-06-0169)
                - [x] `c` (character, no whitespace skip), `s` (string, whitespace-delimited). <!-- lib/c/stdio/scanf.c:scan_core --> (REQ: REQ-06-0170)
                - [x] `[` (scanset): character class matching, `^` negation, `]` as first char. <!-- lib/c/stdio/scanf.c:build_scanset --> (REQ: REQ-06-0171)
                - [x] `p` (pointer), `n` (store chars consumed). <!-- lib/c/stdio/scanf.c:scan_core --> (REQ: REQ-06-0172)
            - [x] Return number of successfully assigned items, or `EOF`. <!-- lib/c/stdio/scanf.c:scan_core --> (REQ: REQ-06-0173)
            - [x] Handle input exhaustion mid-conversion. <!-- lib/c/stdio/scanf.c:scan_core --> (REQ: REQ-06-0174)
        - [x] **`scanf` Family Wrappers:** (REQ: REQ-06-0175)
            - [x] `fscanf(FILE *stream, const char *fmt, ...)`. <!-- lib/c/stdio/scanf.c:fscanf --> (REQ: REQ-06-0176)
            - [x] `scanf(const char *fmt, ...)`: wrapper for `fscanf(stdin, ...)`. <!-- lib/c/stdio/scanf.c:scanf --> (REQ: REQ-06-0177)
    ...
    - [x] **Man Pages:** (REQ: REQ-06-0250, REQ-06-0404, REQ-06-0452, REQ-06-0488, REQ-06-0527, REQ-06-0573, REQ-06-0629, REQ-06-0668, REQ-06-0693, REQ-06-0734, REQ-06-0769, REQ-06-0793, REQ-06-0839)
        - [x] `man/man3/fopen.3` — File open. Covers `fopen()`, `fdopen()`, `freopen()`. Mode string parsing, `"x"` flag. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, EXAMPLES, SEE ALSO. <!-- usr.man/man3/fopen.3 --> (REQ: REQ-06-0251)
        - [x] `man/man3/fclose.3` — File close. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, SEE ALSO. <!-- usr.man/man3/fclose.3 --> (REQ: REQ-06-0252)
        - [x] `man/man3/fread.3` — Block I/O. Covers `fread()` and `fwrite()`. SYNOPSIS, DESCRIPTION, RETURN VALUE, EXAMPLES, SEE ALSO. <!-- usr.man/man3/fread.3 --> (REQ: REQ-06-0253)
        - [x] `man/man3/fgets.3` — Line input. Covers `fgets()`, `fputs()`, `gets()` removal note. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, SEE ALSO. <!-- usr.man/man3/fgets.3 --> (REQ: REQ-06-0254)
        - [x] `man/man3/fgetc.3` — Character I/O. Covers `fgetc()`, `getc()`, `getchar()`, `fputc()`, `putc()`, `putchar()`, `ungetc()`. SYNOPSIS, DESCRIPTION, RETURN VALUE, SEE ALSO. <!-- usr.man/man3/fgetc.3 --> (REQ: REQ-06-0255)
        - [x] `man/man3/fseek.3` — File positioning. Covers `fseek()`, `ftell()`, `rewind()`, `fgetpos()`, `fsetpos()`, `fseeko()`, `ftello()`. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, SEE ALSO. <!-- usr.man/man3/fseek.3 --> (REQ: REQ-06-0256)
        - [x] `man/man3/fflush.3` — Flush stream. Document `NULL` argument behavior. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, SEE ALSO. <!-- usr.man/man3/fflush.3 --> (REQ: REQ-06-0257)
        - [x] `man/man3/setvbuf.3` — Set stream buffering. Covers `setvbuf()`, `setbuf()`, `setlinebuf()`. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, EXAMPLES, SEE ALSO. <!-- usr.man/man3/setvbuf.3 --> (REQ: REQ-06-0258)
        - [x] `man/man3/feof.3` — Stream status. Covers `feof()`, `ferror()`, `clearerr()`. SYNOPSIS, DESCRIPTION, RETURN VALUE, SEE ALSO. <!-- usr.man/man3/feof.3 --> (REQ: REQ-06-0259)
        - [x] `man/man3/perror.3` — Print error message. SYNOPSIS, DESCRIPTION, SEE ALSO. <!-- usr.man/man3/perror.3 --> (REQ: REQ-06-0260)
        - [x] `man/man3/tmpfile.3` — Temporary files. Covers `tmpfile()` and `tmpnam()` deprecation note. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, SEE ALSO. <!-- usr.man/man3/tmpfile.3 --> (REQ: REQ-06-0261)
        - [x] `man/man3/printf.3` — Formatted output. Covers `printf()`, `fprintf()`, `sprintf()`, `snprintf()`, `dprintf()`, `asprintf()` and `v*` variants. Full format specification. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, EXAMPLES, SEE ALSO. <!-- usr.man/man3/printf.3 --> (REQ: REQ-06-0262)
        - [x] `man/man3/scanf.3` — Formatted input. Covers `scanf()`, `fscanf()`, `sscanf()` and `v*` variants. Full format specification including scansets. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, EXAMPLES, SEE ALSO. <!-- usr.man/man3/scanf.3 --> (REQ: REQ-06-0263)
- [ ] **String/Mem:** (REQ: REQ-06-0264)
    ...
    - [/] **Math Library (`lib/m/`):**
        - [/] **Architecture & Environment:**
            ...
            - [x] **FENV (Floating-Point Environment):** (REQ: REQ-06-0269)
                - [x] **Header (`include/fenv.h`) Definition (C99/POSIX):** (REQ: REQ-06-0270)
                    - [x] Define `fenv_t` type (full x87 environment: CW, SW, TW, FIP, FCS, FDP, FDS, MXCSR on SSE-capable). (REQ: REQ-06-0271)
                    - [x] Define `fexcept_t` type (exception status word snapshot). (REQ: REQ-06-0272)
                    - [x] Define exception flag macros: (REQ: REQ-06-0273)
                        - [x] `FE_INVALID` (bit 0, x87 IE). (REQ: REQ-06-0274)
                        - [x] `FE_DENORMAL` (bit 1, x87 DE — non-standard extension, guard with `#ifdef`). (REQ: REQ-06-0275)
                        - [x] `FE_DIVBYZERO` (bit 2, x87 ZE). (REQ: REQ-06-0276)
                        - [x] `FE_OVERFLOW` (bit 3, x87 OE). (REQ: REQ-06-0277)
                        - [x] `FE_UNDERFLOW` (bit 4, x87 UE). (REQ: REQ-06-0278)
                        - [x] `FE_INEXACT` (bit 5, x87 PE). (REQ: REQ-06-0279)
                        - [x] `FE_ALL_EXCEPT` (bitwise OR of all supported exception flags). (REQ: REQ-06-0280)
                    - [x] Define rounding mode macros: (REQ: REQ-06-0281)
                        - [x] `FE_TONEAREST` (CW bits 10-11 = 00). (REQ: REQ-06-0282)
                        - [x] `FE_DOWNWARD` (CW bits 10-11 = 01). (REQ: REQ-06-0283)
                        - [x] `FE_UPWARD` (CW bits 10-11 = 10). (REQ: REQ-06-0284)
                        - [x] `FE_TOWARDZERO` (CW bits 10-11 = 11). (REQ: REQ-06-0285)
                    - [x] Define `FE_DFL_ENV` macro (pointer to default environment, `((const fenv_t *)-1)` or static). (REQ: REQ-06-0286)
                    - [x] Provide function prototypes for all `fe*` functions. (REQ: REQ-06-0287)
                    - [x] Include guard (`_FENV_H`) and C++ `extern "C"` wrapper. (REQ: REQ-06-0288)
                - [x] **Exception Handling Functions:** (REQ: REQ-06-0289)
                    - [x] `feclearexcept(int excepts)`: (REQ: REQ-06-0290)
                        - [x] Read x87 status word via `fnstsw`. (REQ: REQ-06-0291, REQ-06-0297, REQ-06-0313)
                        - [x] Clear requested exception bits. (REQ: REQ-06-0292)
                        - [x] Restore via `fldenv` (requires full env save/modify/restore cycle). (REQ: REQ-06-0293)
                        - [x] If SSE present, also clear bits in MXCSR via `ldmxcsr`. (REQ: REQ-06-0290)
                        - [x] Return 0 on success, non-zero on failure. (REQ: REQ-06-0041, REQ-06-0295)
                    - [x] `fegetexceptflag(fexcept_t *flagp, int excepts)`: (REQ: REQ-06-0296)
                        - [x] Read x87 status word via `fnstsw`. (REQ: REQ-06-0291, REQ-06-0297, REQ-06-0313)
                        - [x] Mask with `excepts`, store into `*flagp`. (REQ: REQ-06-0298)
                        - [x] If SSE present, OR in MXCSR exception bits. (REQ: REQ-06-0296)
                        - [x] Return 0 on success. (REQ: REQ-06-0300, REQ-06-0305, REQ-06-0311, REQ-06-0333, REQ-06-0340, REQ-06-0347, REQ-06-0352)
                    - [x] `feraiseexcept(int excepts)`: (REQ: REQ-06-0301)
                        - [x] Raise specified exceptions by performing operations that trigger them (e.g., divide-by-zero for `FE_DIVBYZERO`, 0.0/0.0 for `FE_INVALID`). (REQ: REQ-06-0302)
                        - [x] Alternatively: save env, set SW bits, load env, then `fwait` to trigger trap. (REQ: REQ-06-0303)
                        - [x] Ensure `FE_OVERFLOW` and `FE_UNDERFLOW` also set `FE_INEXACT` per C99 7.6.2.3. (REQ: REQ-06-0304)
                        - [x] Return 0 on success. (REQ: REQ-06-0300, REQ-06-0305, REQ-06-0311, REQ-06-0333, REQ-06-0340, REQ-06-0347, REQ-06-0352)
                    - [x] `fesetexceptflag(const fexcept_t *flagp, int excepts)`: (REQ: REQ-06-0306)
                        - [x] Save full x87 environment via `fnstenv`. (REQ: REQ-06-0307)
                        - [x] Replace status word exception bits (masked by `excepts`) from `*flagp`. (REQ: REQ-06-0308)
                        - [x] Restore via `fldenv` — must NOT raise exceptions (set bits only). (REQ: REQ-06-0309)
                        - [x] If SSE present, update MXCSR accordingly. (REQ: REQ-06-0306)
                        - [x] Return 0 on success. (REQ: REQ-06-0300, REQ-06-0305, REQ-06-0311, REQ-06-0333, REQ-06-0340, REQ-06-0347, REQ-06-0352)
                    - [x] `fetestexcept(int excepts)`: (REQ: REQ-06-0312)
                        - [x] Read x87 status word via `fnstsw`. (REQ: REQ-06-0291, REQ-06-0297, REQ-06-0313)
                        - [x] If SSE present, OR in MXCSR sticky bits. (REQ: REQ-06-0312)
                        - [x] Return bitwise AND of current exceptions with `excepts`. (REQ: REQ-06-0315)
                - [x] **Rounding Mode Functions:** (REQ: REQ-06-0316)
                    - [x] `fegetround()`: (REQ: REQ-06-0317)
                        - [x] Read x87 control word via `fnstcw`. (REQ: REQ-06-0318, REQ-06-0323)
                        - [x] Extract rounding mode bits (bits 10-11). (REQ: REQ-06-0319)
                        - [x] Return corresponding `FE_*` rounding constant. (REQ: REQ-06-0320)
                    - [x] `fesetround(int rdir)`: (REQ: REQ-06-0321)
                        - [x] Validate `rdir` is one of `FE_TONEAREST`, `FE_DOWNWARD`, `FE_UPWARD`, `FE_TOWARDZERO`. (REQ: REQ-06-0322)
                        - [x] Read x87 control word via `fnstcw`. (REQ: REQ-06-0318, REQ-06-0323)
                        - [x] Modify rounding mode bits (bits 10-11). (REQ: REQ-06-0324)
                        - [x] Write back via `fldcw`. (REQ: REQ-06-0325)
                        - [x] If SSE present, also update MXCSR rounding bits (bits 13-14). (REQ: REQ-06-0321)
                        - [x] Return 0 on success, non-zero for invalid `rdir`. (REQ: REQ-06-0327)
                - [x] **Environment Save/Restore Functions:** (REQ: REQ-06-0328)
                    - [x] `fegetenv(fenv_t *envp)`: (REQ: REQ-06-0329)
                        - [x] Save full x87 environment via `fnstenv` into `*envp`. (REQ: REQ-06-0330)
                        - [x] Restore control word afterwards (x87 `fnstenv` masks all exceptions as side effect, so re-load CW). (REQ: REQ-06-0331)
                        - [x] If SSE present, also save MXCSR into `*envp`. (REQ: REQ-06-0329)
                        - [x] Return 0 on success. (REQ: REQ-06-0300, REQ-06-0305, REQ-06-0311, REQ-06-0333, REQ-06-0340, REQ-06-0347, REQ-06-0352)
                    - [x] `feholdexcept(fenv_t *envp)`: (REQ: REQ-06-0334)
                        - [x] Save current environment into `*envp` via `fnstenv`. (REQ: REQ-06-0335)
                        - [x] Clear all exception flags in status word. (REQ: REQ-06-0336)
                        - [x] Mask all exceptions in control word (set exception mask bits 0-5). (REQ: REQ-06-0337)
                        - [x] Install this non-stop environment via `fldenv`. (REQ: REQ-06-0338)
                        - [x] If SSE present, save and mask MXCSR similarly. (REQ: REQ-06-0334)
                        - [x] Return 0 on success. (REQ: REQ-06-0300, REQ-06-0305, REQ-06-0311, REQ-06-0333, REQ-06-0340, REQ-06-0347, REQ-06-0352)
                        - [x] This enables non-stop (non-trapping) FP mode for temporary computation. (REQ: REQ-06-0341)
                    - [x] `fesetenv(const fenv_t *envp)`: (REQ: REQ-06-0342)
                        - [x] If `envp == FE_DFL_ENV`, load default environment (e.g., `fninit` then adjust, or load a static default). (REQ: REQ-06-0342)
                        - [x] Else, load full x87 environment from `*envp` via `fldenv`. (REQ: REQ-06-0344)
                        - [x] If SSE present, also restore MXCSR from `*envp`. (REQ: REQ-06-0342)
                        - [x] Must NOT raise exceptions — install state silently. (REQ: REQ-06-0346)
                        - [x] Return 0 on success. (REQ: REQ-06-0300, REQ-06-0305, REQ-06-0311, REQ-06-0333, REQ-06-0340, REQ-06-0347, REQ-06-0352)
                    - [x] `feupdateenv(const fenv_t *envp)`: (REQ: REQ-06-0348)
                        - [x] Read currently raised exceptions via `fetestexcept(FE_ALL_EXCEPT)`. (REQ: REQ-06-0349)
                        - [x] Install `*envp` (or default) via `fesetenv(envp)`. (REQ: REQ-06-0350)
                        - [x] Re-raise the previously pending exceptions via `feraiseexcept()`. (REQ: REQ-06-0351)
                        - [x] Return 0 on success. (REQ: REQ-06-0300, REQ-06-0305, REQ-06-0311, REQ-06-0333, REQ-06-0340, REQ-06-0347, REQ-06-0352)
                        - [x] This is the "merge exceptions then restore" primitive. (REQ: REQ-06-0353)
                - [x] **`FENV_ACCESS` Pragma Support:** (REQ: REQ-06-0354)
                    - [x] Document compiler interaction: `#pragma STDC FENV_ACCESS ON` advisory. (REQ: REQ-06-0355)
                    - [x] Ensure `fenv.h` includes comment noting GCC `#pragma GCC optimize ("no-fast-math")` equivalent. (REQ: REQ-06-0356)
                    - [x] Add `FENV_ACCESS` documentation in header and man pages. (REQ: REQ-06-0357)
                - [x] **Implementation Details (i386/x87 Backend — `lib/m/src/fenv.c`):** (REQ: REQ-06-0358)
                    - [x] All functions implemented as i386 inline assembly or `.S` file for x87 `fnstcw`/`fldcw`/`fnstsw`/`fnstenv`/`fldenv`/`fnclex`/`fwait`. (REQ: REQ-06-0359)
                    - [x] SSE/MXCSR path gated by runtime or compile-time `__SSE__` check. (REQ: REQ-06-0360)
                    - [x] `fenv_t` layout must match x87 `fnstenv`/`fldenv` 28-byte block (+ optional MXCSR field). (REQ: REQ-06-0361)
                    - [x] Ensure `fninit` is used (not `finit`) to avoid waiting for pending exceptions during default-env setup. (REQ: REQ-06-0362)
                    - [x] All functions are `__attribute__((noinline))` to prevent optimizer from reordering FP state access. (REQ: REQ-06-0363)
                - [x] **Testing:** (REQ: REQ-06-0180, REQ-06-0364, REQ-06-1123, REQ-06-1219)
                    - [x] **Unit Tests (`tests/lib/m/unit/test_fenv.c`):** (REQ: REQ-06-0365)
                        - [x] Test `feclearexcept()`: raise each exception, clear it, verify via `fetestexcept()`. (REQ: REQ-06-0366)
                        - [x] Test `feclearexcept(FE_ALL_EXCEPT)`: clear all, verify none pending. (REQ: REQ-06-0367)
                        - [x] Test `feclearexcept(0)`: no-op, verify no side effects. (REQ: REQ-06-0368)
                        - [x] Test `fegetexceptflag()` / `fesetexceptflag()`: round-trip flag save/restore. (REQ: REQ-06-0369)
                        - [x] Test `fesetexceptflag()` does NOT raise exceptions (just sets sticky bits). (REQ: REQ-06-0370)
                        - [x] Test `feraiseexcept()`: raise individual exceptions, verify via `fetestexcept()`. (REQ: REQ-06-0371)
                        - [x] Test `feraiseexcept(FE_OVERFLOW)` also sets `FE_INEXACT` (C99 requirement). (REQ: REQ-06-0372)
                        - [x] Test `feraiseexcept(FE_UNDERFLOW)` also sets `FE_INEXACT`. (REQ: REQ-06-0373)
                        - [x] Test `feraiseexcept()` with multiple flags ORed together. (REQ: REQ-06-0374)
                        - [x] Test `fetestexcept()`: verify returns only requested bits. (REQ: REQ-06-0375)
                        - [x] Test `fegetround()` default is `FE_TONEAREST`. (REQ: REQ-06-0376)
                        - [x] Test `fesetround()` / `fegetround()` round-trip for all four modes. (REQ: REQ-06-0377)
                        - [x] Test `fesetround()` with invalid argument returns non-zero. (REQ: REQ-06-0378)
                        - [x] Test `fesetround()` actually affects rounding: add 1.0 + tiny value and check direction. (REQ: REQ-06-0379)
                        - [x] Test `fegetenv()` / `fesetenv()` round-trip preserves full state. (REQ: REQ-06-0380)
                        - [x] Test `fesetenv(FE_DFL_ENV)` resets to default (no exceptions, `FE_TONEAREST`). (REQ: REQ-06-0381)
                        - [x] Test `feholdexcept()`: saves state, clears exceptions, masks all traps. (REQ: REQ-06-0382)
                        - [x] Test `feholdexcept()` followed by exception-raising operations don't trap. (REQ: REQ-06-0383)
                        - [x] Test `feupdateenv()`: merges pending exceptions from non-stop region back after restore. (REQ: REQ-06-0384)
                        - [x] Test `feupdateenv()` re-raises exceptions from before `feholdexcept()` was called. (REQ: REQ-06-0385)
                        - [x] Test interaction: `feholdexcept()` → compute with exceptions → `feupdateenv()` → verify merged. (REQ: REQ-06-0386)
                    - [x] **Property Tests (`tests/lib/m/property/prop_fenv.c`):** (REQ: REQ-06-0387)
                        - [x] Property: `fesetround(m); fegetround() == m` for all valid `m`. (REQ: REQ-06-0388)
                        - [x] Property: `fesetround(invalid)` returns non-zero and does not change mode. (REQ: REQ-06-0389)
                        - [x] Property: `feclearexcept(e); fetestexcept(e) == 0` for any valid exception mask `e`. (REQ: REQ-06-0390)
                        - [x] Property: `feraiseexcept(e); (fetestexcept(e) & e) == e` for any valid `e`. (REQ: REQ-06-0391)
                        - [x] Property: `fegetexceptflag(&f, e); feclearexcept(FE_ALL_EXCEPT); fesetexceptflag(&f, e); fetestexcept(e)` equals original. (REQ: REQ-06-0392)
                        - [x] Property: `fegetenv(&env); /* modify state */; fesetenv(&env)` restores original rounding and exceptions. (REQ: REQ-06-0393)
                        - [x] Property: `fesetenv(FE_DFL_ENV); fegetround() == FE_TONEAREST && fetestexcept(FE_ALL_EXCEPT) == 0`. (REQ: REQ-06-0394)
                        - [x] Property: `feholdexcept()` results in `fetestexcept(FE_ALL_EXCEPT) == 0` and non-stop mode. (REQ: REQ-06-0395)
                        - [x] Property: `feupdateenv()` preserves exceptions raised during non-stop region. (REQ: REQ-06-0396)
                        - [x] Property: exception flags are sticky — `feraiseexcept(e)` followed by unrelated FP ops retains `e`. (REQ: REQ-06-0397)
                    - [x] **Fuzz Tests (`tests/lib/m/fuzz/fuzz_fenv.c`):** (REQ: REQ-06-0398)
                        - [x] Fuzz random combinations of exception flag bits to `feclearexcept()` / `feraiseexcept()` / `fetestexcept()`. (REQ: REQ-06-0399)
                        - [x] Fuzz random rounding mode values to `fesetround()` (including invalid values). (REQ: REQ-06-0400)
                        - [x] Fuzz random sequences of `fegetenv()`/`fesetenv()`/`feholdexcept()`/`feupdateenv()` interleaved with FP operations. (REQ: REQ-06-0401)
                        - [x] Fuzz random `fexcept_t` values through `fegetexceptflag()`/`fesetexceptflag()` round-trips. (REQ: REQ-06-0402)
                        - [x] Verify no crashes, no undefined behavior, status word consistency after each fuzzed sequence. (REQ: REQ-06-0403)
                - [x] **Man Pages:** (REQ: REQ-06-0250, REQ-06-0404, REQ-06-0452, REQ-06-0488, REQ-06-0527, REQ-06-0573, REQ-06-0629, REQ-06-0668, REQ-06-0693, REQ-06-0734, REQ-06-0769, REQ-06-0793, REQ-06-0839)
                    - [x] `man/man7/fenv.7` — Overview of floating-point environment, x87/SSE interaction, `FENV_ACCESS` pragma, and usage patterns. (REQ: REQ-06-0405)
                    - [x] `man/man3/feclearexcept.3` — Clear floating-point exception flags. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, EXAMPLES, SEE ALSO. (REQ: REQ-06-0406)
                    - [x] `man/man3/fegetexceptflag.3` — Get floating-point exception flags. Covers `fegetexceptflag()` and `fesetexceptflag()` together (paired save/restore API). SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, EXAMPLES, SEE ALSO. (REQ: REQ-06-0407)
                    - [x] `man/man3/feraiseexcept.3` — Raise floating-point exceptions. Document `FE_OVERFLOW`/`FE_UNDERFLOW` implying `FE_INEXACT`. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, EXAMPLES, SEE ALSO. (REQ: REQ-06-0408)
                    - [x] `man/man3/fetestexcept.3` — Test floating-point exception flags. SYNOPSIS, DESCRIPTION, RETURN VALUE, EXAMPLES, SEE ALSO. (REQ: REQ-06-0409)
                    - [x] `man/man3/fegetround.3` — Get/set floating-point rounding mode. Covers `fegetround()` and `fesetround()` together. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, EXAMPLES, SEE ALSO. (REQ: REQ-06-0410)
                    - [x] `man/man3/fegetenv.3` — Get/set floating-point environment. Covers `fegetenv()` and `fesetenv()`. SYNOPSIS, DESCRIPTION (document `FE_DFL_ENV`), RETURN VALUE, ERRORS, EXAMPLES, SEE ALSO. (REQ: REQ-06-0411)
                    - [x] `man/man3/feholdexcept.3` — Save environment and enter non-stop mode. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, EXAMPLES (show `feholdexcept()` → compute → `feupdateenv()` pattern), SEE ALSO. (REQ: REQ-06-0412)
                    - [x] `man/man3/feupdateenv.3` — Restore environment and re-raise saved exceptions. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, EXAMPLES, SEE ALSO. (REQ: REQ-06-0413)
            - [x] **Type Variants (f, l suffixes):** (REQ: REQ-06-0414)
                - [x] **Float Variants (`f` suffix — `lib/m/src/mathf.c`):** (REQ: REQ-06-0415)
                    - [x] Implement `sinf()`, `cosf()`, `tanf()`, `asinf()`, `acosf()`, `atanf()`, `atan2f()`. (REQ: REQ-06-0416)
                    - [x] Implement `sinhf()`, `coshf()`, `tanhf()`, `asinhf()`, `acoshf()`, `atanhf()`. (REQ: REQ-06-0417)
                    - [x] Implement `expf()`, `exp2f()`, `expm1f()`, `logf()`, `log2f()`, `log10f()`, `log1pf()`. (REQ: REQ-06-0418)
                    - [x] Implement `powf()`, `sqrtf()`, `cbrtf()`, `hypotf()`. (REQ: REQ-06-0419)
                    - [x] Implement `fabsf()`, `fmodf()`, `remainderf()`, `remquof()`, `fmaf()`. (REQ: REQ-06-0420)
                    - [x] Implement `fmaxf()`, `fminf()`, `fdimf()`. (REQ: REQ-06-0421)
                    - [x] Implement `ceilf()`, `floorf()`, `truncf()`, `roundf()`, `rintf()`, `nearbyintf()`. (REQ: REQ-06-0422)
                    - [x] Implement `lroundf()`, `llroundf()`, `lrintf()`, `llrintf()`. (REQ: REQ-06-0423)
                    - [x] Implement `frexpf()`, `ldexpf()`, `modff()`, `scalbnf()`, `scalblnf()`. (REQ: REQ-06-0424)
                    - [x] Implement `ilogbf()`, `logbf()`, `nextafterf()`, `nexttowardf()`, `copysignf()`, `nanf()`. (REQ: REQ-06-0425)
                    - [x] Implement `erff()`, `erfcf()`, `tgammaf()`, `lgammaf()`. (REQ: REQ-06-0426)
                    - [x] Implement `sincosf()`. (REQ: REQ-06-0427)
                - [x] **Long Double Variants (`l` suffix — `lib/m/src/mathl.c`):** (REQ: REQ-06-0428)
                    - [x] Implement `sinl()`, `cosl()`, `tanl()`, `asinl()`, `acosl()`, `atanl()`, `atan2l()`. (REQ: REQ-06-0429)
                    - [x] Implement `sinhl()`, `coshl()`, `tanhl()`, `asinhl()`, `acoshl()`, `atanhl()`. (REQ: REQ-06-0430)
                    - [x] Implement `expl()`, `exp2l()`, `expm1l()`, `logl()`, `log2l()`, `log10l()`, `log1pl()`. (REQ: REQ-06-0431)
                    - [x] Implement `powl()`, `sqrtl()`, `cbrtl()`, `hypotl()`. (REQ: REQ-06-0432)
                    - [x] Implement `fabsl()`, `fmodl()`, `remainderl()`, `remquol()`, `fmal()`. (REQ: REQ-06-0433)
                    - [x] Implement `fmaxl()`, `fminl()`, `fdiml()`. (REQ: REQ-06-0434)
                    - [x] Implement `ceill()`, `floorl()`, `truncl()`, `roundl()`, `rintl()`, `nearbyintl()`. (REQ: REQ-06-0435)
                    - [x] Implement `lroundl()`, `llroundl()`, `lrintl()`, `llrintl()`. (REQ: REQ-06-0436)
                    - [x] Implement `frexpl()`, `ldexpl()`, `modfl()`, `scalbnl()`, `scalblnl()`. (REQ: REQ-06-0437)
                    - [x] Implement `ilogbl()`, `logbl()`, `nextafterl()`, `nexttowardl()`, `copysignl()`, `nanl()`. (REQ: REQ-06-0438)
                    - [x] Implement `erfl()`, `erfcl()`, `tgammal()`, `lgammal()`. (REQ: REQ-06-0439)
                    - [x] Implement `sincosl()`. (REQ: REQ-06-0440)
                - [x] **i386 Note:** On x87, `long double` is 80-bit extended precision (native FPU format). `float`/`double` variants should use x87 internally and truncate on return. (REQ: REQ-06-0441)
            - [x] **Generic Math (`<tgmath.h>`):** (REQ: REQ-06-0442)
                - [x] Implement C99/C11 `<tgmath.h>` type-generic macros: (REQ: REQ-06-0443)
                    - [x] Dispatch to `f`, ` `, or `l` suffix based on argument type via `_Generic` (C11) or `__builtin_choose_expr` + `__builtin_types_compatible_p` (GCC extension). (REQ: REQ-06-0444)
                    - [x] Cover all math functions with type-generic wrappers: trig, hyperbolic, exp/log, pow/sqrt, rounding, manipulation, fenv-interacting. (REQ: REQ-06-0445)
                    - [x] Include complex variants dispatch (if/when `<complex.h>` is implemented). (REQ: REQ-06-0446)
                    - [x] Ensure macro expansion does not evaluate arguments multiple times (use statement expressions if needed). (REQ: REQ-06-0447)
                - [x] **Testing (`tests/lib/m/unit/test_tgmath.c`):** (REQ: REQ-06-0448)
                    - [x] Test type dispatch: `sin((float)x)` calls `sinf()`, `sin((double)x)` calls `sin()`, `sin((long double)x)` calls `sinl()`. (REQ: REQ-06-0449)
                    - [x] Test all covered function families dispatch correctly. (REQ: REQ-06-0450)
                    - [x] Test no double-evaluation of arguments with side effects. (REQ: REQ-06-0451)
                - [x] **Man Pages:** (REQ: REQ-06-0250, REQ-06-0404, REQ-06-0452, REQ-06-0488, REQ-06-0527, REQ-06-0573, REQ-06-0629, REQ-06-0668, REQ-06-0693, REQ-06-0734, REQ-06-0769, REQ-06-0793, REQ-06-0839)
        ...
        - [x] **Classification & Comparison (C99/C23/POSIX):**
            - [x] `fpclassify()` (REQ: REQ-06-0454)
            - [x] `isfinite()` (REQ: REQ-06-0455)
            - [x] `isinf()` (REQ: REQ-06-0456)
            - [x] `isnan()` (REQ: REQ-06-0457)
            - [x] `isnormal()` (REQ: REQ-06-0458)
            - [x] `signbit()` (REQ: REQ-06-0459)
            - [x] `isgreater()`, `isgreaterequal()`, `isless()`, `islessequal()`, `islessgreater()`, `isunordered()`: (REQ: REQ-06-0460)
                - [x] Implement as macros using `__builtin_isgreater` etc. (GCC) or manual quiet-NaN-safe comparisons. (REQ: REQ-06-0461)
                - [x] Must not raise `FE_INVALID` on unordered operands (unlike plain `<`/`>`). (REQ: REQ-06-0462)
            - [x] `iseqsig()`, `issignaling()` (C23): (REQ: REQ-06-0463)
                - [x] `iseqsig()`: equality comparison that DOES raise `FE_INVALID` on NaN. (REQ: REQ-06-0464)
                - [x] `issignaling()`: detect signaling NaN via bit pattern inspection. (REQ: REQ-06-0465)
            - [x] `iscanonical()`, `issubnormal()`, `iszero()` (C23): (REQ: REQ-06-0466)
                - [x] `iscanonical()`: always 1 for IEEE 754 binary formats (all values canonical). (REQ: REQ-06-0467)
                - [x] `issubnormal()`: `fpclassify(x) == FP_SUBNORMAL`. (REQ: REQ-06-0468)
                - [x] `iszero()`: `fpclassify(x) == FP_ZERO`. (REQ: REQ-06-0469)
            - [x] **Testing (`tests/lib/m/unit/test_classify.c`):** (REQ: REQ-06-0470)
                - [x] Test `fpclassify()` returns correct category for: `+0.0`, `-0.0`, `1.0`, `-1.0`, `INFINITY`, `-INFINITY`, `NAN`, `DBL_MIN/2` (denorm), `DBL_MAX`. (REQ: REQ-06-0471)
                - [x] Test `isfinite()` true for normals/denormals/zeros, false for inf/nan. (REQ: REQ-06-0472)
                - [x] Test `isinf()` true for `±INFINITY` only. (REQ: REQ-06-0473)
                - [x] Test `isnan()` true for `NAN`, quiet NaN, signaling NaN. (REQ: REQ-06-0474)
                - [x] Test `isnormal()` false for zero, denormal, inf, nan. (REQ: REQ-06-0475)
                - [x] Test `signbit()` for `+0.0`, `-0.0`, `+1.0`, `-1.0`, `+INFINITY`, `-INFINITY`, `NAN`, `-NAN`. (REQ: REQ-06-0476)
                - [x] Test `isgreater()` etc. do NOT raise `FE_INVALID` when one operand is NaN. (REQ: REQ-06-0477)
                - [x] Test `isunordered()` true iff either operand is NaN. (REQ: REQ-06-0478)
                - [x] Test `iseqsig()` raises `FE_INVALID` on NaN operand. (REQ: REQ-06-0479)
                - [x] Test `issignaling()` detects sNaN bit pattern. (REQ: REQ-06-0480)
                - [x] Test float, double, and long double variants. (REQ: REQ-06-0481)
            - [x] **Property Tests (`tests/lib/m/property/prop_classify.c`):** (REQ: REQ-06-0482)
                - [x] Property: exactly one of `isinf(x)`, `isnan(x)`, `isfinite(x)` is true for any `x`. (REQ: REQ-06-0483)
                - [x] Property: `isnormal(x)` implies `isfinite(x)`. (REQ: REQ-06-0484)
                - [x] Property: `issubnormal(x)` implies `isfinite(x) && !isnormal(x) && !iszero(x)`. (REQ: REQ-06-0485)
                - [x] Property: `signbit(-x) != signbit(x)` for all non-NaN `x`. (REQ: REQ-06-0486)
                - [x] Property: `isunordered(x, y)` iff `isnan(x) || isnan(y)`. (REQ: REQ-06-0487)
            - [x] **Man Pages:** (REQ: REQ-06-0250, REQ-06-0404, REQ-06-0452, REQ-06-0488, REQ-06-0527, REQ-06-0573, REQ-06-0629, REQ-06-0668, REQ-06-0693, REQ-06-0734, REQ-06-0769, REQ-06-0793, REQ-06-0839)
                - [x] `man/man3/fpclassify.3` — Floating-point classification. Covers `fpclassify()`, `isfinite()`, `isinf()`, `isnan()`, `isnormal()`, `signbit()`. SYNOPSIS, DESCRIPTION, RETURN VALUE, EXAMPLES, SEE ALSO. (REQ: REQ-06-0489)
                - [x] `man/man3/isgreater.3` — Quiet comparison macros. Covers `isgreater()` through `isunordered()`. Document non-signaling NaN behavior. SYNOPSIS, DESCRIPTION, RETURN VALUE, SEE ALSO. (REQ: REQ-06-0490)
        - [x] **Basic Arithmetic:**
            - [x] `fabs()` (REQ: REQ-06-0491)
            - [x] `fmod()` (REQ: REQ-06-0492)
            - [x] `remainder()` (REQ: REQ-06-0493)
            - [x] `remquo()`: (REQ: REQ-06-0494)
                - [x] Compute remainder AND store low-order bits of quotient in `*quo`. (REQ: REQ-06-0495)
                - [x] Sign of `*quo` is sign of `x/y`. (REQ: REQ-06-0496)
                - [x] At least 3 bits of quotient stored. (REQ: REQ-06-0497)
            - [x] `fmax()` (REQ: REQ-06-0498)
            - [x] `fmin()` (REQ: REQ-06-0499)
            - [x] `fdim()` (REQ: REQ-06-0500)
            - [x] `fma()` (Fused Multiply-Add): (REQ: REQ-06-0501)
                - [x] Compute `(x * y) + z` with single rounding (as if infinite precision intermediate). (REQ: REQ-06-0502)
                - [x] Critical for numerical accuracy in dot-products, compensated summation. (REQ: REQ-06-0503)
                - [x] On x87: no hardware FMA — use software double-double decomposition or Dekker's algorithm. (REQ: REQ-06-0501)
                - [x] Raise `FE_INEXACT`/`FE_OVERFLOW`/`FE_UNDERFLOW` appropriately. (REQ: REQ-06-0505)
            - [x] `fmaximum()`, `fminimum()`, `fmaximum_num()`, `fminimum_num()`, `fmaximum_mag()`, `fminimum_mag()` (C23): (REQ: REQ-06-0506)
                - [x] `fmaximum()`/`fminimum()`: NaN-propagating (NaN if either operand is NaN). (REQ: REQ-06-0507)
                - [x] `fmaximum_num()`/`fminimum_num()`: NaN-ignoring (return the non-NaN operand). (REQ: REQ-06-0508)
                - [x] `fmaximum_mag()`/`fminimum_mag()`: Compare absolute values, NaN-propagating. (REQ: REQ-06-0509)
                - [x] All distinguish `+0.0` from `-0.0`: `fmaximum(+0.0, -0.0) == +0.0`. (REQ: REQ-06-0510)
            - [x] **Testing (`tests/lib/m/unit/test_arith.c`):** (REQ: REQ-06-0511)
                - [x] Test `fabs()`: positive, negative, zero, -0.0, ∞, -∞, NaN. (REQ: REQ-06-0512)
                - [x] Test `fmod()`: basic cases, sign of result matches dividend, zero dividend, NaN propagation. (REQ: REQ-06-0513)
                - [x] Test `remainder()`: IEEE remainder (can be negative), ties to even quotient. (REQ: REQ-06-0514)
                - [x] Test `remquo()`: remainder matches `remainder()`, quotient low bits correct. (REQ: REQ-06-0515)
                - [x] Test `fma()`: `fma(a, b, c)` vs naive `a*b+c` — detect cases where single rounding differs. (REQ: REQ-06-0516)
                - [x] Test `fma()` special values: 0×∞+NaN, ∞×finite±∞. (REQ: REQ-06-0517)
                - [x] Test `fmax()`/`fmin()`: NaN handling (return non-NaN), ±0.0 distinction. (REQ: REQ-06-0518)
                - [x] Test `fdim()`: positive difference, zero when x ≤ y, NaN propagation. (REQ: REQ-06-0519)
                - [x] Test C23 `fmaximum()`/`fminimum()` NaN propagation vs `fmax()`/`fmin()` NaN-ignoring. (REQ: REQ-06-0520)
            - [x] **Property Tests (`tests/lib/m/property/prop_arith.c`):** (REQ: REQ-06-0521)
                - [x] Property: `fabs(x) >= 0` for all `x` (including `-0.0` → `+0.0`). (REQ: REQ-06-0522)
                - [x] Property: `fmod(x, y)` has same sign as `x` and `|fmod(x,y)| < |y|`. (REQ: REQ-06-0523)
                - [x] Property: `fma(x, y, 0.0) == x * y` for exact products (no rounding needed). (REQ: REQ-06-0524)
                - [x] Property: `fmax(x, y) >= x && fmax(x, y) >= y` for non-NaN inputs. (REQ: REQ-06-0525)
                - [x] Property: `fdim(x, y) + y >= x` for finite non-NaN inputs. (REQ: REQ-06-0526)
            - [x] **Man Pages:** (REQ: REQ-06-0250, REQ-06-0404, REQ-06-0452, REQ-06-0488, REQ-06-0527, REQ-06-0573, REQ-06-0629, REQ-06-0668, REQ-06-0693, REQ-06-0734, REQ-06-0769, REQ-06-0793, REQ-06-0839)
                - [x] `man/man3/fabs.3` — Absolute value. SYNOPSIS, DESCRIPTION, RETURN VALUE, SEE ALSO. (REQ: REQ-06-0528)
                - [x] `man/man3/fmod.3` — Floating-point remainder. Covers `fmod()` and `remainder()` with IEEE semantics differences. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, EXAMPLES, SEE ALSO. (REQ: REQ-06-0529)
                - [x] `man/man3/remquo.3` — Remainder with quotient. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, EXAMPLES, SEE ALSO. (REQ: REQ-06-0530)
                - [x] `man/man3/fma.3` — Fused multiply-add. Document precision advantage. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, EXAMPLES, SEE ALSO. (REQ: REQ-06-0531)
                - [x] `man/man3/fmax.3` — Maximum/minimum. Covers `fmax()`, `fmin()`, `fdim()`. SYNOPSIS, DESCRIPTION, RETURN VALUE, EXAMPLES, SEE ALSO. (REQ: REQ-06-0532)
        - [x] **Rounding:** (REQ: REQ-06-0533, REQ-06-0551, REQ-06-0555, REQ-06-0566)
            - [x] `ceil()` (REQ: REQ-06-0533)
            - [x] `floor()` (REQ: REQ-06-0534)
            - [x] `trunc()` (REQ: REQ-06-0535)
            - [x] `round()` (REQ: REQ-06-0536)
            - [x] `rint()` (REQ: REQ-06-0537)
            - [x] `nearbyint()` (REQ: REQ-06-0538)
            - [x] `lround()`, `llround()` (REQ: REQ-06-0541)
            - [x] `lrint()`, `llrint()` (REQ: REQ-06-0544)
            - [x] `roundeven()` (C23): (REQ: REQ-06-0548)
                - [x] Round to nearest, ties to even (same as default `FE_TONEAREST` `rint()`). (REQ: REQ-06-0549)
                - [x] Explicit function — does not depend on current rounding mode. (REQ: REQ-06-0550)
            - [x] `fromfp()`, `fromfpx()`, `ufromfp()`, `ufromfpx()` (C23): (REQ: REQ-06-0551)
                - [x] Convert to integer type with explicit rounding mode and width. (REQ: REQ-06-0552)
                - [x] `fromfp()`: signed, `ufromfp()`: unsigned. (REQ: REQ-06-0553)
                - [x] `*x` variants raise `FE_INEXACT` on non-integer input; non-`x` variants do not. (REQ: REQ-06-0554)
            - [x] **Unit Tests (`tests/lib/m/unit/test_round.c`):** (REQ: REQ-06-0555)
                - [x] Test `ceil()`, `floor()`, `trunc()`, `round()`, `roundeven()`. (REQ: REQ-06-0556)
                - [x] Test `rint()`, `nearbyint()`, `lround()`, `llround()`, `lrint()`, `llrint()`. (REQ: REQ-06-0561)
                - [x] Test fromfp family: `fromfp()`, `fromfpf()`, `fromfpl()` (REQ: REQ-06-0553)
                - [x] Test ufromfp family: `ufromfp()`, `ufromfpf()`, `ufromfpul()` (REQ: REQ-06-0553)
                - [x] Test edge cases: ±0.0, ±∞, NaN, ±0.5, ±1.5, large values. (REQ: REQ-06-0565)
            - [x] **Property Tests (`tests/lib/m/property/prop_round.c`):** (REQ: REQ-06-0566)
                - [x] Property: `ceil(x) >= x` for all finite `x`. (REQ: REQ-06-0567)
                - [x] Property: `floor(x) <= x` for all finite `x`. (REQ: REQ-06-0568)
                - [x] Property: `|trunc(x)| <= |x|` for all finite `x`. (REQ: REQ-06-0569)
                - [x] Property: `floor(x) <= round(x) <= ceil(x)` for all `x` where the three are defined. (REQ: REQ-06-0570)
                - [x] Property: `rint(x) == x` for integer values of `x`. (REQ: REQ-06-0571)
                - [x] Property: rounding functions are idempotent. (REQ: REQ-06-0572)
                - [x] Property: fromfp rounding mode handling. (REQ: REQ-06-0554)
            - [x] **Man Pages:** (REQ: REQ-06-0250, REQ-06-0404, REQ-06-0452, REQ-06-0488, REQ-06-0527, REQ-06-0573, REQ-06-0629, REQ-06-0668, REQ-06-0693, REQ-06-0734, REQ-06-0769, REQ-06-0793, REQ-06-0839)
                - [x] `usr.man/man3/ceil.3` — Round upward. Covers `ceil()`, `ceilf()`, `ceill()`. SYNOPSIS, DESCRIPTION, RETURN VALUE, SEE ALSO. (REQ: REQ-06-0574)
                - [x] `usr.man/man3/floor.3` — Round downward. SYNOPSIS, DESCRIPTION, RETURN VALUE, SEE ALSO. (REQ: REQ-06-0575)
                - [x] `usr.man/man3/round.3` — Round to nearest, ties away from zero. Covers `round()`, `lround()`, `llround()`. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, SEE ALSO. (REQ: REQ-06-0576)
                - [x] `usr.man/man3/rint.3` — Round to nearest integer using current mode. Covers `rint()`, `nearbyint()`, `lrint()`, `llrint()`. Document `FE_INEXACT` difference. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, SEE ALSO. (REQ: REQ-06-0577)
                - [x] `usr.man/man3/trunc.3` — Round toward zero. SYNOPSIS, DESCRIPTION, RETURN VALUE, SEE ALSO. (REQ: REQ-06-0578)
        - [x] **Exponential, Logarithmic & Power:**
            - [x] `exp()` <!-- lib/m/src/math_exp.c:exp --> (REQ: REQ-06-0579)
            - [x] `exp2()` <!-- lib/m/src/math_exp.c:exp2 --> (REQ: REQ-06-0580)
            - [x] `expm1()` <!-- lib/m/src/math_exp.c:expm1 --> (REQ: REQ-06-0581)
            - [x] `exp10()`, `exp10m1()`, `exp2m1()` (C23): (REQ: REQ-06-0582)
                - [x] `exp10(x)`: compute `10^x`. On x87: `x * log2(10)` → `f2xm1`/`fscale`. (REQ: REQ-06-0583)
                - [x] `exp10m1(x)`: compute `10^x - 1` accurately for small `x`. (REQ: REQ-06-0584)
                - [x] `exp2m1(x)`: compute `2^x - 1` accurately for small `x` (direct `f2xm1` for |x| < 1). (REQ: REQ-06-0585)
            - [x] `log()` <!-- lib/m/src/math_exp.c:log --> (REQ: REQ-06-0586)
            - [x] `log2()` <!-- lib/m/src/math_exp.c:log2 --> (REQ: REQ-06-0587)
            - [x] `log10()` <!-- lib/m/src/math_exp.c:log10 --> (REQ: REQ-06-0588)
            - [x] `log1p()` <!-- lib/m/src/math_exp.c:log1p --> (REQ: REQ-06-0589)
            - [x] `log10p1()`, `log2p1()`, `logp1()` (C23): (REQ: REQ-06-0590)
                - [x] `logp1(x)`: alias for `log1p(x)` — compute `ln(1+x)` accurately for small `x`. (REQ: REQ-06-0591)
                - [x] `log2p1(x)`: compute `log2(1+x)` accurately. On x87: use `fyl2xp1` for |x| < 1-√2/2. (REQ: REQ-06-0592)
                - [x] `log10p1(x)`: compute `log10(1+x)` accurately. (REQ: REQ-06-0593)
            - [x] `pow()` <!-- lib/m/src/math_exp.c:pow --> (REQ: REQ-06-0594)
            - [x] `pown()`, `powr()`, `rootn()`, `compound()` (C23): (REQ: REQ-06-0595)
                - [x] `pown(x, n)`: `x` raised to integer power `n` (`intmax_t`). Binary exponentiation. (REQ: REQ-06-0596)
                - [x] `powr(x, y)`: `e^(y * ln(x))`, domain x ≥ 0. Different NaN/±0 semantics from `pow()`. (REQ: REQ-06-0597)
                - [x] `rootn(x, n)`: n-th root of `x`. `rootn(x, 2) == sqrt(x)`, `rootn(x, 3) == cbrt(x)`. (REQ: REQ-06-0598)
                - [x] `compound(x, n)`: `(1+x)^n` computed stably for small `x`. (REQ: REQ-06-0599)
            - [x] `sqrt()` <!-- lib/m/src/math_exp.c:sqrt --> (REQ: REQ-06-0600)
            - [x] `rsqrt()` <!-- lib/m/src/math_exp.c:rsqrt --> (REQ: REQ-06-0601)
                - [x] Reciprocal square root: `1/sqrt(x)`. (REQ: REQ-06-0602)
                - [x] On x87: `fsqrt` then `fdivr` with 1.0. (REQ: REQ-06-0601)
            - [x] `cbrt()` <!-- lib/m/src/math_exp.c:cbrt --> (REQ: REQ-06-0604)
            - [x] `hypot()` <!-- lib/m/src/math_exp.c:hypot --> (REQ: REQ-06-0605)
            - [x] **Testing (`tests/lib/m/unit/test_explog.c`):** (REQ: REQ-06-0606)
                - [x] Test `exp(0)==1`, `exp(1)≈M_E`, `exp(-∞)==0`, `exp(+∞)==+∞`, `exp(NaN)==NaN`. (REQ: REQ-06-0607)
                - [x] Test `exp2(0)==1`, `exp2(10)==1024`. (REQ: REQ-06-0608)
                - [x] Test `expm1(0)==0`, `expm1(tiny)≈tiny` (accuracy for small x vs naive `exp(x)-1`). (REQ: REQ-06-0609)
                - [x] Test `log(1)==0`, `log(M_E)≈1`, `log(0)==-∞`, `log(-1)==NaN` + `FE_INVALID`, `log(+∞)==+∞`. (REQ: REQ-06-0610)
                - [x] Test `log2(1)==0`, `log2(1024)==10`. (REQ: REQ-06-0611)
                - [x] Test `log10(1)==0`, `log10(1000)==3`. (REQ: REQ-06-0612)
                - [x] Test `log1p(0)==0`, `log1p(tiny)≈tiny` (accuracy for small x). (REQ: REQ-06-0613)
                - [x] Test `pow(2,10)==1024`, `pow(-1,2)==1`, `pow(0,0)==1` (C99 convention). (REQ: REQ-06-0614)
                - [x] Test `pow(x, 0)==1` for all x (including ∞, NaN per C99 F.9.4.4). (REQ: REQ-06-0615)
                - [x] Test `sqrt(4)==2`, `sqrt(0)==0`, `sqrt(-1)==NaN`, `sqrt(+∞)==+∞`. (REQ: REQ-06-0616)
                - [x] Test `cbrt(-8)==-2`, `cbrt(0)==0`. (REQ: REQ-06-0617)
                - [x] Test `hypot(3,4)==5`, `hypot(x,0)==fabs(x)`, `hypot(∞,NaN)==∞`. (REQ: REQ-06-0618)
                - [x] Test `rsqrt(4)==0.5`, `rsqrt(0)==+∞`, `rsqrt(-1)==NaN`. (REQ: REQ-06-0619)
            - [x] **Property Tests (`tests/lib/m/property/prop_explog.c`):** (REQ: REQ-06-0620)
                - [x] Property: `exp(log(x)) ≈ x` for positive finite `x` (within ULP tolerance). (REQ: REQ-06-0621)
                - [x] Property: `log(exp(x)) ≈ x` for moderate `x` (no overflow/underflow). (REQ: REQ-06-0622)
                - [x] Property: `exp2(log2(x)) ≈ x` for positive finite `x`. (REQ: REQ-06-0623)
                - [x] Property: `sqrt(x) * sqrt(x) ≈ x` for positive `x`. (REQ: REQ-06-0624)
                - [x] Property: `hypot(x,y) >= fabs(x) && hypot(x,y) >= fabs(y)`. (REQ: REQ-06-0625)
                - [x] Property: `pow(x, 1.0) == x` for all `x`. (REQ: REQ-06-0626)
                - [x] Property: `expm1(x) + 1 ≈ exp(x)` for all `x` (accuracy comparison, not equality). (REQ: REQ-06-0627)
                - [x] Property: `log1p(expm1(x)) ≈ x` for moderate `x`. (REQ: REQ-06-0628)
            - [x] **Man Pages:** (REQ: REQ-06-0250, REQ-06-0404, REQ-06-0452, REQ-06-0488, REQ-06-0527, REQ-06-0573, REQ-06-0629, REQ-06-0668, REQ-06-0693, REQ-06-0734, REQ-06-0769, REQ-06-0793, REQ-06-0839)
                - [x] `man/man3/exp.3` <!-- usr.man/man3/exp.3 --> — Exponential functions. Covers `exp()`, `exp2()`, `expm1()`. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, EXAMPLES, SEE ALSO. (REQ: REQ-06-0630)
                - [x] `man/man3/log.3` <!-- usr.man/man3/log.3 --> — Logarithmic functions. Covers `log()`, `log2()`, `log10()`, `log1p()`. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, EXAMPLES, SEE ALSO. (REQ: REQ-06-0631)
                - [x] `man/man3/pow.3` <!-- usr.man/man3/pow.3 --> — Power functions. Covers `pow()`, `sqrt()`, `cbrt()`, `hypot()`. Document special value semantics extensively (C99 Annex F). SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, EXAMPLES, SEE ALSO. (REQ: REQ-06-0632)
        - [/] **Trigonometric:**
            ...
            - [x] `sinpi()`, `cospi()`, `tanpi()` (C23): <!-- lib/m/src/math.c --> (REQ: REQ-06-0640)
                ...
                - [x] Key benefit: exact values without `M_PI` multiplication error. <!-- mod-2 reduction in sinpi/cospi/tanpi avoids forming M_PI*x for integer/half-integer args --> (REQ: REQ-06-0644)
            - [x] `asinpi()`, `acospi()`, `atanpi()`, `atan2pi()` (C23): <!-- lib/m/src/math.c --> (REQ: REQ-06-0645)
            ...
            - [x] **Property Tests (`tests/lib/m/property/prop_trig.c`):** <!-- tests/lib/m/property/prop_trig.c --> (REQ: REQ-06-0661)
        ...
        - [ ] **Hyperbolic:** (REQ: REQ-06-0671)
            ...
            - [x] **Testing (`tests/lib/m/unit/test_hyper.c`):** <!-- tests/lib/m/unit/test_hyper.c --> (REQ: REQ-06-0678)
            ...
            - [x] **Property Tests (`tests/lib/m/property/prop_hyper.c`):** <!-- tests/lib/m/property/prop_hyper.c --> (REQ: REQ-06-0686)
                ...
                - [x] Property: `tanh(x) == sinh(x)/cosh(x)` for moderate `x`. <!-- tests/lib/m/property/prop_hyper.c:prop_tanh_definition --> (REQ: REQ-06-0690)
                - [x] Property: `asinh(sinh(x)) ≈ x` for all `x`. <!-- tests/lib/m/property/prop_hyper.c:prop_asinh_sinh_inverse --> (REQ: REQ-06-0691)
        ...
        - [/] **Manipulation:**
            ...
            - [x] `ilogb()`, `logb()`: <!-- lib/m/src/math.c --> (REQ: REQ-06-0702)
                ...
                - [x] `logb(x)`: extract unbiased exponent as `double`. `logb(0)` → `-∞` (pole), `logb(±∞)` → `+∞`. <!-- lib/m/src/math.c:logb --> (REQ: REQ-06-0704)
            - [x] `nextafter()` <!-- lib/m/src/math.c:nextafter --> (REQ: REQ-06-0705)
            ...
            - [x] `nan()`: <!-- lib/m/src/math.c:nan --> (REQ: REQ-06-0712)
                - [x] `nan(tagp)`: convert string tag to quiet NaN. `nan("") == NAN`, `nan("123")` → NaN with tag payload. <!-- lib/m/src/math.c:nan --> (REQ: REQ-06-0713)
            - [x] **Testing (`tests/lib/m/unit/test_manip.c`):** <!-- tests/lib/m/unit/test_manip.c --> (REQ: REQ-06-0714)
                ...
                - [x] Test `ilogb()`: `ilogb(1.0)==0`, `ilogb(2.0)==1`, `ilogb(0.5)==-1`. (REQ: REQ-06-0720)
            ...
            - [x] **Property Tests (`tests/lib/m/property/prop_manip.c`):** (REQ: REQ-06-0728)
                ...
                - [x] Property: `scalbn(x, 0) == x`. <!-- tests/lib/m/property/prop_manip.c:prop_scalbn_zero_exp --> (REQ: REQ-06-0733)
            - [ ] **Man Pages:** (REQ: REQ-06-0250, REQ-06-0404, REQ-06-0452, REQ-06-0488, REQ-06-0527, REQ-06-0573, REQ-06-0629, REQ-06-0668, REQ-06-0693, REQ-06-0734, REQ-06-0769, REQ-06-0793, REQ-06-0839)
                ...
                - [x] `man/man3/nextafter.3` — Next representable value. Covers `nextafter()`, `nexttoward()`, `nextup()`, `nextdown()`. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, SEE ALSO. <!-- usr.man/man3/nextafter.3 --> (REQ: REQ-06-0739)
                - [x] `man/man3/copysign.3` — Copy sign of a number. SYNOPSIS, DESCRIPTION, RETURN VALUE, SEE ALSO. <!-- usr.man/man3/copysign.3 --> (REQ: REQ-06-0740)
                - [x] `man/man3/nan.3` — Generate quiet NaN. SYNOPSIS, DESCRIPTION, RETURN VALUE, EXAMPLES, SEE ALSO. <!-- usr.man/man3/nan.3 --> (REQ: REQ-06-0741)
        - [ ] **Error & Gamma Functions (POSIX/C11):** (REQ: REQ-06-0742)
            - [x] `erf()`, `erfc()`: <!-- lib/m/src/math.c --> (REQ: REQ-06-0743)
            ...
            - [x] `tgamma()`, `lgamma()` (with `signgam`): <!-- lib/m/src/math.c --> (REQ: REQ-06-0748)
                - [x] `tgamma(x)`: true Gamma function, `Γ(x)`. <!-- lib/m/src/math.c:tgamma --> (REQ: REQ-06-0749)
                - [x] `tgamma(n) == (n-1)!` for positive integers. <!-- lib/m/src/math.c:tgamma — verified tgamma(5)==24 --> (REQ: REQ-06-0750)
                - [x] Poles at non-positive integers: `tgamma(0)` → ±∞, `tgamma(-1)` → NaN + `FE_INVALID`. <!-- lib/m/src/math.c:tgamma raises FE_DIVBYZERO at 0 and FE_INVALID at negative integers --> (REQ: REQ-06-0751)
                - [x] `lgamma(x)`: natural log of absolute value of Gamma: `ln|Γ(x)|`. <!-- lib/m/src/math.c:lgamma --> (REQ: REQ-06-0752)
                - [x] Set external `signgam` to sign of `Γ(x)` (+1 or -1). <!-- lib/m/src/math.c:lgamma --> (REQ: REQ-06-0753)
                - [x] `lgamma_r(x, &signp)`: reentrant variant (BSD extension). <!-- lib/m/src/math.c:lgamma_r --> (REQ: REQ-06-0754)
                - [x] Implementation: Stirling's approximation for large x, Lanczos or rational approximation for small x. <!-- lib/m/src/math.c:lgamma_r — Lanczos via tgamma for x<170, Stirling for larger --> (REQ: REQ-06-0755)
            - [x] **Testing (`tests/lib/m/unit/test_gamma.c`):** <!-- tests/lib/m/unit/test_gamma.c --> (REQ: REQ-06-0756)
                - [x] Test `erf(0)==0`, `erf(1)≈0.8427`, `erf(+∞)==1`, `erf(-∞)==-1`. <!-- tests/lib/m/unit/test_gamma.c:test_erf_basic --> (REQ: REQ-06-0757)
            ...
            - [x] **Property Tests (`tests/lib/m/property/prop_gamma.c`):** <!-- tests/lib/m/property/prop_gamma.c --> (REQ: REQ-06-0764)
                - [x] Property: `erfc(x) + erf(x) ≈ 1.0` for all finite `x`. <!-- tests/lib/m/property/prop_gamma.c:prop_erf_erfc_complement --> (REQ: REQ-06-0765)
                - [x] Property: `erf(-x) == -erf(x)` (odd function). <!-- tests/lib/m/property/prop_gamma.c:prop_erf_odd --> (REQ: REQ-06-0766)
            ...
            - [ ] **Man Pages:** (REQ: REQ-06-0250, REQ-06-0404, REQ-06-0452, REQ-06-0488, REQ-06-0527, REQ-06-0573, REQ-06-0629, REQ-06-0668, REQ-06-0693, REQ-06-0734, REQ-06-0769, REQ-06-0793, REQ-06-0839)
                ...
                - [x] `man/man3/tgamma.3` — Gamma functions. Covers `tgamma()`, `lgamma()`, `lgamma_r()`, `signgam`. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, EXAMPLES, SEE ALSO. <!-- usr.man/man3/tgamma.3 --> (REQ: REQ-06-0771)
        - [x] **Bessel Functions (POSIX/XSI):** (REQ: REQ-06-0772)
            - [x] `j0()`, `j1()`, `jn()`: (REQ: REQ-06-0773)
                - [x] Bessel functions of the first kind: `J₀(x)`, `J₁(x)`, `Jₙ(x)`. <!-- lib/m/src/math.c:j0,j1,jn --> (REQ: REQ-06-0774)
                - [x] `j0(0)==1`, `j1(0)==0`, `jn(0, x)==j0(x)`, `jn(1, x)==j1(x)`. <!-- lib/m/src/math.c:2078,2087,2131 --> (REQ: REQ-06-0775)
                - [x] Implementation: polynomial/rational approximation for small x, asymptotic expansion for large x, Miller's backward recurrence for `jn()`. <!-- lib/m/src/math.c: Taylor series |x|<=8, Hankel asymptotic |x|>8, Miller backward recurrence in jn() when n>x --> (REQ: REQ-06-0776)
            - [x] `y0()`, `y1()`, `yn()`: (REQ: REQ-06-0777)
                - [x] Bessel functions of the second kind: `Y₀(x)`, `Y₁(x)`, `Yₙ(x)`. <!-- lib/m/src/math.c:y0,y1,yn --> (REQ: REQ-06-0778)
                - [x] Singular at `x=0`: `y0(0)==-∞`, `y1(0)==-∞`. <!-- lib/m/src/math.c:2096,2106 --> (REQ: REQ-06-0779)
                - [x] Domain: `x > 0` only. `y0(-1)` → NaN + `FE_INVALID`. <!-- lib/m/src/math.c:2095,2105 feraiseexcept(FE_INVALID) --> (REQ: REQ-06-0780)
            - [x] **Testing (`tests/lib/m/unit/test_bessel.c`):** (REQ: REQ-06-0781)
                - [x] Test `j0(0)==1`, `j1(0)==0`. <!-- tests/lib/m/unit/test_bessel.c:test_j0,test_j1 --> (REQ: REQ-06-0782)
                - [x] Test `j0(x)` known values: `j0(2.4048...)≈0` (first zero). <!-- tests/lib/m/unit/test_bessel.c:test_j0 --> (REQ: REQ-06-0783)
                - [x] Test `jn(0, x)==j0(x)`, `jn(1, x)==j1(x)`. <!-- tests/lib/m/unit/test_bessel.c:test_jn --> (REQ: REQ-06-0784)
                - [x] Test `y0(x)`, `y1(x)` known values. <!-- tests/lib/m/unit/test_bessel.c:test_y0,test_y1 --> (REQ: REQ-06-0785)
                - [x] Test `y0(0)==-∞`, `y1(0)==-∞`. <!-- tests/lib/m/unit/test_bessel.c:test_y0,test_y1 --> (REQ: REQ-06-0786)
                - [x] Test domain: `y0(-1)` → NaN, `y1(-1)` → NaN. <!-- tests/lib/m/unit/test_bessel.c — NaN + FE_INVALID --> (REQ: REQ-06-0787)
                - [x] Test NaN propagation. <!-- tests/lib/m/unit/test_bessel.c:test_nan_inf --> (REQ: REQ-06-0788)
            - [x] **Property Tests (`tests/lib/m/property/prop_bessel.c`):** (REQ: REQ-06-0789)
                - [x] Property: `jn(n, x)` satisfies Bessel recurrence: `J_{n-1}(x) + J_{n+1}(x) = (2n/x) * J_n(x)`. <!-- tests/lib/m/property/prop_bessel.c:prop_j_recurrence --> (REQ: REQ-06-0790)
                - [x] Property: `|j0(x)| <= 1` for all `x >= 0`. <!-- tests/lib/m/property/prop_bessel.c:prop_j0_bound --> (REQ: REQ-06-0791)
                - [x] Property: `yn(n, x)` satisfies same recurrence. <!-- tests/lib/m/property/prop_bessel.c:prop_y_recurrence --> (REQ: REQ-06-0792)
            - [ ] **Man Pages:** (REQ: REQ-06-0250, REQ-06-0404, REQ-06-0452, REQ-06-0488, REQ-06-0527, REQ-06-0573, REQ-06-0629, REQ-06-0668, REQ-06-0693, REQ-06-0734, REQ-06-0769, REQ-06-0793, REQ-06-0839)
                - [x] `man/man3/j0.3` — Bessel functions. Covers `j0()`, `j1()`, `jn()`, `y0()`, `y1()`, `yn()`. SYNOPSIS, DESCRIPTION, RETURN VALUE, ERRORS, EXAMPLES, SEE ALSO. <!-- usr.man/man3/j0.3 --> (REQ: REQ-06-0794)
    - [x] **Comprehensive Math Library Tests (Cross-Cutting):** (REQ: REQ-06-0795)
        - [x] **Fuzz Tests (`tests/lib/m/fuzz/fuzz_math.c`):** <!-- tests/lib/m/fuzz/fuzz_math.c --> (REQ: REQ-06-0796)
            - [x] Fuzz all implemented math functions with random `double` inputs (including denormals, ±0, ±∞, NaN, max/min values). (REQ: REQ-06-0797)
            - [x] Verify no crashes, no SIGFPE (unless expected), consistent NaN propagation. (REQ: REQ-06-0798)
            - [x] Verify errno is set correctly for domain/range errors. (REQ: REQ-06-0799)
            - [x] Verify `fetestexcept()` flags raised appropriately. (REQ: REQ-06-0800)
        - [x] **Fuzz Tests (`tests/lib/m/fuzz/fuzz_mathf.c`):** <!-- tests/lib/m/fuzz/fuzz_mathf.c --> (REQ: REQ-06-0801)
            - [x] Same as above but for all `float` variant functions. (REQ: REQ-06-0802)
        - [x] **Accuracy Tests (`tests/lib/m/unit/test_accuracy.c`):** <!-- tests/lib/m/unit/test_accuracy.c --> (REQ: REQ-06-0803)
            - [x] Compare all math functions against known high-precision reference values (MPFR or hardcoded tables). (REQ: REQ-06-0804)
            - [x] Verify results within 1 ULP for correctly-rounded functions, 2-3 ULP for faithfully-rounded. (REQ: REQ-06-0805)
            - [x] Test at boundary values: smallest/largest normal, smallest denormal, values near singularities. (REQ: REQ-06-0806)
    - [x] **Optimizations (i386/x87 Inline Assembly):** (REQ: REQ-06-0807)
        - [x] **Trigonometric Functions:** (REQ: REQ-06-0808)
            - [x] `sin()` / `cos()`: Use `fsin` / `fcos` instructions. <!-- lib/m/src/math_trig.c --> (REQ: REQ-06-0809)
            - [x] `sincos()`: Use `fsincos` if supported, else combine. <!-- lib/m/src/math_trig.c --> (REQ: REQ-06-0810)
            - [x] `tan()`: Use `fptan`; push `1.0` then `fptan`, ignore the 1.0. <!-- lib/m/src/math_trig.c --> (REQ: REQ-06-0811)
            - [x] `atan2()`: Use `fpatan`. <!-- lib/m/src/math_trig.c --> (REQ: REQ-06-0812)
            - [x] **Range Reduction:** Implement `fprem1` loop for arguments > 2^63 (Intel Manual reduction). <!-- lib/m/src/math_trig.c --> (REQ: REQ-06-0813)
        - [x] **Logarithms & Exponentials:** (REQ: REQ-06-0814)
            - [x] `log2(x)`: Load 1.0, load x, use `fyl2x`. <!-- lib/m/src/math_exp.c --> (REQ: REQ-06-0815)
            - [x] `log(x)`: Load ln(2), load x, use `fyl2x`. <!-- lib/m/src/math_exp.c --> (REQ: REQ-06-0816)
            - [x] `log10(x)`: Load log10(2), load x, use `fyl2x`. <!-- lib/m/src/math_exp.c --> (REQ: REQ-06-0817)
            - [x] `pow(x, y)`: <!-- lib/m/src/math_exp.c --> (REQ: REQ-06-0818)
                - [x] If y is integer, use binary exponentiation. <!-- lib/m/src/math_exp.c --> (REQ: REQ-06-0818)
                - [x] Else: `y * log2(x)` -> `frndint` -> `f2xm1` -> `fscale`. <!-- lib/m/src/math_exp.c --> (REQ: REQ-06-0820)
            - [x] `exp2(x)`: `f2xm1` (for -1 < x < 1) with `fscale` for integer part. <!-- lib/m/src/math_exp.c --> (REQ: REQ-06-0821)
        - [x] **Basic Arithmetic:** (REQ: REQ-06-0822)
            - [x] `sqrt()`: `fsqrt`. <!-- lib/m/src/math_exp.c --> (REQ: REQ-06-0823)
            - [x] `fabs()`: `fabs`. <!-- lib/m/src/math_round.c --> (REQ: REQ-06-0824)
            - [x] `fmod()`: `fprem`. <!-- lib/m/src/math_round.c --> (REQ: REQ-06-0825)
            - [x] `remainder()`: `fprem1` (IEEE compliant remainder). <!-- lib/m/src/math_round.c --> (REQ: REQ-06-0826)
        - [x] **Rounding & Manipulation:** (REQ: REQ-06-0827)
            - [x] `rint()`: `frndint` (honors current CW rounding mode). <!-- lib/m/src/math_round.c --> (REQ: REQ-06-0828)
            - [x] `nearbyint()`: `frndint` (without raising inexact exception). <!-- lib/m/src/math_round.c --> (REQ: REQ-06-0829)
            - [x] `scalbn(x, n)`: Load x, load n, `fscale`. <!-- lib/m/src/math_round.c --> (REQ: REQ-06-0830)
            - [x] `lrint()` / `llrint()`: `fistp` (store integer). <!-- lib/m/src/math_round.c --> (REQ: REQ-06-0831)
        - [x] **Testing (`tests/lib/m/unit/test_x87_opt.c`):** <!-- tests/lib/m/unit/test_x87_opt.c --> (REQ: REQ-06-0832)
            - [x] Verify x87-optimized functions produce identical results to generic C implementations. (REQ: REQ-06-0833)
            - [x] Test x87 range reduction: `sin(x)` for `x > 2^63` matches software range reduction. (REQ: REQ-06-0834)
            - [x] Test `fprem` loop termination (C1 flag check). (REQ: REQ-06-0835)
            - [x] Benchmark x87 vs generic: verify speedup for core functions. (REQ: REQ-06-0836)
            - [x] Test `nearbyint()` x87 path does NOT raise `FE_INEXACT` (vs `rint()` which does). (REQ: REQ-06-0837)
            - [x] Test `fistp` path for `lrint()`/`llrint()` matches current rounding mode. (REQ: REQ-06-0838)
        - [x] **Man Pages:** (REQ: REQ-06-0250, REQ-06-0404, REQ-06-0452, REQ-06-0488, REQ-06-0527, REQ-06-0573, REQ-06-0629, REQ-06-0668, REQ-06-0693, REQ-06-0734, REQ-06-0769, REQ-06-0793, REQ-06-0839)
            - [x] `man/man7/math_x87.7` — i386/x87 math optimizations overview. Document which functions use hardware instructions, range reduction strategy, accuracy guarantees, and when generic fallback is used. SYNOPSIS, DESCRIPTION, NOTES, SEE ALSO. <!-- usr.man/man7/math_x87.7 --> (REQ: REQ-06-0840)
...
- [ ] **System Interface Libraries:** (REQ: REQ-06-0922)
    ...
    - [ ] **Native Kernel Introspection (`libsys` / `libkernel`):** (REQ: REQ-06-0923)
        ...
        - [x] **Process Information API (`lib/sys`):** (REQ: REQ-06-1006)
            - [x] `sys_proc_count()`: total number of processes. <!-- lib/sys/proc.c --> (REQ: REQ-06-1007)
            - [x] `sys_proc_list(pid_t *pids, size_t *count)`: list all PIDs. <!-- lib/sys/proc.c --> (REQ: REQ-06-1008)
            - [x] `sys_proc_info(pid_t pid, sys_procinfo_t *info)`: detailed process info (incl. `bitness`). <!-- lib/sys/proc.c --> (REQ: REQ-06-1009)
            - [x] `sys_proc_threads(pid_t pid, tid_t *tids, size_t *count)`: list threads. <!-- lib/sys/proc.c --> (REQ: REQ-06-1010)
            - [x] `sys_proc_fds(pid_t pid, sys_fd_t *fds, size_t *count)`: list open FDs. <!-- lib/sys/proc.c --> (REQ: REQ-06-1011)
            - [x] `sys_proc_maps(pid_t pid, sys_map_t *maps, size_t *count)`: memory mappings. <!-- lib/sys/proc.c --> (REQ: REQ-06-1012)
            - [x] `sys_proc_cwd(pid_t pid, char *buf, size_t len)`: current working directory. <!-- lib/sys/proc.c --> (REQ: REQ-06-1013)
            - [x] `sys_proc_exe(pid_t pid, char *buf, size_t len)`: executable path. <!-- lib/sys/proc.c --> (REQ: REQ-06-1014)
            - [x] `sys_proc_cmdline(pid_t pid, char **argv, size_t *argc)`: command line. <!-- lib/sys/proc.c --> (REQ: REQ-06-1015)
            - [x] `sys_proc_environ(pid_t pid, char **envp, size_t *envc)`: environment. <!-- lib/sys/proc.c --> (REQ: REQ-06-1016)
            - [x] **Deep Requirements (Architecture-Critical):** (REQ: REQ-06-1006)
                - [x] Define stable, versioned semantics for every `sys_procinfo_t` field (required/optional/zeroed). (REQ: REQ-06-1006)
                - [x] Standardize two-pass sizing contract for variable-length APIs (`*_list`, `*_threads`, `*_fds`, `*_maps`, `*_cmdline`, `*_environ`). (REQ: REQ-06-1006)
                - [x] Define permission model for cross-process visibility (self, same-uid, privileged). (REQ: REQ-06-1006)
                - [x] Add regression tests proving deterministic output ordering and snapshot consistency. (REQ: REQ-06-1006)
                - [x] Primary consumers: `bin/ps`, `bin/top`, future `libkvm` compatibility layer. (REQ: REQ-06-1006)
        - [x] **Memory Statistics API (`lib/sys`):** (REQ: REQ-06-1017)
            - [x] `sys_vm_stats(sys_vmstat_t *stats)`: global VM statistics. <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1018)
            - [x] `sys_vm_info(sys_vminfo_t *info)`: memory zone info (DMA, Normal, HighMem). <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1019)
            - [x] `sys_vm_swap(sys_swapinfo_t *swap)`: swap usage statistics. <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1020)
            - [x] `sys_vm_buffers(sys_bufinfo_t *buf)`: buffer cache statistics. <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1021)
            - [x] `sys_vm_slabs(sys_slabinfo_t *slabs, size_t *count)`: slab allocator stats. <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1022)
            - [x] **Deep Requirements (Architecture-Critical):** (REQ: REQ-06-1017)
                - [x] Standardize all units (bytes/pages/KB) and document conversion rules in headers and man pages. (REQ: REQ-06-1017)
                - [x] Guarantee monotonic/coherent read semantics for counters exposed in one structure read. (REQ: REQ-06-1017)
                - [x] Define overflow-safe 32-bit userspace behavior for 64-bit counters. (REQ: REQ-06-1017)
                - [x] Add tests comparing `sys_vm_*` outputs to kernel PMM/VM internal counters. (REQ: REQ-06-1017)
                - [x] Primary consumers: `bin/free`, future `bin/top` memory panes, monitoring tools. (REQ: REQ-06-1017)
        - [x] **CPU Information API (`lib/sys`):** (REQ: REQ-06-1023)
            - [x] `sys_cpu_count()`: number of CPUs (online/possible/present). <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1024)
            - [x] `sys_cpu_info(int cpu, sys_cpuinfo_t *info)`: per-CPU info (model, MHz, cache). <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1025)
            - [x] `sys_cpu_times(int cpu, sys_cputimes_t *times)`: per-CPU time accounting. <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1026)
            - [x] `sys_cpu_loadavg(double *avg1, double *avg5, double *avg15)`: load averages. <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1027)
            - [x] `sys_cpu_topology(int cpu, sys_cputopo_t *topo)`: socket/core/thread topology. <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1028)
            - [x] **Deep Requirements (Architecture-Critical):** (REQ: REQ-06-1023)
                - [x] Define online/offline CPU semantics and hotplug behavior for `sys_cpu_count()`. (REQ: REQ-06-1023)
                - [x] Define per-CPU sampling interval and normalization for `sys_cpu_times()`. (REQ: REQ-06-1023)
                - [x] Document canonical load-average representation and rounding behavior. (REQ: REQ-06-1023)
                - [x] Add SMP-aware correctness tests under CPU affinity and migration. (REQ: REQ-06-1023)
                - [x] Primary consumers: `bin/top`, scheduler diagnostics, performance tooling. (REQ: REQ-06-1023)
        - [x] **System Information API (`lib/sys`):** (REQ: REQ-06-1029)
            - [x] `sys_uptime(struct timespec *ts)`: system uptime. <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1030)
            - [x] `sys_boottime(struct timespec *ts)`: boot timestamp. <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1031)
            - [x] `sys_hostname(char *buf, size_t len)`: system hostname. <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1032)
            - [x] `sys_domainname(char *buf, size_t len)`: NIS/YP domain name. <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1033)
            - [x] `sys_kernel_version(sys_version_t *ver)`: kernel version info. <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1034)
            - [x] **Deep Requirements (Architecture-Critical):** (REQ: REQ-06-1029)
                - [x] Define clock source and monotonic/realtime guarantees for uptime/boottime. (REQ: REQ-06-1029)
                - [x] Unify hostname/domainname behavior between `sysctl`, `uname`, and `sys_*` APIs. (REQ: REQ-06-1029)
                - [x] Add ABI stability tests for `sys_version_t` layout and string truncation rules. (REQ: REQ-06-1029)
                - [x] Primary consumers: shell/session tools, telemetry headers in `top`/`free`/`ps`. (REQ: REQ-06-1029)
        - [x] **Network Information API (`lib/sys`):** (REQ: REQ-06-1035)
            - [x] `sys_net_interfaces(sys_netif_t *ifs, size_t *count)`: list interfaces. <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1036)
            - [x] `sys_net_addrs(const char *ifname, sys_netaddr_t *addrs, size_t *count)`: addresses. <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1037)
            - [x] `sys_net_stats(const char *ifname, sys_netstats_t *stats)`: interface statistics. <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1038)
            - [x] `sys_net_routes(sys_route_t *routes, size_t *count)`: routing table. <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1039)
            - [x] `sys_net_arp(sys_arpentry_t *entries, size_t *count)`: ARP table. <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1040)
        - [x] **Disk / Storage Information API (`lib/sys`):** (REQ: REQ-06-1041)
            - [x] `sys_disk_list(sys_diskinfo_t *disks, size_t *count)`: list block devices. <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1042)
            - [x] `sys_disk_stats(const char *dev, sys_diskstat_t *stats)`: I/O stats per device. <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1043)
            - [x] `sys_mount_list(sys_mountinfo_t *mounts, size_t *count)`: mounted filesystems. <!-- lib/sys/sysinfo.c --> (REQ: REQ-06-1044)
            - [x] **Deep Requirements (Architecture-Critical):** (REQ: REQ-06-1041)
                - [x] Define canonical mount-entry ordering, mount flags normalization, and option representation. (REQ: REQ-06-1041)
                - [x] Guarantee consistency between mount control path (`mount(2)`, `umount(2)`) and reporting path (`sys_mount_list`). (REQ: REQ-06-1041)
                - [x] Add integration tests validating `bin/mount` output against `sys_mount_list`. (REQ: REQ-06-1041)
                - [x] Primary consumers: `bin/mount`, `bin/umount`, future storage dashboards. (REQ: REQ-06-1041)
        - [x] **Data Structures (`lib/sys/include/sys/sysinfo.h`):** (REQ: REQ-06-1045)
            - [x] `sys_procinfo_t`: pid, ppid, pgid, sid, uid, gid, state, name, times, memory. <!-- include/sys/sysinfo.h --> (REQ: REQ-06-1046)
            - [x] `sys_vmstat_t`: total, free, available, buffers, cached, swap_total, swap_free. <!-- include/sys/sysinfo.h --> (REQ: REQ-06-1047)
            - [x] `sys_cpuinfo_t`: vendor, model, family, stepping, mhz, cache_size, flags. <!-- include/sys/sysinfo.h --> (REQ: REQ-06-1048)
            - [x] `sys_cputopo_t`: socket_id, core_id, thread_id, numa_node. <!-- include/sys/sysinfo.h --> (REQ: REQ-06-1049)
            - [x] `sys_netif_t`: name, index, flags, mtu, hwaddr, type. <!-- include/sys/sysinfo.h --> (REQ: REQ-06-1050)
            - [x] `sys_diskinfo_t`: name, size, sector_size, model, serial. <!-- include/sys/sysinfo.h --> (REQ: REQ-06-1051)
            - [x] `sys_mountinfo_t`: device, mountpoint, fstype, flags. <!-- include/sys/sysinfo.h --> (REQ: REQ-06-1052)
