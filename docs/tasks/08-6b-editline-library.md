...
- [x] **Terminal Abstraction (`terminal.c`):** (REQ: REQ-08-0160)
    - [x] **Raw Mode:** (REQ: REQ-08-0161)
        - [x] `terminal_set_raw(EditLine *el)`: save orig termios, set raw (no echo, no canon, no signals, CS8, VMIN=1, VTIME=0). (REQ: REQ-08-0162)
        - [x] `terminal_set_orig(EditLine *el)`: restore saved termios. (REQ: REQ-08-0163)
    - [x] **Capability Queries:** (REQ: REQ-08-0164)
        - [x] Query terminal dimensions via `ioctl(TIOCGWINSZ)`. (REQ: REQ-08-0165)
        - [x] Fallback to `$COLUMNS` / `$LINES` environment variables. (REQ: REQ-08-0166)
        - [x] Fallback to 80×24 default. (REQ: REQ-08-0167)
        - [x] Cache dimensions; invalidate on `SIGWINCH`. (REQ: REQ-08-0168)
    - [x] **Termcap/Terminfo Support:** (REQ: REQ-08-0169)
        - [x] Query `$TERM` and look up capabilities. (REQ: REQ-08-0170)
        - [x] Support cursor motion: `cm` (absolute), `le`/`nd`/`up`/`do` (relative). (REQ: REQ-08-0171)
        - [x] Support clear: `cl` (screen), `ce` (to end of line), `cd` (to end of screen). (REQ: REQ-08-0172)
        - [x] Support insert/delete: `ic`/`dc` (char), `al`/`dl` (line). (REQ: REQ-08-0173)
        - [x] Support attributes: `md` (bold), `me` (reset), `so`/`se` (standout). (REQ: REQ-08-0174)
        - [x] Support scrolling: `sr` (reverse), `sf` (forward). (REQ: REQ-08-0175)
        - [x] Hardcoded ANSI/VT100 fallback when termcap unavailable. (REQ: REQ-08-0176)
    - [x] **Output Buffering:** (REQ: REQ-08-0177)
        - [x] Buffer terminal writes and flush in batches (reduce syscall overhead). (REQ: REQ-08-0178)
        - [x] Ensure flush on prompt display and before blocking reads. (REQ: REQ-08-0179)

- [x] **Signal Handling:** (REQ: REQ-08-0180)
    - [x] **`SIGWINCH` Handler:** (REQ: REQ-08-0181)
        - [x] Install handler in `el_gets`; remove on return. (REQ: REQ-08-0182)
        - [x] On receipt: re‑query `TIOCGWINSZ`, update stored dimensions, force full redraw. (REQ: REQ-08-0181)
    - [x] **`SIGINT` Handler:** (REQ: REQ-08-0184)
        - [x] If `EL_SIGNAL` is set: install handler in `el_gets`. (REQ: REQ-08-0184)
        - [x] On receipt: discard current line, print `^C\\n`, reset buffer, reprint prompt (do not exit). (REQ: REQ-08-0184)
    - [x] **`SIGQUIT` Handler:** (REQ: REQ-08-0187)
        - [x] If `EL_SIGNAL` is set: install handler, ignore `SIGQUIT` during editing. (REQ: REQ-08-0187)
    - [x] **`SIGTSTP` / `SIGCONT` Handler:** (REQ: REQ-08-0189)
        - [x] `SIGTSTP`: restore terminal, send `SIGTSTP` to self. (REQ: REQ-08-0190)
        - [x] `SIGCONT`: re‑enter raw mode, redraw prompt + buffer. (REQ: REQ-08-0191)
    - [x] **`SIGTERM` / `SIGHUP` Handler:** (REQ: REQ-08-0192)
        - [x] Restore terminal, propagate signal (allow clean shell exit). (REQ: REQ-08-0193)
    - [x] **Signal Mask Discipline:** (REQ: REQ-08-0194)
        - [x] Save old signal dispositions in `el_gets` entry, restore on exit. (REQ: REQ-08-0195)
        - [x] Use `sigaction` (not `signal`) for reliable semantics. (REQ: REQ-08-0196)
        - [x] Set `SA_RESTART` where appropriate. (REQ: REQ-08-0197)

- [x] **Key Map Subsystem:** (REQ: REQ-08-0198)
    - [x] **Key Map Structure:** (REQ: REQ-08-0199)
        - [x] Define `el_keymap_t` as a 256‑entry table mapping byte → action (function pointer or nested map pointer). (REQ: REQ-08-0200)
        - [x] Support multi‑byte key sequences via chained keymaps (e.g., ESC → sub‑map). (REQ: REQ-08-0201)
        - [x] Default keymaps: `emacs_keymap`, `vi_insert_keymap`, `vi_command_keymap`. (REQ: REQ-08-0202)
    - [x] **Key Binding API:** (REQ: REQ-08-0203)
        - [x] `el_set(el, EL_BIND, key_sequence, command_name)`: bind key to named command. (REQ: REQ-08-0204)
        - [x] `el_set(el, EL_BIND, key_sequence, NULL)`: unbind key. (REQ: REQ-08-0205)
        - [x] Parse key sequence notation: `^A`, `\\e`, `\\M-`, `\\C-`, `\\e[A`, literal chars. (REQ: REQ-08-0206)
    - [x] **Action Registry:** (REQ: REQ-08-0207)
        - [x] Register all built‑in editing commands with string names. (REQ: REQ-08-0208)
        - [x] Support user‑defined custom action callbacks via `el_set(el, EL_ADDFN, name, help, func)`. (REQ: REQ-08-0209)
        - [x] `el_set(el, EL_BIND, "-a", ...)`: bind in vi alternate (command) keymap. (REQ: REQ-08-0210)

- [x] **Programmable Completion:** (REQ: REQ-08-0211)
    - [x] **Completion Callback:** (REQ: REQ-08-0212)
        - [x] `el_set(el, EL_ADDFN, "complete", "Complete", completion_fn)`: register custom completion function. (REQ: REQ-08-0213)
        - [x] Callback signature: `unsigned char fn(EditLine *el, int ch)`. (REQ: REQ-08-0214)
        - [x] Default: filename completion (glob + opendir scan). (REQ: REQ-08-0215)
    - [x] **Filename Completion Engine:** (REQ: REQ-08-0216)
        - [x] Extract word under cursor (back to whitespace or special char). (REQ: REQ-08-0217)
        - [x] Expand `~` prefix before lookup. (REQ: REQ-08-0218)
        - [x] `opendir` + `readdir` scan for matching prefixes. (REQ: REQ-08-0219)
        - [x] If single match: insert remainder + trailing space (or `/` for directory). (REQ: REQ-08-0216)
        - [x] If multiple matches: insert common prefix, then list alternatives on second tab. (REQ: REQ-08-0216)
        - [x] Display alternatives in columns (use terminal width). (REQ: REQ-08-0222)
    - [x] **Integration with Shell:** (REQ: REQ-08-0223)
        - [x] Shell provides custom completion callback that understands builtins, paths, variables, hostnames. (REQ: REQ-08-0224)
        - [x] Document callback interface in `editline(3)`. (REQ: REQ-08-0225)
...
- [x] **Tokenizer (`tok_init` / `tok_str` / `tok_end`):** (REQ: REQ-08-0226)
    ...
    - [x] Handle continuation (incomplete quotes → return continuation status). (REQ: REQ-08-0232)
...
- [x] **`el_set()` / `el_get()` Operations:** (REQ: REQ-08-0234)
    ...
    - [x] `EL_SETFP`: set FILE * (in/out/err). (REQ: REQ-08-0251)
    - [x] `EL_EDITMODE`: enable/disable editing (passthrough mode). (REQ: REQ-08-0252)

- [x] **`el_source()` and `.editrc` Parsing:** (REQ: REQ-08-0253)
    - [x] `el_source(EditLine *el, const char *file)`: read and execute editrc commands. (REQ: REQ-08-0254)
    - [x] Default path: `~/.editrc` if `file` is `NULL`. (REQ: REQ-08-0255)
    - [x] **Supported `.editrc` Commands:** (REQ: REQ-08-0256)
        - [x] `bind [-a] [-e] [-v] [-s] [key [command]]`: bind key to command. (REQ: REQ-08-0257)
        - [x] `echotc cap [arg ...]`: execute termcap string. (REQ: REQ-08-0258)
        - [x] `edit [on|off]`: enable/disable editing. (REQ: REQ-08-0259)
        - [x] `settc cap value`: set termcap variable. (REQ: REQ-08-0260)
        - [x] `setty [-a] [-d] [-q] [-x] mode ...`: set tty modes. (REQ: REQ-08-0261)
    - [x] Comment lines (`#`) and blank lines. (REQ: REQ-08-0262)
    - [x] Per‑program sections: `prog:command args` (match `prog` against `el->prog`). (REQ: REQ-08-0263)
    - [x] Error reporting: print line number and diagnostic on parse errors, continue. (REQ: REQ-08-0264)

- [x] **History Management (`history.c`):** (REQ: REQ-08-0265)
    - [x] **Core (Implemented):** (REQ: REQ-08-0266)
        - [x] `history_init()`: allocate `History` with linked list, default max 100. (REQ: REQ-08-0267)
        - [x] `history_end(History *h)`: free all nodes and list. (REQ: REQ-08-0268)
        - [x] `H_SETSIZE` (1): set maximum entries; evict oldest on overflow. (REQ: REQ-08-0269)
        - [x] `H_ENTER` (10): add entry (skip duplicates of tail). (REQ: REQ-08-0270)
        - [x] `H_FIRST` (3): set cursor to head. (REQ: REQ-08-0271)
        - [x] `H_LAST` (4): set cursor to tail. (REQ: REQ-08-0272)
        - [x] `H_PREV` (5): move cursor backward (toward oldest). (REQ: REQ-08-0273)
    ...
    - [x] **Missing Operations:** (REQ: REQ-08-0275)
        ...
        - [x] `H_NEXT_STR` (13): search forward for entry containing string. (REQ: REQ-08-0282)
        - [x] `H_PREV_STR` (14): search backward for entry containing string. (REQ: REQ-08-0283)
        - [x] `H_NEXT_EVENT` (15): move to next entry matching event number. (REQ: REQ-08-0284)
        - [x] `H_PREV_EVENT` (16): move to previous entry matching event number. (REQ: REQ-08-0285)
        - [x] `H_LOAD` (17): load history from file (one entry per line). (REQ: REQ-08-0286)
        - [x] `H_SAVE` (18): save history to file. (REQ: REQ-08-0287)
        - [x] `H_CLEAR` (19): clear all entries. (REQ: REQ-08-0288)
    - [x] **History Numbering:** (REQ: REQ-08-0289)
        - [x] Assign monotonically increasing event numbers to entries. (REQ: REQ-08-0290)
        - [x] Set `HistEvent.num` on all operations that return an entry. (REQ: REQ-08-0291)
    - [x] **Unique History (Dedup):** (REQ: REQ-08-0292)
        - [x] Option to suppress consecutive duplicate entries (already done for tail). (REQ: REQ-08-0293)
        - [x] Option to remove all duplicates on insert (keep most recent). (REQ: REQ-08-0294)
    - [x] **Timestamp Support:** (REQ: REQ-08-0295)
        - [x] Store entry timestamp (for save/load fidelity with extended format). (REQ: REQ-08-0296)
    - [x] **File Format:** (REQ: REQ-08-0297)
        - [x] Simple format: one entry per line, newlines escaped as `\\n`. (REQ: REQ-08-0298)
        - [x] Write atomically (tmp + rename) to prevent corruption. (REQ: REQ-08-0299)

- [x] **Readline Compatibility Layer (`readline.c` — extended):** (REQ: REQ-08-0300)
    - [x] **Implemented:** (REQ: REQ-08-0077, REQ-08-0301)
        - [x] `el_gets(EditLine *el, int *count)`: main read loop with raw mode, insert, backspace, arrows, history, `^C`, `^D`, `^L`. (REQ: REQ-08-0302)
    - [x] **API Shim Functions (for applications using readline(3)):** (REQ: REQ-08-0303)
        - [x] `readline(const char *prompt)`: wrapper that creates/reuses `EditLine`, calls `el_gets`, returns `strdup`'d result. (REQ: REQ-08-0304)
        - [x] `add_history(const char *line)`: wrapper around `history(h, &ev, H_ENTER, line)`. (REQ: REQ-08-0305)
        - [x] `rl_bind_key(int key, rl_command_func_t *function)`: map to `el_set(EL_BIND, ...)`. (REQ: REQ-08-0306)
        - [x] `rl_set_prompt(const char *prompt)`: map to `el_set(EL_PROMPT, ...)`. (REQ: REQ-08-0307)
        - [x] `rl_on_new_line()`: map to refresh. (REQ: REQ-08-0308)
        - [x] `rl_forced_update_display()`: map to `el_set(EL_REFRESH)`. (REQ: REQ-08-0309)
    - [x] **Readline Variables (Global Shims):** (REQ: REQ-08-0310)
        - [x] `rl_line_buffer`: pointer to current line buffer. (REQ: REQ-08-0311)
        - [x] `rl_point`: cursor position. (REQ: REQ-08-0312)
        - [x] `rl_end`: end of buffer position. (REQ: REQ-08-0313)
        - [x] `rl_readline_name`: application name. (REQ: REQ-08-0314)
        - [x] `rl_attempted_completion_function`: completion callback. (REQ: REQ-08-0315)
    - [x] **Header:** provide `readline/readline.h` and `readline/history.h` compatibility headers. (REQ: REQ-08-0316)

- [x] **UTF‑8 / Wide‑Character Support:** (REQ: REQ-08-0317)
    - [x] **Input Decoding:** (REQ: REQ-08-0318)
        - [x] Decode multi‑byte UTF‑8 sequences on input into `wchar_t` codepoints. (REQ: REQ-08-0319)
        - [x] Validate sequences: reject overlong encodings, surrogates, values > U+10FFFF. (REQ: REQ-08-0320)
        - [x] Handle incomplete sequences at read boundary (buffer and re‑read). (REQ: REQ-08-0321)
    - [x] **Display Width:** (REQ: REQ-08-0322)
        - [x] Use `wcwidth()` / project‑local equivalent to determine display width of each codepoint. (REQ: REQ-08-0323)
        - [x] Handle zero‑width combining characters (display width 0). (REQ: REQ-08-0324)
        - [x] Handle CJK double‑width characters (display width 2). (REQ: REQ-08-0325)
        - [x] Handle non‑printable characters (display as `^X` or `\uXXXX`). (REQ: REQ-08-0326)
    - [x] **Cursor Navigation:** (REQ: REQ-08-0031, REQ-08-0327)
        - [x] `forward-char` / `backward-char` move by codepoint, not byte. (REQ: REQ-08-0328)
        - [x] `forward-word` / `backward-word` recognize Unicode word boundaries (letter/digit vs punctuation). (REQ: REQ-08-0329)
        - [x] Delete/backspace remove entire multi‑byte codepoint. (REQ: REQ-08-0330)
    - [x] **Line Buffer:** (REQ: REQ-08-0331)
        - [x] Store as UTF‑8 bytes internally (no wchar_t buffer). (REQ: REQ-08-0332)
        - [x] Cursor and length track byte positions; convert to character positions for display. (REQ: REQ-08-0333)
    - [x] **Locale Integration:** (REQ: REQ-08-0334)
        - [x] Detect locale encoding via `nl_langinfo(CODESET)` or `$LC_CTYPE`. (REQ: REQ-08-0335)
        - [x] Fall back to ASCII‑only mode if locale is not UTF‑8. (REQ: REQ-08-0336)

- [x] **Multi‑Line Editing:** (REQ: REQ-08-0337)
    - [x] Detect `\\n` within line buffer (e.g., from continuation or `el_insertstr`). (REQ: REQ-08-0338)
    - [x] Track logical lines vs physical screen rows. (REQ: REQ-08-0339)
    - [x] Up/Down arrow within multi‑line buffer: move between logical lines (not history). (REQ: REQ-08-0340)
    - [x] Correct cursor positioning across wrapped physical lines. (REQ: REQ-08-0341)
    - [x] `el_gets` continuation support: return continuation indicator for incomplete input (unmatched quotes, trailing `\\`). (REQ: REQ-08-0342)

- [x] **Testing:** (REQ: REQ-08-0343)
    - [x] **Unit Tests (`tests/lib/edit/`):** (REQ: REQ-08-0344)
        - [x] **Line Buffer Tests:** (REQ: REQ-08-0345)
            - [x] Insert at beginning, middle, end. (REQ: REQ-08-0346)
            - [x] Delete at beginning, middle, end. (REQ: REQ-08-0347)
            - [x] Cursor movement: forward/backward char, word, begin/end of line. (REQ: REQ-08-0348)
            - [x] Buffer growth: insert beyond initial capacity. (REQ: REQ-08-0349)
            - [x] Kill and yank: `^K`, `^U`, `^Y`. (REQ: REQ-08-0350)
            - [x] Undo: verify each operation reverts. (REQ: REQ-08-0351)
        - [x] **History Tests:** (REQ: REQ-08-0352)
            - [x] `H_ENTER` respects max size eviction. (REQ: REQ-08-0353)
            - [x] `H_PREV` / `H_NEXT` traverse entire list correctly. (REQ: REQ-08-0354)
            - [x] `H_NEXT_STR` / `H_PREV_STR` find correct entries. (REQ: REQ-08-0355)
            - [x] `H_LOAD` / `H_SAVE` round‑trip (write then read back, compare). (REQ: REQ-08-0356)
            - [x] `H_CLEAR` empties all entries. (REQ: REQ-08-0357)
            - [x] Duplicate suppression: consecutive and all‑duplicates modes. (REQ: REQ-08-0358)
        - [x] **Key Map Tests:** (REQ: REQ-08-0359)
            - [x] Bind, unbind, rebind a key; verify action dispatches correctly. (REQ: REQ-08-0360)
            - [x] Multi‑byte sequence binding (ESC `[A` → `previous-history`). (REQ: REQ-08-0361)
            - [x] Per‑program `.editrc` binding. (REQ: REQ-08-0362)
        - [x] **Tokenizer Tests:** (REQ: REQ-08-0363)
            - [x] Simple whitespace splitting. (REQ: REQ-08-0364)
            - [x] Quoted strings (single, double). (REQ: REQ-08-0365)
            - [x] Backslash escaping within quotes. (REQ: REQ-08-0366)
            - [x] Continuation detection (incomplete quotes). (REQ: REQ-08-0367)
        - [x] **`.editrc` Parser Tests:** (REQ: REQ-08-0368)
            - [x] Valid `bind` commands. (REQ: REQ-08-0369)
            - [x] Comment and blank line handling. (REQ: REQ-08-0370)
            - [x] Program‑specific sections. (REQ: REQ-08-0371)
            - [x] Malformed line: error message + continue. (REQ: REQ-08-0372)
        - [x] **Emacs Binding Tests:** (REQ: REQ-08-0373)
            - [x] Verify all `^A`–`^Z` and `M-*` bindings dispatch to correct actions. (REQ: REQ-08-0374)
        - [x] **Vi Mode Tests:** (REQ: REQ-08-0375)
            - [x] Insert → command → insert transitions. (REQ: REQ-08-0376)
            - [x] Motion commands (`w`, `b`, `e`, `$`, `0`). (REQ: REQ-08-0377)
            - [x] Edit commands (`d`, `c`, `x`, `r`, `p`). (REQ: REQ-08-0378)
            - [x] Count prefixes (`3dw`). (REQ: REQ-08-0379)
            - [x] Search (`/`, `?`, `n`, `N`). (REQ: REQ-08-0380)
    - [x] **Property Tests:** (REQ: REQ-08-0381)
        - [x] Line buffer invariants: `0 <= cursor <= len < cap`, buffer NUL‑terminated. (REQ: REQ-08-0382)
        - [x] History invariants: `size <= max_size`, doubly‑linked list consistency. (REQ: REQ-08-0383)
        - [x] Randomized editing sequences (insert, delete, move, undo) never crash or corrupt. (REQ: REQ-08-0384)
        - [x] Kill ring: yank always produces previously killed text. (REQ: REQ-08-0385)
    - [x] **Fuzz Tests:** (REQ: REQ-08-0386)
        - [x] Fuzz `el_gets` input stream with random bytes (harness: in‑memory `FILE *`). (REQ: REQ-08-0387)
        - [x] Fuzz `.editrc` parser with random config files. (REQ: REQ-08-0388)
        - [x] Fuzz tokenizer with random line buffers. (REQ: REQ-08-0389)
    - [x] **Integration Tests:** (REQ: REQ-08-0390)
        - [x] Drive `el_gets` via PTY: send escape sequences, verify output bytes. (REQ: REQ-08-0391)
        - [x] History file round‑trip: save, load, compare. (REQ: REQ-08-0392)
        - [x] Readline-compat: build a minimal readline app against `libedit`, verify prompt + completion. (REQ: REQ-08-0393)

- [x] **Documentation:** (REQ: REQ-08-0394)
    - [x] Man page: `editline(3)` — overview of the library. **(Exists)** (REQ: REQ-08-0395)
    - [x] Man page: `el_init(3)`, `el_gets(3)`, `el_set(3)`, `el_get(3)`, `el_line(3)`, `el_end(3)`, `el_reset(3)`, `el_source(3)`, `el_resize(3)` — API reference. **(Exists)** (REQ: REQ-08-0396)
    - [x] Man page: `editrc(5)` — `.editrc` file format and commands. **(Exists)** (REQ: REQ-08-0397)
    - [x] Man page: `history(3)` — history API (`history_init`, `history_end`, `H_*` ops). **(Exists)** (REQ: REQ-08-0398)
    - [x] Man page: `tok_init(3)` — tokenizer API. (REQ: REQ-08-0399)
    - [x] Update `editline(3)` with vi mode documentation. (REQ: REQ-08-0400)
    - [x] Update `editline(3)` with completion callback documentation. (REQ: REQ-08-0401)
    - [x] Update `history(3)` with `H_LOAD` / `H_SAVE` / timestamps documentation. (REQ: REQ-08-0402)
    - [x] Document readline compatibility layer and header shim. (REQ: REQ-08-0403)
