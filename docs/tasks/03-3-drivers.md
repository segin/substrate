...
### 3. Drivers (`sys/drivers`)
- [x] **Driver-Model Migration & Bus-Owned Handoff:** (Imported from `17-13`)
    ...
    - [x] Add `lsusb` as the USB-side inspection companion to `lspci` and `devtree`.
...
- [ ] **Userspace USB Library (`lib/usb` — libusb 1.0 Compatible):** (REQ: REQ-03-1150)
    ...
    - [x] **Device Enumeration (`device.c`):** (REQ: REQ-03-1202)
      ...
      - [x] Implement `libusb_get_port_number()`, `libusb_get_port_numbers()`, `libusb_get_port_path()`. (REQ: REQ-03-1207)
      - [x] Implement `libusb_get_parent()`. (REQ: REQ-03-1208)
      - [x] Implement `libusb_get_device_speed()`. (REQ: REQ-03-1209)
        - [x] Implement `libusb_get_max_packet_size()` / `libusb_get_max_iso_packet_size()`. (REQ: REQ-03-1210)

    - [x] **Device Open/Close & Configuration (`device.c`):** (REQ: REQ-03-1211)
      - [x] Implement `libusb_open(dev, hdl)` — open `/dev/usb/busN/devM` fd. (REQ: REQ-03-1212)
      - [x] Implement `libusb_open_device_with_vid_pid(ctx, vid, pid)` — convenience wrapper. (REQ: REQ-03-1213)
      ...
      - [x] Implement `libusb_get_configuration()` / `libusb_set_configuration()` via `USBDEVFS_SETCONFIGURATION`. (REQ: REQ-03-1217)
    ...
    - [x] **Hotplug (`hotplug.c`):** (REQ: REQ-03-1261)
      - [x] Implement `libusb_hotplug_register_callback()` — store callback with VID/PID/class filter. (REQ: REQ-03-1262)
      - [x] Implement `libusb_hotplug_deregister_callback()`. (REQ: REQ-03-1263)
      - [x] Implement `libusb_hotplug_get_user_data()`. (REQ: REQ-03-1264)
      - [x] Implement hotplug monitoring via `/proc/device-events` USB add/remove notifications. (REQ: REQ-03-1265)
      - [x] Implement `LIBUSB_HOTPLUG_ENUMERATE` flag — fire callbacks for already-connected devices. (REQ: REQ-03-1266)

    - [x] **Substrate Backend (`os/substrate.c`):** (REQ: REQ-03-1267)
        - [x] Implement `substrate_get_device_list()` — enumerate `/dev/usb/` tree. (REQ: REQ-03-1268)
        - [x] Implement `substrate_open()` / `substrate_close()` — open/close device node fd. (REQ: REQ-03-1269)
        - [x] Implement `substrate_submit_transfer()` — build and submit `usbdevfs_urb`. (REQ: REQ-03-1270)
        - [x] Implement `substrate_cancel_transfer()` — `USBDEVFS_DISCARDURB`. (REQ: REQ-03-1271)
        - [x] Implement `substrate_handle_transfer_completion()` — reap URBs via `USBDEVFS_REAPURBNDELAY`. (REQ: REQ-03-1272)
        - [x] Implement `substrate_claim_interface()` / `substrate_release_interface()`. (REQ: REQ-03-1273)
        - [x] Implement `substrate_set_configuration()` / `substrate_set_interface_alt_setting()`. (REQ: REQ-03-1274)
        - [x] Implement `substrate_clear_halt()` / `substrate_reset_device()`. (REQ: REQ-03-1275)
        - [x] Implement `substrate_kernel_driver_active()` / `substrate_detach_kernel_driver()` / `substrate_attach_kernel_driver()`. (REQ: REQ-03-1276)
        - [x] Implement `substrate_get_device_descriptor()` / `substrate_get_config_descriptor()` via ioctl. (REQ: REQ-03-1277)
    ...
    - [ ] **Testing:** (REQ: REQ-03-1286)
        - [x] **Unit Tests (Host Build):** (REQ: REQ-03-1287)
            ...
            - [x] `test_hotplug.c`: callback registration/deregistration, filter matching, enumerate flag. (REQ: REQ-03-1293)
    ...
    - [x] **Documentation:** (REQ: REQ-03-1305)
      - [x] Create `usr.man/man3/libusb_init.3` (and init family). (REQ: REQ-03-1306)
      - [x] Create `usr.man/man3/libusb_open.3`. (REQ: REQ-03-1307)
      - [x] Create `usr.man/man3/libusb_control_transfer.3`. (REQ: REQ-03-1308)
      - [x] Create `usr.man/man3/libusb_bulk_transfer.3`. (REQ: REQ-03-1309)
      - [x] Create `usr.man/man3/libusb_submit_transfer.3`. (REQ: REQ-03-1310)
      - [x] Create `usr.man/man3/libusb_handle_events.3`. (REQ: REQ-03-1311)
      - [x] Create `usr.man/man3/libusb_hotplug_register_callback.3`. (REQ: REQ-03-1312)
      - [x] Update `ARCHITECTURE.md` with `lib/usb` entry. (REQ: REQ-03-1313)
...
- [ ] **Storage Subsystem (Unified SCSI Stack):** (REQ: REQ-03-0001)
    ...
    - [x] **Core Architecture (CAM/Mid-layer):** (REQ: REQ-03-0002)
        ...
        - [x] **Command Execution:** (REQ: REQ-03-0077)
    ...
    - [ ] **Transport/HBA Drivers (Low-Level):** (REQ: REQ-03-0178)
        ...
        - [x] **VirtIO-SCSI (`virtio_scsi.c`):** (REQ: REQ-03-0204)
          - [x] `vscsi_execute(link, req)`: map `scsi_request_t` to virtio request descriptor. (REQ: REQ-03-0205)
            - [x] Build request header: LUN (8-byte SAM encoding), tag, task attributes, CDB. (REQ: REQ-03-0206)
            - [x] Attach data buffers as scatter-gather (device-readable for writes, device-writable for reads). (REQ: REQ-03-0207)
            - [x] Attach response buffer (sense data, residual, status). (REQ: REQ-03-0208)
            - [x] Submit to request virtqueue; wait for used buffer notification. (REQ: REQ-03-0209)
          - [x] `vscsi_setup_event_buffers()`: pre-populate event virtqueue. (REQ: REQ-03-0210)
          - [x] `vscsi_process_events()`: handle hotplug add/remove, transport reset, parameter change events. (REQ: REQ-03-0211)
            - [x] Control virtqueue: INQUIRY, REPORT LUNS, task management via control queue. (REQ: REQ-03-0212)
            - [x] Multi-queue support: one request virtqueue per vCPU (if negotiated). (REQ: REQ-03-0213)
...
- [x] **Floppy Disk Controller (`sys/drivers/storage/floppy/`):** (REQ: REQ-03-0327)
    ...
    - [x] **Motor Control:** (REQ: REQ-03-0350)
    ...
    - [ ] **Testing:** (REQ: REQ-03-0218, REQ-03-0317, REQ-03-0396, REQ-03-0594, REQ-03-0926, REQ-03-1120)
        ...
        - [x] Integration: disk change detection on QEMU. <!-- tests/sys/test_floppy_qemu.c, tests/sys/run_floppy_qemu.sh --> (REQ: REQ-03-0401)
        - [x] Integration: format + read‑back on blank floppy image. <!-- tests/sys/test_floppy_qemu.c, tests/sys/run_floppy_qemu.sh --> (REQ: REQ-03-0402)
    - [x] **Documentation:** (REQ: REQ-03-0236, REQ-03-0324, REQ-03-0403, REQ-03-0613, REQ-03-1138)
...
- [x] **NVMe:** (REQ: REQ-03-0422)
    ...
    - [x] **Namespace Discovery:** (REQ: REQ-03-0429)
    ...
    - [x] **I/O Queue Creation:** (REQ: REQ-03-0432)
        - [x] Determine number of queues supported. <!-- sys/drivers/storage/nvme/nvme.c, nvme.h, tests/sys/host_test_nvme.c --> (REQ: REQ-03-0433)
        - [x] Issue `Create I/O Completion Queue`. <!-- sys/drivers/storage/nvme/nvme.c, nvme.h, tests/sys/host_test_nvme.c --> (REQ: REQ-03-0434)
        - [x] Issue `Create I/O Submission Queue`. <!-- sys/drivers/storage/nvme/nvme.c, nvme.h, tests/sys/host_test_nvme.c --> (REQ: REQ-03-0435)
    - [x] **Block I/O:** (REQ: REQ-03-0436)
        - [x] PRP (Physical Region Page) List construction for data buffers. <!-- sys/drivers/storage/nvme/nvme.c, nvme.h, tests/sys/host_test_nvme.c --> (REQ: REQ-03-0437)
        - [x] Issue Read/Write NVM commands. <!-- sys/drivers/storage/nvme/nvme.c, nvme.h, tests/sys/host_test_nvme.c --> (REQ: REQ-03-0438)
- [x] **Partitioning & DevFS:** (REQ: REQ-03-0439)
...
- [ ] **Input:** <!-- ps2.c, keyboard.c, mouse.c, input.h --> (REQ: REQ-03-0442)
    ...
    - [ ] **Keyboard Driver (`sys/drivers/input/keyboard.c`, `keyboard.h`):** (REQ: REQ-03-0470)
        ...
        - [x] **Scancode Set Decoding:** (REQ: REQ-03-0476)
            ...
            - [x] **Set 2 (AT):** (REQ: REQ-03-0488)
                - [x] Native Set 2 decoder for controllers without translation. <!-- sys/drivers/input/keyboard.c:scancode2_to_keycode, scancode2_e0_to_keycode, keyboard_handle_set2 --> (REQ: REQ-03-0489)
                - [x] `0xF0` break prefix instead of bit 7. <!-- sys/drivers/input/keyboard.c:keyboard_handle_set2 — kbd_s2_break_pending --> (REQ: REQ-03-0490)
        ...
        - [x] **Input Subsystem Integration:** (REQ: REQ-03-0540, REQ-03-0578)
    ...
    - [ ] **Mouse Driver (`sys/drivers/input/mouse.c`, `mouse.h`):** (REQ: REQ-03-0549)
        ...
        - [x] **Packet Decoding:** (REQ: REQ-03-0555)
            ...
            - [x] **IntelliMouse Extension (4‑byte):** (REQ: REQ-03-0561)
                - [x] Detect via magic sample‑rate sequence (200, 100, 80 → ID 0x03). <!-- sys/drivers/input/ps2.c:ps2_mouse_knock + ps2_mouse_setup --> (REQ: REQ-03-0562)
                - [x] Byte 3: Z‑axis (scroll wheel), signed 8-bit on IntelliMouse (signed 4-bit on Explorer). <!-- sys/drivers/input/mouse.c:mouse_handler gen==3 branch --> (REQ: REQ-03-0563)
            - [x] **IntelliMouse Explorer (5‑button):** (REQ: REQ-03-0564)
                - [x] Detect via magic sequence (200, 200, 80 → ID 0x04). <!-- sys/drivers/input/ps2.c:ps2_mouse_setup second knock --> (REQ: REQ-03-0565)
                - [x] Byte 3: Z‑axis (bits 0–3), buttons 4–5 (bits 4–5). <!-- sys/drivers/input/mouse.c gen==4 branch — sign-extends 4-bit Z, BTN_SIDE/BTN_EXTRA --> (REQ: REQ-03-0566)
    ...
    - [ ] **Testing:** (REQ: REQ-03-0218, REQ-03-0317, REQ-03-0396, REQ-03-0594, REQ-03-0926, REQ-03-1120)
        - [ ] **Unit Tests:** (REQ: REQ-03-0219, REQ-03-0595, REQ-03-1121)
            - [x] Scancode Set 1 decoding: verify all single‑byte and E0‑extended codes. (REQ: REQ-03-0596)
            - [x] Modifier tracking: all combinations of shift/ctrl/alt/capslock/numlock. (REQ: REQ-03-0597)
            - [x] Keymap: verify ASCII output for full US‑QWERTY layout (unshifted, shifted, ctrl). (REQ: REQ-03-0598)
...
- [x] **Console Subsystem (`sys/console`):** (REQ: REQ-03-0617)
    ...
    - [x] **Backend Drivers:** (REQ: REQ-03-0679)
        - [x] **VGA Text Mode Console:** <!-- sys/drivers/video/hw_text.c, sys/include/sys/vt.h, sys/include/sys/vtio.h, sys/drivers/console/vt.c, sys/drivers/console/ansi_handler.c, tests/sys/host_test_hw_text.c, tests/sys/host_test_ansi.c, tests/sys/host_test_itimer.c, docs/specs/driver_vt.md, usr.man/man4/vt.4 --> (REQ: REQ-03-0680)
            ...
            - [x] **Scrolling:** <!-- sys/drivers/video/hw_text.c, tests/sys/host_test_hw_text.c, sys/drivers/console/ansi_handler.c, tests/sys/host_test_ansi.c, docs/specs/driver_vt.md, usr.man/man4/vt.4 --> (REQ: REQ-03-0695, REQ-03-0826)
                - [x] Software scroll (memmove video buffer). (REQ: REQ-03-0696)
                - [x] Hardware scroll (CRTC start address register). <!-- sys/drivers/video/hw_text.c, tests/sys/host_test_hw_text.c --> (REQ: REQ-03-0697)
            ...
            - [x] **Attributes:** <!-- sys/include/kern/ansi_handler.h, sys/drivers/console/ansi_handler.c, sys/include/sys/vt.h, sys/drivers/console/vt.c, sys/drivers/video/hw_text.c, tests/sys/host_test_ansi.c, tests/sys/host_test_hw_text.c, docs/specs/driver_vt.md, usr.man/man4/vt.4 --> (REQ: REQ-03-0699)
                ...
                - [x] Reverse video, bold, underline emulation. <!-- sys/include/kern/ansi_handler.h, sys/drivers/console/ansi_handler.c, sys/include/sys/vt.h, sys/drivers/console/vt.c, sys/drivers/video/hw_text.c, tests/sys/host_test_ansi.c, tests/sys/host_test_hw_text.c --> (REQ: REQ-03-0702)
        ...
        - [x] **Framebuffer Console (Graphical):** (REQ: REQ-03-0726)
            ...
            - [x] **Drivers:** (REQ: REQ-03-0732)
              - [x] **VESA VBE (Linear Framebuffer):** (REQ: REQ-03-0733)
                ...
                - [x] Linear framebuffer mapping (LFB base address). (REQ: REQ-03-0736)
                - [x] Protected mode interface (PM32 entry points). (REQ: REQ-03-0737)
                - [x] EDID retrieval for monitor detection. (REQ: REQ-03-0738)
                - [x] **UEFI GOP (Graphics Output Protocol):** (REQ: REQ-03-0739)
                  - [x] GOP protocol location and initialization. (REQ: REQ-03-0740)
                    - [x] Mode query and switching. (REQ: REQ-03-0741)
                  - [x] Framebuffer base and stride retrieval. (REQ: REQ-03-0742)
                    - [x] EFI runtime services integration. (REQ: REQ-03-0743)
                ...
                - [x] **VirtIO-GPU:** <!-- sys/drivers/virtio/virtio.c, sys/drivers/virtio/virtio.h, sys/drivers/virtio/virtio_gpu.c, tests/sys/host_test_virtio_gpu.c, tests/sys/Makefile, docs/specs/driver_virtio_gpu.md, usr.man/man4/virtio_gpu.4 --> (REQ: REQ-03-0750)
            ...
            - [x] **Rendering:** (REQ: REQ-03-0763)
                - [x] **Font Support:** (REQ: REQ-03-0764)
                    ...
                    - [x] PSF2 font parser (Unicode table, variable metrics). (REQ: REQ-03-0766)
                    - [x] BDF/PCF font support (optional). (REQ: REQ-03-0767)
                    - [x] Built-in fallback font (8x16 VGA ROM font). (REQ: REQ-03-0768)
                    - [x] Font glyph cache (hash table lookup). (REQ: REQ-03-0769)
                    - [x] Unicode to glyph mapping (cmap). (REQ: REQ-03-0770)
                - [x] **Blitting Operations:** (REQ: REQ-03-0771)
                    - [x] `fb_fillrect()`: Solid color fill with ROP support. (REQ: REQ-03-0772)
                    - [x] `fb_copyarea()`: Screen-to-screen blit. (REQ: REQ-03-0773)
                    - [x] `fb_imageblit()`: Mono/color image to framebuffer. (REQ: REQ-03-0774)
                    - [x] Accelerated ops detection and fallback. (REQ: REQ-03-0775)
                    - [x] Clipping (viewport bounds checking). (REQ: REQ-03-0776)
                - [x] **Character Rendering:** (REQ: REQ-03-0777)
                    - [x] Glyph rendering with foreground/background colors. (REQ: REQ-03-0778)
                    - [x] Bold rendering (shift and OR, or bold font). (REQ: REQ-03-0779)
            ...
            - [x] **Cursor:** (REQ: REQ-03-0788)
                ...
                - [x] **Hardware Cursor:** (REQ: REQ-03-0793)
                    ...
                    - [x] Cursor enable/disable. <!-- sys/drivers/virtio/virtio.h, sys/drivers/virtio/virtio_gpu.c, tests/sys/host_test_virtio_gpu.c, usr.man/man4/virtio_gpu.4, docs/specs/driver_virtio_gpu.md --> (REQ: REQ-03-0797)
            - [x] **Emulation (VT102/ANSI):** (REQ: REQ-03-0798)
                ...
                - [x] **Character Sets:** <!-- sys/include/kern/ansi_handler.h, sys/drivers/console/ansi_handler.c, tests/sys/host_test_ansi.c, docs/specs/driver_vt.md --> (REQ: REQ-03-0838)
                    ...
                    - [x] UTF-8 decoding and Unicode support. <!-- sys/include/kern/ansi_handler.h, sys/drivers/console/ansi_handler.c, tests/sys/host_test_ansi.c, docs/specs/driver_vt.md --> (REQ: REQ-03-0842)
                - [x] **Tabs:** <!-- sys/include/sys/vt.h, sys/include/kern/ansi_handler.h, sys/drivers/console/vt.c, sys/drivers/console/ansi_handler.c, sys/drivers/video/hw_text.c, tests/sys/host_test_ansi.c, tests/sys/host_test_hw_text.c, docs/specs/driver_vt.md --> (REQ: REQ-03-0843)
                    - [x] HTS (horizontal tab set). <!-- sys/include/kern/ansi_handler.h, sys/drivers/console/ansi_handler.c, sys/drivers/video/hw_text.c, tests/sys/host_test_ansi.c, tests/sys/host_test_hw_text.c, docs/specs/driver_vt.md --> (REQ: REQ-03-0844)
                    - [x] TBC (tab clear: current, all). <!-- sys/include/kern/ansi_handler.h, sys/drivers/console/ansi_handler.c, sys/drivers/video/hw_text.c, tests/sys/host_test_ansi.c, tests/sys/host_test_hw_text.c, docs/specs/driver_vt.md --> (REQ: REQ-03-0845)
                    - [x] CHT/CBT (cursor horizontal tab forward/back). <!-- sys/include/kern/ansi_handler.h, sys/drivers/console/ansi_handler.c, sys/drivers/video/hw_text.c, tests/sys/host_test_ansi.c, tests/sys/host_test_hw_text.c, docs/specs/driver_vt.md --> (REQ: REQ-03-0846)
            ...
            - [x] **TTY Binding:** <!-- sys/drivers/video/fb_console.c, tests/sys/host_test_fb_console.c, docs/specs/driver_fb_console.md --> (REQ: REQ-03-0703, REQ-03-0853, REQ-03-0879)
                ...
                - [x] Window size tracking (TIOCGWINSZ/TIOCSWINSZ). <!-- sys/drivers/video/fb_console.c, tests/sys/host_test_fb_console.c, docs/specs/driver_fb_console.md --> (REQ: REQ-03-0857)
    ...
    - [x] **Features:** <!-- sys/include/sys/vt.h, sys/drivers/console/vt.c, sys/drivers/input/keyboard.c, sys/drivers/video/vga.c, tests/sys/host_test_vt.c, tests/sys/host_test_keyboard.c --> (REQ: REQ-03-0889)
...
- [x] **RNG Subsystem (`/dev/random`, `/dev/urandom`):** <!-- random.c, random_internal.h, sys/random.h --> (REQ: REQ-03-0892)Infrastructure:** (REQ: REQ-03-0727, REQ-03-0893)
    ...
    - [x] **Device Interfaces:** <!-- random.c:410-494 --> (REQ: REQ-03-1011)
        ...
        - [x] **`getrandom()` Syscall:** (REQ: REQ-03-1029)
          - [x] Implement `sys_getrandom(buf, len, flags)`. (REQ: REQ-03-1030)
          - [x] `GRND_RANDOM` flag (use blocking pool). (REQ: REQ-03-1031)
          - [x] `GRND_NONBLOCK` flag (return EAGAIN if not seeded). (REQ: REQ-03-1032)
          - [x] `GRND_INSECURE` flag (return data even if not seeded). (REQ: REQ-03-1033)
          - [x] Personality support (native, Linux, FreeBSD). (REQ: REQ-03-1034)
          - [x] Register syscall number in all personality tables. (REQ: REQ-03-1035)
    ...
    - [x] **IOCTLs & Administrative Interface:** (REQ: REQ-03-1041)
        - [x] **IOCTL Commands:** (REQ: REQ-03-1042)
            - [x] `RNDGETENTCNT`: Return entropy estimate (bits). <!-- sys/include/sys/random.h, sys/kern/random.c, tests/sys/host_test_random_ioctl.c --> (REQ: REQ-03-1043)
            - [x] `RNDADDTOENTCNT`: Add to entropy count (privileged). <!-- sys/include/sys/random.h, sys/kern/random.c, tests/sys/host_test_random_ioctl.c --> (REQ: REQ-03-1044)
            - [x] `RNDADDENTROPY`: Add entropy data + credit (privileged). <!-- sys/include/sys/random.h, sys/kern/random.c, tests/sys/host_test_random_ioctl.c --> (REQ: REQ-03-1045)
            - [x] `RNDZAPENTCNT`: Zero entropy count (privileged). <!-- sys/include/sys/random.h, sys/kern/random.c, tests/sys/host_test_random_ioctl.c --> (REQ: REQ-03-1046)
            - [x] `RNDCLEARPOOL`: Clear entropy pool (privileged). <!-- sys/include/sys/random.h, sys/kern/random.c, tests/sys/host_test_random_ioctl.c --> (REQ: REQ-03-1047)
            - [x] `RNDRESEEDCRNG`: Force CSPRNG reseed (privileged). <!-- sys/include/sys/random.h, sys/kern/random.c, tests/sys/host_test_random_ioctl.c --> (REQ: REQ-03-1048)
        - [x] **Implementation:** (REQ: REQ-03-0976, REQ-03-1049)
            - [x] Implement `random_dev_ioctl()` dispatcher. <!-- sys/kern/random.c, tests/sys/host_test_random_ioctl.c --> (REQ: REQ-03-1050)
            - [x] Privilege checks (CAP_SYS_ADMIN or root). <!-- sys/kern/random.c, tests/sys/host_test_random_ioctl.c --> (REQ: REQ-03-1051)
            - [x] Input validation for user-provided entropy. <!-- sys/kern/random.c, tests/sys/host_test_random_ioctl.c --> (REQ: REQ-03-1052)
            - [x] Copyin/copyout for userspace buffers. <!-- sys/kern/random.c, tests/sys/host_test_random_ioctl.c --> (REQ: REQ-03-1053)
    ...
    - [x] **Security & Correctness:** (REQ: REQ-03-1059)
        - [x] **Fork Safety:** (REQ: REQ-03-1060)
            - [x] Implement `random_reseed_on_fork()` hook. <!-- sys/kern/random.c --> (REQ: REQ-03-1061)
            - [x] Call from `proc_fork()` after child creation. <!-- sys/pm/process.c:proc_fork_common --> (REQ: REQ-03-1062)
            - [x] Mix PID, timestamp into child's CSPRNG state. <!-- random_reseed_on_fork() --> (REQ: REQ-03-1063)
            - [x] Ensure parent and child diverge immediately. <!-- random_reseed_on_fork() forces reseed --> (REQ: REQ-03-1064)
            - [x] Wipe any copied CSPRNG buffer in child. <!-- explicit_bzero of seed_data --> (REQ: REQ-03-1065)
        - [x] **Exec Safety:** (REQ: REQ-03-1066)
            - [x] Wipe userspace-visible RNG state on `execve`. <!-- random_on_exec() in kern_execve --> (REQ: REQ-03-1067)
            - [x] Reset any per-process CSPRNG state. <!-- random_on_exec() forces reseed --> (REQ: REQ-03-1068)
            - [x] Ensure no entropy leakage across exec boundary. <!-- random_on_exec() --> (REQ: REQ-03-1069)
        - [x] **Memory Protection:** (REQ: REQ-03-1070)
            - [x] Use `explicit_bzero()` for sensitive state clearing. <!-- sys/kern/random.h, chacha20_rekey/wipe, sys_getrandom --> (REQ: REQ-03-1071)
            - [ ] Mark CSPRNG state pages non-swappable. (REQ: REQ-03-1072)
            - [x] Clear key material immediately after rekey. <!-- chacha20_rekey uses explicit_bzero --> (REQ: REQ-03-1073)
            - [x] Avoid leaving entropy in temporary buffers. <!-- explicit_bzero in sys_getrandom, random_reseed --> (REQ: REQ-03-1074)
    ...
    - [ ] **Performance Optimization:** (REQ: REQ-03-1104)
        - [x] **Fast Path:** (REQ: REQ-03-1105)
            - [x] Per-CPU CSPRNG state (avoid lock contention). <!-- percpu_rng[RANDOM_MAX_CPUS], random_percpu_get_bytes() --> (REQ: REQ-03-1106)
            - [x] Batch output generation (64-byte blocks). <!-- chacha20_extract() called per-CPU; full block cached in ctx --> (REQ: REQ-03-1107)
            - [x] Minimize lock hold time. <!-- per-CPU path skips global output_lock; only per-slot spinlock held --> (REQ: REQ-03-1108)
            - [x] Lockless entropy harvesting counters. <!-- __sync_fetch_and_add outside entropy_lock in random_harvest() --> (REQ: REQ-03-1109)
