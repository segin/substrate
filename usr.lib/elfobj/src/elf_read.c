...
static elf_err_t parse_relocations(elfobj_t *obj, symtab_index_t *maps, size_t map_count) {
    ...
    for (i = 0; i < obj->section_count; ++i) {
        ...
        if (obj->machine == EM_RISCV && sec->type != SHT_RELA) {
            return ELF_ERR_FORMAT;
        }
        if (obj->machine == EM_LOONGARCH && sec->type != SHT_RELA) {
            return ELF_ERR_FORMAT;
        }
        if (obj->machine == EM_68K && sec->type != SHT_RELA) {
            return ELF_ERR_FORMAT;
        }
        if (obj->machine == EM_VAX && sec->type != SHT_RELA) {
            return ELF_ERR_FORMAT;
        }
        if ((obj->machine == EM_PPC || obj->machine == EM_PPC64) && sec->type != SHT_RELA) {
            return ELF_ERR_FORMAT;
        }
        if (obj->machine == EM_ALPHA && sec->type != SHT_RELA) {
            return ELF_ERR_FORMAT;
        }
        if (obj->machine == EM_IA_64 && sec->type != SHT_RELA) {
            return ELF_ERR_FORMAT;
        }
...
static elf_err_t parse_object(elfobj_t *obj) {
    ...
    obj->abiversion = b[EI_ABIVERSION];
    ...
    if (obj->machine != EM_386 && obj->machine != EM_X86_64 &&
        ...
        obj->machine != EM_MIPS && obj->machine != EM_RISCV &&
