...
#define MODEPARSE_BITS (S_ISUID | S_ISGID | S_ISVTX | S_IRWXU | S_IRWXG | S_IRWXO)
...
static bool
parse_symbolic(const char *mode_string, struct mode_change *out,
...
struct mode_change *
modeparse_compile(const char *mode_string, char *errbuf, size_t errbuf_len)
{
    struct mode_change *mode;
    ...
    mode = (struct mode_change *)calloc(1, sizeof(*mode));
    if (mode == NULL) {
        set_error(errbuf, errbuf_len, "out of memory");
        return NULL;
    }
    ...
    if (is_octal_string(mode_string)) {
        ...
        if (parsed < 0 || parsed > (long)MODEPARSE_BITS) {
            set_error(errbuf, errbuf_len,
...
mode_t
modeparse_apply(const struct mode_change *mode, mode_t old_mode)
{
    size_t i;
    mode_t new_mode;
    ...
    if (mode->numeric) {
        return (old_mode & ~MODEPARSE_BITS) |
            (mode->numeric_mode & MODEPARSE_BITS);
    }

    new_mode = old_mode & MODEPARSE_BITS;
    ...
    return (old_mode & ~MODEPARSE_BITS) | (new_mode & MODEPARSE_BITS);
}
...
int
parse_mode(const char *mode_string, mode_t old_mode, mode_t *out_mode,
    char *errbuf, size_t errbuf_len)
{
    struct mode_change *mode;

    if (out_mode == NULL) {
        set_error(errbuf, errbuf_len, "output mode pointer is null");
        return -1;
    }

    mode = modeparse_compile(mode_string, errbuf, errbuf_len);
    if (mode == NULL) {
        return -1;
    }

    *out_mode = modeparse_apply(mode, old_mode);
    modeparse_free(mode);
    return 0;
}