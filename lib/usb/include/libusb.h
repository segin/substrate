...
#else
#define LIBUSB_FLEXIBLE_ARRAY 0
#endif
...
static inline uint16_t libusb_cpu_to_le16(const uint16_t x)
{
	...
	tmp.b8[1] = (uint8_t)(x >> 8);
	tmp.b8[0] = (uint8_t)(x & 0xff);
	return tmp.b16;
}
...
struct libusb_ssplus_usb_device_capability_descriptor {
	...
	struct libusb_ssplus_sublink_attribute sublinkSpeedAttributes[LIBUSB_FLEXIBLE_ARRAY];
};
...
typedef enum libusb_hotplug_event libusb_hotplug_event;

enum libusb_hotplug_flag {
	LIBUSB_HOTPLUG_ENUMERATE = (1 << 0)
};

typedef enum libusb_hotplug_flag libusb_hotplug_flag;
...
int LIBUSB_CALL libusb_get_config_descriptor_by_value(libusb_device *dev, uint8_t configuration_value, struct libusb_config_descriptor **config);
...
int LIBUSB_CALL libusb_get_interface_association_descriptors(libusb_device *dev, uint8_t config_index, struct libusb_interface_association_descriptor_array **iad_array);
	...
	unsigned int packet)
{
	...
	target = (int)packet;
	if (target >= transfer->num_iso_packets) {
		return NULL;
	}
	...
	unsigned int packet)
{
	...
	return transfer->buffer + ((int)transfer->iso_packet_desc[0].length * target);
}
	...
	uint8_t desc_type, uint8_t desc_index, unsigned char *data, int length)
{
	return libusb_control_transfer(dev_handle, LIBUSB_ENDPOINT_IN,
		...
		(uint16_t)((desc_type << 8) | desc_index), 0, data,
...
int LIBUSB_CALL libusb_hotplug_register_callback(libusb_context *ctx,
	int events, int flags, int vendor_id, int product_id, int dev_class,
